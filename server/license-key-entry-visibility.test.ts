...
describe("Kiểm tra key trước khi lưu", () => {
  it("luôn hiển thị key đang nhập và dùng biểu tượng mắt để xem key đã lưu", () => {
    ...
    expect(view).toContain('<input type="text" autoComplete="new-password" value={pendingKey}');
    expect(view).not.toContain('<input type="password" autoComplete="new-password" value={pendingKey}');
