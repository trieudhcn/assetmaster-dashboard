...
const accessoryCodeMocks = vi.hoisted(() => ({ getNextInventorySupplySequence: vi.fn() }));
const accessoryGroupMocks = vi.hoisted(() => ({ getNextAccessoryGroupSequence: vi.fn() }));

vi.mock("./db", () => ({ ...supplyImportMocks, ...accessoryCodeMocks, ...accessoryGroupMocks,
...
describe("company.get", () => {
  ...
  beforeEach(() => { supplyImportMocks.createSupplyImportSession.mockResolvedValue(501); supplyImportMocks.createSupplyImportItem.mockResolvedValue(1); supplyImportMocks.updateSupplyImportSession.mockResolvedValue(undefined); accessoryCodeMocks.getNextInventorySupplySequence.mockResolvedValue(8); accessoryGroupMocks.getNextAccessoryGroupSequence.mockResolvedValue(3); });
...
describe("asset Excel import safeguards", () => {
  ...
  it("returns the next PK accessory code for the create form", async () => {
    await expect(adminCaller().supplies.nextCode()).resolves.toEqual({ code: "PK-008" });
    expect(accessoryCodeMocks.getNextInventorySupplySequence).toHaveBeenCalledTimes(1);
  });
