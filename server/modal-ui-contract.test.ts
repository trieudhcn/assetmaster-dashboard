...
describe("modal presentation contract", () => {
  it("parses database decimals and Vietnamese separators as the same VND integer amount", () => {
    ...
    expect(numberToVietnameseWords(160000)).toBe(
  ...
  it("shows a shared inline warning and blocks accessory saving for invalid currency input", () => {
    const currencyInput = readProjectFile(
      "client/src/components/CurrencyInput.tsx"
    );
    const supplies = readProjectFile(
  ...
  it("handles Khấu hao/Thanh lý like assets returned to vendors", () => {
    ...
    expect(routers).toMatch(
      /asset\?\.status\s*!==\s*"returned_to_vendor"\s*&&\s*asset\?\.status\s*!==\s*"retired"/
    );
  });
  ...
  it("keeps new-maintenance request controls aligned when the estimated cost shows words", () => {
    ...
    expect(operations).toContain(
      'className="flex min-h-11 self-start items-center justify-center'
    );
  });
  ...
  it("prevents helper text below fields from stretching sibling controls across forms", () => {
    ...
    const catalogDropdowns = readProjectFile(
      "client/src/components/AssetCatalogDropdowns.tsx"
    );
    const supplies = readProjectFile(
      "client/src/pages/SuppliesInventoryView.tsx"
    );

    expect(currencyInput).toContain(
      'className="currency-input-with-helper w-full"'
    );
    expect(stylesheet).toContain(
      ".grid:has(.currency-input-with-helper) { align-items: start; }"
    );
    expect(stylesheet).toContain(
      ".form-helper-grid > * { align-self: start; }"
    );
  ...
  it("renders action tooltip from a body-level portal rather than a clipping pseudo-element", () => {
    const tooltipComponent = readProjectFile(
      "client/src/components/FloatingActionTooltip.tsx"
    );
    const home = readProjectFile("client/src/pages/Home.tsx");
  ...
  it("gives division action icons accessible labels for the shared hover tooltip", () => {
    const organization = readProjectFile(
      "client/src/pages/OrganizationManagementPage.tsx"
    );
  ...
  it("standardizes cancel labels and employee management icon tooltips", () => {
    const home = readProjectFile("client/src/pages/Home.tsx");
    const employees = readProjectFile(
  ...
  it("keeps category pagination compact and drawer close actions consistent", () => {
    const categories = readProjectFile(
      "client/src/pages/AssetCategoryManagementPage.tsx"
    );
    const employees = readProjectFile(
      "client/src/pages/EmployeeManagementView.tsx"
    );
    const historyDrawer = readProjectFile(
      "client/src/components/AssetImportRecovery.tsx"
    );
    const stylesheet = readProjectFile("client/src/index.css");

    expect(categories).toContain("const PAGE_SIZE = 5");
    expect(categories).toContain(
      "filteredCategories.slice(pageStart, pageStart + PAGE_SIZE)"
    );
  ...
  it("supports category search in both the asset form and category management list", () => {
    ...
    const categoryPicker = readProjectFile(
      "client/src/components/AssetCategoryPicker.tsx"
    );
  ...
  it("keeps purchase date sourced from persisted assets and restores the maintenance reason field", () => {
    const home = readProjectFile("client/src/pages/Home.tsx");
    expect(home).toContain(
      "purchaseDate: asset.purchaseDate ? dateInputValue(asset.purchaseDate)"
    );
    expect(home).toContain(
      "date: dateInputValue(asset.purchaseDate || asset.date)"
    );
  ...
  it("shows a guided import preview with update control, field comparison and detailed progress", () => {
    const importModal = readProjectFile(
      "client/src/components/AssetImportModal.tsx"
    );
    const importRecovery = readProjectFile(
      "client/src/components/AssetImportRecovery.tsx"
    );
    const importHistory = readProjectFile(
    ...
    expect(importRecovery).toContain(
      'aria-label="Thu nhỏ thẻ hoàn tác import"'
    );
  ...
  it("exposes a quantity-based supply inventory workflow with movements and low-stock warnings", () => {
    ...
    expect(home).toContain(
      "<SuppliesInventoryView canEditSectionLabels={isAdmin} />"
    );
    ...
    expect(supplies).toContain(
      'aria-label="Ghi nhận nhập, xuất hoặc cấp phát"'
    );
    ...
    expect(supplies).toContain(
      "Mã phụ kiện được hệ thống khóa sau khi tạo mới."
    );
  ...
  it("keeps supply search readable and supports staff or manual recipients for issues", () => {
    ...
    expect(routers).toContain(
      "Không tìm thấy nhân sự đang hoạt động được chọn."
    );
  });
  ...
  it("adds supply issue slips, stock returns, and an Excel history report", () => {
    ...
    const issueSlips = readProjectFile(
    ...
    expect(routers).toContain(
      'PK-${issueYear}-${String(sequence).padStart(3, "0")}'
    );
  ...
  it("allows one recipient to receive multiple supply types in one issue slip", () => {
    ...
    expect(supplies).toContain(
      "issuePreviewItems.map((item) => ({ supplyId: item.supplyId, quantity: item.quantity }))"
    );
    expect(supplies).toContain(
      "Một phụ kiện chỉ được chọn một lần trong cùng phiếu."
    );
    expect(routers).toMatch(
      /items:\s*z\s*\.\s*array\(\s*z\s*\.\s*object\(\s*\{\s*supplyId/
    );
    expect(routers).toContain(
      "Một phụ kiện chỉ được xuất một lần trong cùng phiếu."
    );
  });
  ...
  it("uses shared status tones and standard ten-row pagination for supply issue slips", () => {
    ...
    expect(issueSlips).toContain(
      "border-[#CDE5E5] bg-[#ECF8F7] text-[#087A6A]"
    );
    expect(issueSlips).toContain(
      "border-[#C7DDF8] bg-[#EAF3FF] text-[#2666A8]"
    );
  });
  ...
  it("opens the multi-item issue-slip PDF from the newly created slip and preserves every issued item", () => {
    ...
    expect(supplies).toContain(
      "issuePreviewItems.map((item) => ({ supplyCode:"
    );
  ...
  it("shows detailed accessory receipt and return history inside the employee profile", () => {
    ...
    expect(employees).toContain(
      "Đã trả {item.returnedQuantity} · Còn {outstanding}"
    );
  ...
  it("confirms and returns active licenses to inventory when an employee leaves", () => {
    ...
    expect(employees).toContain(
      "trpc.employees.activeLicenseAssignments.useQuery"
    );
    ...
    expect(routers).toContain(
      "revokeSoftwareLicenseAssignment(assignment.id, transaction)"
    );
  });
  ...
  it("includes handover accessories in employee holdings without creating a duplicate PK issue slip", () => {
    ...
    expect(issueSlips).toContain(
      "row.recipientUserId !== Number(selectedEmployeeId)"
    );
    expect(employees).toContain(
  ...
  it("shows only outstanding handover accessories with an actionable BG certificate in employee statistics", () => {
    ...
    expect(db).toContain(
      "handoverSupplyItems.issuedQuantity} - ${handoverSupplyItems.returnedQuantity} > 0"
    );
  ...
  it("groups held accessories by BG or PK source and previews the selected source without leaving supplies", () => {
    ...
    expect(supplies).toContain(
      "Gộp mỗi biên bản BG hoặc phiếu cấp phát PK thành một nhóm"
    );
    ...
    expect(supplies).toContain(
      "Biên bản bàn giao ${previewHandover.referenceCode}"
    );
  ...
  it("offers print and PDF export from the inline handover preview using the shared document template", () => {
    ...
    expect(supplies).toContain(
      'preparingPdf === "print" ? "Đang chuẩn bị..." : "In"'
    );
    expect(supplies).toContain(
      'preparingPdf === "export" ? "Đang tạo..." : "Xuất PDF"'
    );
  ...
  it("centers the inline BG preview and uses a user-defined, safe PDF filename", () => {
    ...
    expect(handoverPdf).toContain(
      "options?: { autoPrint?: boolean; fileName?: string }"
    );
  ...
  it("removes creation dates from PDF footers and remembers a custom BG filename by reference code", () => {
    ...
    expect(supplies).toContain(
      "handoverPdfFileNameStorageKey(handover.referenceCode)"
    );
  });
  ...
  it("keeps the BG dialog centered and persists PDF filenames for all document-code families", () => {
    ...
    expect(pdfExport).toContain(
      "options?.skipFilenamePrompt || rememberedFileName"
    );
  });
  ...
  it("styles the PDF preview close action with the same control language as print and download", () => {
    ...
    expect(css).toContain(
      '[role="dialog"][aria-label^="Xem trước"] > div > div:last-child > button:first-child'
    );
  ...
  it("uses a balanced fixed width and viewport-safe size for the inline BG preview dialog", () => {
    ...
    expect(css).toContain(
      "max-height: min(46rem, calc(100dvh - 3rem)) !important"
    );
  ...
  it("renders the active BG preview through a viewport-centered portal instead of the shared Dialog layout", () => {
    ...
    expect(supplies).toContain(
      "bg-handover-preview-overlay fixed inset-0 z-[320] flex min-h-dvh items-center justify-center"
    );
    expect(supplies).toContain(
      "bg-handover-preview-panel flex max-h-[calc(100dvh-2rem)]"
    );
    ...
    expect(supplies).not.toContain(
      'className="modal-close-action"><X size={13} />Đóng</button>'
    );
  ...
  it("opens handover accessories from the employee profile in the shared viewport overlay", () => {
    const employeeView = readProjectFile(
      "client/src/pages/EmployeeManagementView.tsx"
    );
    const supplies = readProjectFile(
      "client/src/components/SupplyIssueSlipManager.tsx"
    ...
    expect(employeeView).toContain(
      "<InlineHandoverPreviewDialog handoverId={previewHandoverId} onClose={() => setPreviewHandoverId(null)} />"
    );
    expect(employeeView).not.toContain(
  ...
  it("keeps the employee accessory history compact and suppresses unnecessary decimal zeroes", () => {
    ...
    expect(employees).toContain("<CompactEmployeeSupplyHistorySection");
    expect(employees).toContain(
      "Theo dõi từng phiếu, số lượng đã nhận và đã hoàn trả."
    );
  });
  ...
  it("filters accessory distribution statistics by searchable employee and reports active quantities", () => {
    ...
    expect(issueSlips).toContain(
      "<SearchableSelect value={selectedEmployeeId}"
    );
    expect(issueSlips).toContain("Nhân sự cần kiểm tra");
    expect(issueSlips).toContain(
      "Chọn một nhân sự để xem số lượng phụ kiện đang giữ và đã trả."
    );
    expect(issueSlips).toContain(
      "đang giữ · ${numberText(item.returned)} đã trả"
    );
    expect(issueSlips).toContain(
      'summaryLabel = mode === "department" ? "Đang cấp"'
    );
  ...
  it("keeps accessory certificate card actions side by side without wrapping on mobile", () => {
    ...
    expect(css).toContain(
      "Explicit classes keep each mobile certificate action"
    );
    expect(css).toContain("@media (max-width: 639px)");
    expect(css).toContain(".holding-source-card__actions");
    expect(supplies).toContain(
      "holding-source-card__actions grid w-full grid-cols-2"
    );
    expect(supplies).toContain(
      "holding-source-card__action inline-flex h-10 min-w-0"
    );
  ...
  it("stacks organization chart controls on mobile so department names and counts remain readable", () => {
    ...
    expect(css).toContain(
      ".organization-management-page section.mt-5.overflow-hidden"
    );
  ...
  it("adds a normalized quick search for organization names and codes with a clear affordance", () => {
    ...
    expect(organization).toContain(
      'className="organization-search-input field-input h-10 w-full text-xs"'
    );
    expect(organization).toContain(
      'className="organization-search-icon pointer-events-none absolute left-3 top-1/2 z-10'
    );
    expect(css).toContain(
      ".organization-quick-search .organization-search-input"
    );
    ...
    expect(organization).toContain(
      "organization-quick-search w-full min-w-0 lg:max-w-md"
    );
    expect(organization).toContain(
      "flex flex-col gap-3 border-b border-[#E7EEF3] px-5 py-5 lg:flex-row"
    );
    ...
    expect(organization).toContain(
      "const isCollapsed = !hasOrganizationSearchQuery && !expandedDepartmentIds.has(department.id)"
    );
  ...
  it("keeps the activity log clear control hidden when empty and supports the desktop slash shortcut", () => {
    ...
    const searchClear = readProjectFile(
      "client/src/components/SearchClearAffordance.tsx"
    );
  ...
  it("supports grouped retirement drafts with item-level reasons, signed copies, and a close-only-after-upload flow", () => {
    ...
    expect(router).toContain(
      "Hãy tải biên bản đã ký tay trước khi xác nhận đóng."
    );
    ...
    expect(manager).toContain(
      "onClose={() => closeCertificate.mutateAsync({ id: certificate.id })}"
    );
  });
  ...
  it("updates salvage values only after a signed copy, warns above purchase total, and groups retirement exports by TL code", () => {
    ...
    expect(router).toContain(
      "Chỉ có thể cập nhật giá trị thu hồi sau khi đã tải bản ký tay"
    );
  ...
  it("filters grouped retirement certificates by lifecycle status and protects the search input plus bulk deselection", () => {
    ...
    expect(css).toContain(
      ".retirement-certificate-search { padding-left: 2.7rem !important; }"
    );
  });
  ...
  it("keeps retirement certificate lists compact while exposing full details only when expanded", () => {
    ...
    expect(manager).toContain(
      "retirement-certificate-details-${certificate.id}"
    );
  ...
  it("prints one landscape grouped-retirement table and supports certificate search plus pagination", () => {
    ...
    expect(manager).toContain(
      'Hiển thị <b className="text-[#60758A]">{certificateStartRecord}–{certificateEndRecord}</b>'
    );
    ...
    expect(manager).not.toContain(
      'filteredCertificates.length > CERTIFICATE_PAGE_SIZE && <div className="mt-4 flex flex-col gap-3 border-t'
    );
  });
  ...
  it("brands issue-slip PDFs and protects automatically sourced recipient departments", () => {
    ...
    expect(supplies).toContain(
      "Phòng ban được khóa theo hồ sơ nhân sự đã chọn."
    );
  ...
  it("uses only company branding rather than the product name in document headers", () => {
    ...
    expect(operations).not.toContain(
      'doc.text(company.websiteTitle || "AssetMaster – Hệ thống Quản lý Tài sản"'
    );
    expect(operations).toContain(
      "Biên bản được lập từ dữ liệu đã chốt của đơn vị."
    );
  });
  ...
  it("shows an interactive calendar affordance and live asset totals for categories", () => {
    ...
    const datePicker = readProjectFile(
      "client/src/components/DatePickerField.tsx"
    );

    expect(home).toContain("data-purchase-date-picker");
    expect(home).toContain(
      "<DatePickerField value={dateInputValue(formData.date)}"
    );
  ...
  it("exposes warranty date input and preserves purchase date after maintenance", () => {
    ...
    expect(home).toContain(
      "DatePickerField value={dateInputValue(formData.warrantyUntil)}"
    );
    expect(home).toContain("disabled={preservePurchaseDate}");
    expect(routers).toMatch(
      /const\s+safeChanges\s*=\s*\{\s*\.\.\.persistedChanges,\s*\.\.\.supplierReturnChanges,\s*\.\.\.retirementChanges,\s*purchaseDate:\s*current\.purchaseDate\s*,?\s*\}/
    );
  ...
  it("shows readable date and status values in asset history", () => {
    ...
    expect(history).toContain(
      'fieldName === "status" || fieldName === "condition"'
    );
    expect(history).toContain(
      'fieldName === "purchaseDate" || fieldName === "warrantyUntil"'
    );
  ...
  it("keeps category search clearing contained and safeguards categories with linked assets", () => {
    ...
    expect(categories).toContain(
      '<div className="relative mt-3"><input value={query}'
    );
  ...
  it("shows warranty expiry warnings and provides warranty filters in the asset catalog", () => {
    ...
    expect(home).toContain(
      'warrantyFilter === "Sắp hết hạn" && warrantyState === "expiring"'
    );
...
it("locks returned-vendor and retired assets from manual editing and keeps them outside audit scope", () => {
  ...
  expect(home).toContain(
    'target.status === "retired" || target.status === "returned_to_vendor"'
  );
  expect(router).toContain('current.status === "retired"');
  expect(router).toMatch(
    /current\.status\s*===\s*"retired"\s*\|\|\s*current\.status\s*===\s*"returned_to_vendor"/
  );
  expect(router).toContain(
...
it("preserves retired status in the asset catalog and warns before selecting an asset still under warranty", () => {
  ...
  expect(home).toContain(
    'asset.status === "retired" ? "Khấu hao/Thanh lý" : "Sẵn có"'
  );
  expect(home).toContain(
    'asset.status === "retired" ? "retired" : "available"'
  );
...
it("shows warranty and repair cost references for assets in expanded retirement certificates", () => {
  ...
  expect(manager).toContain(
    "(assetsQuery.data || []).find((asset) => asset.id === previewRepairTicket.assetId)"
  );
});

it("summarizes disposed asset values by retirement year", () => {
  const retirement = readProjectFile(
    "client/src/pages/RetirementManagementView.tsx"
  );
  const reports = readProjectFile("client/src/pages/ReportsManagementView.tsx");
  const recordedMetric = readProjectFile(
    "client/src/components/RecordedSalvageMetric.tsx"
  );
  ...
  const reportRetirementStart = reports.indexOf(
  ...
  expect(overview).not.toContain(
    'OverviewMetric label="Biên bản trong phạm vi"'
  );
  ...
  expect(recordedMetric).toContain(
    "Chưa có tài sản trong phạm vi năm đang chọn."
  );
  ...
  expect(retirement).not.toContain(
    'OverviewMetric label="Tổng giá trị thanh lý"'
  );
...
it("opens the combined disposal PDF in an in-app preview before download", () => {
  ...
  const previewHost = readProjectFile(
    "client/src/components/ExportPreviewHost.tsx"
  );

  expect(retirement).toContain("Xem trước PDF gộp");
  expect(previewHost).toContain("Bản xem trước PDF gộp");
  expect(previewHost).toContain(
    "Kiểm tra nội dung trực tiếp trên web trước khi tải PDF."
  );
  expect(previewHost).toContain(
    'payload.kind === "pdf" ? "Tải PDF" : "Tải Excel"'
  );
});
...
it("uses the requested business order and display labels in the sidebar", () => {
  ...
  expect(home).toContain(
    'item.label === "Bảo hành & Sửa chữa" ? maintenanceBadgeCount : 0'
  );
});
...
it("offers common retirement reason templates for quick selection", () => {
  ...
  expect(home).toContain(
    "Chọn mẫu để điền nhanh; bạn vẫn có thể chỉnh sửa nội dung."
  );
...
it("separates warranty and repair tickets through a processing channel and tabs", () => {
  ...
  expect(operations).toContain(
    "const [serviceChannelTab, setServiceChannelTab]"
  );
  ...
  expect(routers).toContain(
    'input.serviceChannel === "warranty" ? "BH" : "SC"'
  );
  expect(routers).not.toContain(
    'serviceChannel: z.enum(["warranty", "repair"]).optional()'
  );
  expect(operations).toContain(
    "Kênh xử lý được xác lập theo mã phiếu và không thể thay đổi sau khi tạo."
  );
  expect(operations).not.toContain(
    "<SearchableSelect value={draft.serviceChannel}"
  );
});
...
it("uses the Bao hanh/Sua chua label for asset status and shows a prominent warranty warning", () => {
  ...
  expect(home).toContain(
    'normalizedStatus === "Bảo hành/Sửa chữa" && asset.statusType === "maintenance"'
  );
  expect(home).toContain(
    'const label = optionLabels?.[option] || (option === "Bảo trì" ? "Bảo hành/Sửa chữa" : option);'
  );
...
it("shows repair spending by asset and supports handover accessories from inventory or manual entry", () => {
  ...
  expect(routers).toMatch(/supplyItems:\s*z\s*\.\s*array/);
...
it("returns stocked handover accessories and summarizes service costs by year", () => {
  ...
  expect(routers).toContain("listActiveSoftwareLicenseAssignmentsForHandover");
  expect(routers).toContain("returnedLicenseCount");
  expect(routers).toContain("revoked_with_handover");
  expect(database).toContain("listHandoverSupplyItems");
  expect(database).toContain("listActiveSoftwareLicenseAssignmentsForHandover");
  expect(database).toContain("createHandoverSupplyItem");
  expect(home).toContain("Phụ kiện lấy từ kho");
  expect(home).toContain("phụ kiện kho đang giữ được hoàn về kho");
  expect(home).toContain("handoverLicenseAllocationsQuery");
  expect(home).toContain("data-handover-license-heading");
  expect(home).toContain("data-handover-license-cell");
  expect(home).toContain("headingRow.children[headingRow.children.length - 2]");
  ...
  expect(home).toContain(
    "Đã thu hồi ${returnedLicenseCount} Bản quyền về kho."
  );
...
it("uses one PDF preview and print flow from the quick service-ticket popup", () => {
  const reports = readProjectFile("client/src/pages/ReportsManagementView.tsx");
  const quickPreview = readProjectFile(
  ...
  expect(styles).toContain(
    '[role="dialog"][aria-label="Biên bản bàn giao"] > div > div:first-child'
  );
...
it("indexes recovery certificate numbers and gives every search input a quick-clear action", () => {
  ...
  const clearAffordance = readProjectFile(
    "client/src/components/SearchClearAffordance.tsx"
  );
  const searchableSelect = readProjectFile(
  ...
  expect(clearAffordance).toContain(
    'input.placeholder === "Tìm mã phiếu, tài sản..."'
  );
  expect(clearAffordance).toContain("Tìm mã phiếu, mã thu hồi...");
  expect(app).toContain("<SearchClearAffordance />");
  expect(searchableSelect).toContain(
    'aria-label="Xóa tìm kiếm trong dropdown"'
  );
  expect(home).toContain(
    'matchesVietnameseSearch(`${item.referenceCode} ${item.recoveryCertificateNumber || ""}`, query)'
  );
});
...
it("uses the warranty-repair label in asset forms and shows recovery numbers in returned handover rows", () => {
  ...
  expect(home).toContain(
    '"Thông tin này sẽ được lưu cùng tài sản để theo dõi và hiển thị trong thông báo Bảo hành/Sửa chữa."'
  );
  expect(home).toContain(
    'new CustomEvent("assetmaster-open-recovery-certificate"'
  );
  expect(home).toContain(
    "requestRecoveryCertificate(String(item.recoveryCertificateNumber))"
  );
  expect(home).toContain("Đang chuẩn bị PDF...");
  expect(home).toContain(
    "downloadAssetRecoveryPdf(pdfItem, companyInfo, { skipFilenamePrompt: true })"
  );
  expect(home).toContain(
    "Mở biên bản thu hồi ${item.recoveryCertificateNumber}"
  );
});

it("opens the existing recovery PDF preview when a TH certificate is selected from the handover list", () => {
  const app = readProjectFile("client/src/App.tsx");
  const shortcut = readProjectFile(
    "client/src/components/RecoveryCertificatePdfShortcut.tsx"
  );
  const home = readProjectFile("client/src/pages/Home.tsx");
  const operations = readProjectFile("client/src/pages/OperationsModules.tsx");
  ...
  expect(home).toContain(
    'statusFilter === "Đã có mã biên bản thu hồi" ? hasRecoveryCertificate'
  );
...
it("requires detailed confirmation and supports supplier return evidence", () => {
  ...
  expect(routers).toMatch(
    /storagePut\(\s*`assets\/\$\{asset\.id\}\/supplier-return\//
  );
...
it("excludes returned and disposed assets from company inventory statistics", () => {
  ...
  expect(home).toContain(
    'const inventoryAssetRows = useMemo(() => assetRows.filter((asset) => asset.statusType !== "returned" && asset.statusType !== "retired")'
  );
...
it("supports vendor and brand directory pagination and searchable-select keyboard affordances", () => {
  const vendorPage = readProjectFile(
    "client/src/pages/VendorBrandManagementPage.tsx"
  );
  const searchableSelect = readProjectFile(
    "client/src/components/SearchableSelect.tsx"
  );
...
it("supports supplier and brand value reports with compact currency modes", () => {
  ...
  const interactiveAllocation = readProjectFile(
    "client/src/components/InteractiveValueAllocation.tsx"
  );
  ...
  expect(interactiveAllocation).toContain(
    'className="modal-close-action">Đóng</button>'
  );
  const interactiveAssetDetails = readProjectFile(
    "client/src/components/InteractiveAllocationAssetDetails.tsx"
  );
  expect(reports).toContain("InteractiveAllocationAssetDetails");
  expect(interactiveAssetDetails).toContain(
    "Nhấn vào từng dòng để xem đầy đủ thông tin"
  );
  ...
  expect(interactiveAssetDetails).toContain(
    "Bàn giao ({assetHandovers.length})"
  );
  expect(interactiveAssetDetails).toContain(
    "Bảo hành/Sửa chữa ({assetTickets.length})"
  );
  expect(interactiveAssetDetails).toContain(
    "Chưa có lịch sử bàn giao cho tài sản này."
  );
  expect(interactiveAssetDetails).toContain(
    "Chưa có lịch sử Bảo hành/Sửa chữa cho tài sản này."
  );
  ...
  expect(interactiveAssetDetails).toContain(
    'Mở phiếu {item.serviceChannel === "warranty" ? "Bảo hành" : "Sửa chữa"}'
  );
  expect(interactiveAssetDetails).toContain(
    "assetmaster-open-maintenance-ticket-id"
  );
  ...
  expect(reports).not.toContain(
    'Theo tài sản</div><div className="overflow-x-auto rounded-lg border border-[#E0E3FF]"'
  );
});
...
describe("currency input and scrollbar contract", () => {
  it("uses the reusable currency input for maintenance costs", () => {
    ...
    expect(
      (operations.match(/showWords/g) || []).length
  ...
  it("formats typed and pasted VND values in the asset form", () => {
    ...
    expect(home).toContain(
      '<CurrencyInput value={String(formData.value || "")}'
    );
    expect(home).toContain("showWords />");
    expect(currencyInput).toContain("formatVndInput(value)");
    expect(currencyInput).toContain(
      'className="absolute right-11 top-1/2 z-10'
    );
    expect(currencyInput).toContain(
      "text-[10px] font-medium leading-4 text-[#8AA0B6]"
    );
  ...
  it("keeps handover filters and exports responsive", () => {
    ...
    const alertDialog = readProjectFile(
    ...
    expect(alertDialog).toContain(
      '"min-h-11 w-full justify-center px-5 text-sm active:scale-[0.98] sm:w-auto"'
    );
    ...
    expect(alertDialog).toContain(
      "fixed inset-0 z-[101] grid place-items-center p-4"
    );
    expect(alertDialog).toContain("max-w-[calc(100%-2rem)]");
    expect(home).toContain(
      "onPointerDownOutside={() => setSupplierReturnConfirmOpen(false)}"
    );
    expect(home).toContain(
      'className="relative z-20 flex flex-col gap-4 border-b'
    );
    expect(stylesheet).toContain(
      "The fixed application bar must stay above scrolling data toolbars"
    );
    expect(stylesheet).toContain("header.sticky.top-0.z-20 { z-index: 30; }");
    expect(home).toContain(
      "onEscapeKeyDown={() => setSupplierReturnConfirmOpen(false)}"
    );
    expect(home).toContain(
    ...
    expect(home).toContain(
      "Mở lịch sử thay đổi (${totalChanges ?? 0} bản ghi)"
    );
    ...
    expect(home).toContain(
      'classList.add("icon-action-tooltip", "cursor-help", "outline-none")'
    );
    expect(home).toContain('new CustomEvent("assetmaster:open-asset-history"');
    expect(home).toContain(
      "overflow-visible rounded-xl border border-[#DFE9F0]"
    );
    expect(home).toContain('className="w-full shrink-0 sm:w-[180px]"');
    expect(home).toContain(
      'className="flex flex-wrap items-center gap-2 border-b border-[#E7EEF3] bg-[#FBFCFD] px-5 py-4"'
    );
    expect(reports).toContain("xl:w-[680px] xl:grid-cols-4");
    expect(reports).toContain(
      "whitespace-nowrap rounded-lg border border-[#DDE7F0]"
    );
    ...
    expect(home).toContain(
      "handoverFilterDepartmentsQuery = trpc.departments.listAll.useQuery"
    );
    ...
    expect(home).toContain(
      'assetmaster-danh-sach-tai-san-${isFiltered ? "da-loc" : "tat-ca"}-'
    );
    ...
    expect(routers).toContain(
      'BG-${handoverYear}-${String(handoverSequence).padStart(3, "0")}'
    );
    ...
    const searchableSelectSource = readProjectFile(
      "client/src/components/SearchableSelect.tsx"
    );
    ...
    expect(searchableSelect).toContain(
      "setMenuPosition({ top: openUpward ? rect.top - 6 : rect.bottom + 6"
    );
    ...
    expect(readProjectFile("client/src/index.css")).toContain(
    ...
    expect(
      readProjectFile("client/src/components/MobileTableControls.tsx")
    ).toContain("mobile-column-controls");
    expect(
      readProjectFile("client/src/components/MobileTableControls.tsx")
  ...
  it("keeps vendor and brand searches inside directory headers", () => {
    const vendorBrand = readProjectFile(
      "client/src/pages/VendorBrandManagementPage.tsx"
    );
    ...
    expect(vendorBrand).toContain(
      "Tạo và lưu chứng từ tại menu Hợp đồng mua bán"
    );
...
describe("modal and drawer motion contract", () => {
  it("uses the shared AssetMaster motion markers across overlay primitives", () => {
    ...
    expect(stylesheet).toContain(
      '.assetmaster-overlay-motion[data-state="closed"]'
    );
    expect(stylesheet).toContain(
      '.assetmaster-modal-motion[data-state="closed"]'
    );
    expect(stylesheet).toContain(
      '.assetmaster-drawer-motion[data-state="closed"]'
    );
...
describe("empty illustration, modal skeleton and motion preference", () => {
  ...
  it("provides reusable table skeleton and motion preference toggle", () => {
    expect(
      readProjectFile("client/src/components/ModalTableSkeleton.tsx")
    ).toContain("modal-skeleton-line");
    const motionSettings = readProjectFile(
      "client/src/components/MotionSettingsRelocator.tsx"
    );
...
describe("maintenance history and filter layout contract", () => {
  ...
  it("keeps audit detail dropdowns above the card and constrains the asset picker", () => {
    ...
    expect(searchableSelect).toContain(
      "createPortal(menu, portalContainer || document.body)"
    );
  ...
  it("supports filtered audit sessions, continuous QR scanning and discrepancy exports", () => {
    ...
    expect(operations).toContain(
      'className="flex min-h-[2.45rem] items-center gap-2'
    );
    ...
    expect(operations).toContain(
      "Trang {activeAuditItemsPage}/{auditItemsPageCount}"
    );
    ...
    expect(readProjectFile("server/db.ts")).toContain(
      "^KK-${auditYear}-(\\\\d+)$"
    );
  });
  ...
  it("adds company branding to every exported Excel workbook and to audit PDFs", () => {
    ...
    const assetImport = readProjectFile(
      "client/src/components/AssetImportModal.tsx"
    );

    expect(workbookHelper).toContain(
  ...
  it("previews every export before downloading and applies configurable PDF logo watermarks", () => {
    ...
    const brandPanel = readProjectFile(
      "client/src/components/BrandEnhancementsPanel.tsx"
    );

    expect(previewHost).toContain(
      'exportPreviewEvent = "assetmaster:preview-export"'
    );
  ...
  it("excludes supplier-returned and disposed assets from editable audit choices and exports", () => {
    ...
    expect(operations).toContain(
      "tài sản Trả nhà cung cấp đã được loại trừ khỏi phạm vi kiểm kê."
    );
  ...
  it("uses a compact wrapped asset filter row with room for invoice filtering", () => {
    const home = readProjectFile("client/src/pages/Home.tsx");
    expect(home).toContain(
      "flex flex-wrap items-center gap-2 border-b border-[#E7EEF3] bg-[#FBFCFD] px-5 py-4"
    );
    expect(home).toContain("relative min-w-0 flex-[1_1_260px]");
    expect(home).toContain(
      "<FilterSelect value={invoice} onChange={onInvoiceChange} options={invoiceOptions} isLoading={isFilterDataLoading} />"
    );
  });
  ...
  it("places the handover branch inside the recipient detail card", () => {
    const home = readProjectFile("client/src/pages/Home.tsx");
    expect(home).toContain(
      '{item.department}</div><div className="mt-2 border-t border-[#E7EEF3] pt-2 text-xs font-semibold text-[#526779]">Chi nhánh: <span className="font-bold text-[#193B57]">{item.branch || "Chưa gán"}</span>'
    );
    expect(home).not.toContain(
      '{item.assetCode}</div><div className="mt-2 text-xs font-semibold text-[#D4F5F1]">Chi nhánh: {item.branch'
    );
  });
});
...
it("keeps maintenance UI controls on the shared interaction contracts", () => {
  ...
  const home = readProjectFile("client/src/pages/Home.tsx");
  const assetCatalogDropdowns = readProjectFile(
    "client/src/components/AssetCatalogDropdowns.tsx"
  );
  ...
  const brandEnhancements = readProjectFile(
    "client/src/components/BrandEnhancementsPanel.tsx"
  );
  ...
  expect(
    readProjectFile("client/src/components/SearchableSelect.tsx")
  ).toContain('aria-label="Xóa tìm kiếm trong dropdown"');
});
...
it("hiển thị trực tiếp logo người dùng cập nhật trong sidebar không có khung nền màu", () => {
  ...
  expect(home).toContain(
    'alt="Logo công ty" className="h-12 w-12 object-contain"'
  );
  expect(home).not.toContain(
    'style={{ backgroundColor: companyInfo.brandColor }} className="grid h-10 w-10'
  );
});

it("trình bày visual diff rõ ràng trong lịch sử phiên bản hướng dẫn", () => {
  const helpCenter = readProjectFile("client/src/pages/HelpCenter.tsx");
  const versionDiff = readProjectFile(
...
it("hiển thị thời điểm đồng bộ dashboard từ dữ liệu truy vấn thay vì mốc thời gian cố định", () => {
  ...
  expect(home).toContain(
    "const dashboardLastSyncedAt = assetQuery.dataUpdatedAt"
  );
...
it("hiển thị trạng thái đang chuẩn bị PDF và khóa thao tác lặp trên các biên bản", () => {
  ...
  const servicePreview = readProjectFile(
    "client/src/components/QuickServiceTicketPreview.tsx"
  );
  const operations = readProjectFile("client/src/pages/OperationsModules.tsx");
  ...
  expect(servicePreview).toContain(
    'pdfPreparing === "preview" ? "Đang chuẩn bị PDF..."'
  );
  expect(operations).toContain(
    'isExportingDiscrepancy === "pdf" ? "Đang chuẩn bị PDF..."'
  );
});
...
it("phân trang lịch sử biến động Phụ kiện để drawer không kéo dài khi nhiều giao dịch", () => {
  ...
  expect(supplies).toContain(
    "page: movementHistoryPage, pageSize: MOVEMENT_HISTORY_PAGE_SIZE"
  );
  expect(supplies).toContain(
