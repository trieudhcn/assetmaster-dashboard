...
describe("purchase contract management", () => {
  ...
  const database = readFileSync(
    resolve(import.meta.dirname, "./db.ts"),
    "utf8"
  );
  ...
  const contractsView = readFileSync(
    resolve(
      import.meta.dirname,
  ...
  const invoicesView = readFileSync(
    resolve(
      import.meta.dirname,
  ...
  const procurementView = readFileSync(
    resolve(
      import.meta.dirname,
      "../client/src/pages/ProcurementManagementView.tsx"
    ),
    "utf8"
  );
  const invoiceOperations = readFileSync(
    resolve(
      import.meta.dirname,
      "../client/src/components/InvoiceLineOperationsPanel.tsx"
    ),
    "utf8"
  );
  ...
  const vendorsView = readFileSync(
    resolve(
      import.meta.dirname,
  ...
  const employeeView = readFileSync(
    resolve(
      import.meta.dirname,
  ...
  const activeDirectoryPreview = readFileSync(
    resolve(
      import.meta.dirname,
  ...
  it("exposes controlled CRUD, S3 document handling and contract-item synchronization", () => {
    ...
    expect(router).toContain(
      "Nhà cung cấp của Tài sản được lấy theo Hợp đồng đã chọn"
    );
    expect(router).toContain(
  ...
  it("uses the vendor profile as a contract index instead of a second contract upload flow", () => {
    ...
    expect(contractsView).toContain(
  ...
  it("exposes controlled invoice APIs, direct asset links and S3 invoice documents", () => {
    ...
    expect(router).toContain(
      "Chỉ dòng loại Tài sản mới được dùng để gán Tài sản."
    );
    expect(router).toContain("purchase-invoices/${invoice.id}/documents/");
    expect(router).toMatch(
      /documentType:\s*z\s*\.\s*enum\(\s*\[\s*"invoice_pdf"\s*,\s*"invoice_xml"\s*,\s*"scan"\s*,\s*"delivery_note"\s*,\s*"adjustment"\s*,\s*"other"\s*,?\s*\]\s*\)/
    );
  });
  ...
  it("supports line-level reconciliation, asset allocation, supply receiving and Excel export", () => {
    ...
    expect(invoicesView).toContain(
      "trpc.purchaseInvoices.attachAsset.useMutation"
    );
    expect(invoicesView).toContain(
      "trpc.purchaseInvoices.receiveSupply.useMutation"
    );
    ...
    expect(
      invoicesView.indexOf("data-invoice-reconciliation-export")
    ).toBeLessThan(invoicesView.indexOf("Tạo hóa đơn"));
    expect(invoicesView).toContain(
      "sm:grid-cols-[140px_minmax(0,0.8fr)_84px_96px_minmax(180px,1.35fr)]"
    );
    expect(invoicesView).toContain('inputMode="decimal"');
    expect(invoicesView).toContain("quantityInput(event.target.value)");
    expect(invoicesView).toContain(
      'Đơn vị {line.unit || "Cái"} chỉ nhận số lượng nguyên.'
    );
  ...
  it("creates a new supply atomically from a supply invoice line and keeps contracts value-free in the form", () => {
    ...
    expect(invoicesView).toContain(
      "trpc.purchaseInvoices.createSupplyAndReceive.useMutation"
    );
  ...
  it("shows contract validity dates instead of total value in the contract list", () => {
    const tableStart = contractsView.indexOf(
  ...
  it("offers a read-only Active Directory preview before any employee sync is applied", () => {
    ...
    expect(activeDirectoryPreview).toContain(
      "Xem trước đồng bộ Active Directory"
    );
    expect(activeDirectoryPreview).toContain("CSV/JSON");
    expect(activeDirectoryPreview).toContain(
      "Màn hình này chỉ đọc và không ghi bất kỳ dữ liệu Nhân sự nào"
    );
  ...
  it("warns before discarding changed contract or invoice forms and defaults invoice lines to supplies", () => {
    ...
    expect(contractsView).toContain(
      "Các thay đổi Hợp đồng hiện tại sẽ bị hủy."
    );
  ...
  it("supports camera barcode scanning and keeps invoice action labels horizontally aligned", () => {
    expect(invoiceOperations).toContain(
  ...
  it("categorizes a supply created from an invoice line with an active group and brand", () => {
    ...
    expect(invoicesView).toContain("trpc.assetCategories.list.useQuery()");
    expect(invoicesView).toContain("trpc.brands.list.useQuery()");
    expect(invoiceOperations).toContain(
  ...
  it("keeps contracts focused on agreements and hides direct asset or supply linkage from the contract profile", () => {
    ...
    expect(contractsView).toContain(
      "Tài sản và Phụ kiện được đối soát trực tiếp theo Hóa đơn mua bán."
    );
  });
});
