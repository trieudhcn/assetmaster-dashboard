...
import { AllocationAssetDetails, InteractiveValueAllocation, type AllocationGroup } from "@/components/InteractiveValueAllocation";
...
export function ReportsManagementView() {
  ...
  const selectedAllocationAssets = useMemo(() => {
    ...
    }).map((asset) => ({ ...asset, departmentName: asset.departmentId ? departmentById.get(asset.departmentId)?.name || "Chưa gán" : "Chưa gán" }));
  }, [allocationSelection, inventoryAssets, employeeById, departmentById]);
  ...
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    {hasOrgError ? <section className="rounded-xl border border-[#F2D596] bg-[#FFF9EB] p-5"><div className="font-bold text-[#A86B00]">Không thể tải bộ lọc cơ cấu tổ chức</div><button onClick={() => { void employeesQuery.refetch(); void departmentsQuery.refetch(); void divisionsQuery.refetch(); }} className="mt-3 rounded-lg border border-[#F2D596] bg-white px-3 py-2 text-xs font-bold text-[#A86B00]">Thử lại</button></section> : <>
      ...
      {allocationSelection && <AllocationAssetDetails groupLabel={allocationSelection.type === "division" ? "Bộ Phận" : allocationSelection.type === "brand" ? "Hãng" : "Nhà cung cấp"} groupName={allocationSelection.name} assets={selectedAllocationAssets} currencyMode={currencyMode} onClose={() => setAllocationSelection(null)} />}
