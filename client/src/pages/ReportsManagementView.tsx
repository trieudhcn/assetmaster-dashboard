...
function ServiceCostTrendChart({ data, maxCost, yearLabel, currencyMode, selectedMonth, onSelectMonth }: { data: Array<{ month: string; repairCost: number; warrantyCost: number; totalCost: number }>; maxCost: number; yearLabel: string; currencyMode: CurrencyDisplayMode; selectedMonth: number | null; onSelectMonth: (monthIndex: number) => void }) {
  const dragState = useRef<{ pointerId: number; startX: number; startScrollLeft: number; dragged: boolean } | null>(null);
  const suppressSelectionRef = useRef(false);
  const selectMonth = (index: number) => { if (suppressSelectionRef.current) { suppressSelectionRef.current = false; return; } onSelectMonth(index); };
  const beginDrag = (event: ReactPointerEvent<HTMLDivElement>) => { if (event.pointerType === "mouse" && event.button !== 0) return; const container = event.currentTarget; dragState.current = { pointerId: event.pointerId, startX: event.clientX, startScrollLeft: container.scrollLeft, dragged: false }; container.setPointerCapture(event.pointerId); };
  const moveDrag = (event: ReactPointerEvent<HTMLDivElement>) => { const state = dragState.current; if (!state || state.pointerId !== event.pointerId) return; const delta = event.clientX - state.startX; if (Math.abs(delta) > 4) { state.dragged = true; event.currentTarget.scrollLeft = state.startScrollLeft - delta; event.preventDefault(); } };
  const endDrag = (event: ReactPointerEvent<HTMLDivElement>) => { const state = dragState.current; if (!state || state.pointerId !== event.pointerId) return; if (state.dragged) suppressSelectionRef.current = true; if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); dragState.current = null; };
  return <div className="rounded-xl border border-[#E0E8F0] bg-[#FBFCFE] p-3 sm:p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-xs font-extrabold text-[#193B57]">Biến động chi phí theo tháng</div><div className="mt-1 text-[10px] font-semibold text-[#71869A]">{yearLabel} · Nhấn cột để xem phiếu</div></div><div className="flex items-center gap-2 text-[10px] font-semibold text-[#60758A]"><span className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-[#E59B24]" />Sửa chữa</span><span className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-[#2666A8]" />Bảo hành</span></div></div><div onPointerDown={beginDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} className="mobile-cost-trend-scroll mt-4 -mx-1 cursor-grab overflow-x-auto px-1 pb-2 active:cursor-grabbing sm:mx-0 sm:cursor-auto sm:px-0"><div className="min-w-[540px] sm:min-w-0"><div className="flex h-36 items-end gap-2 border-b border-[#DCE7EF] pb-1">{data.map((item, index) => { const height = maxCost > 0 ? Math.max(3, Math.round((item.totalCost / maxCost) * 100)) : 3; const repairHeight = item.totalCost > 0 ? Math.round((item.repairCost / item.totalCost) * 100) : 0; const isSelected = selectedMonth === index; return <button type="button" key={item.month} onClick={() => selectMonth(index)} className={`group relative flex h-full min-w-9 flex-1 items-end rounded-md p-0.5 outline-none focus-visible:ring-2 focus-visible:ring-[#2666A8] ${isSelected ? "bg-[#EAF3FB]" : "hover:bg-[#F0F6FA]"}`} title={`${item.month}: ${currency(item.totalCost, currencyMode)} · Sửa chữa ${currency(item.repairCost, currencyMode)} · Bảo hành ${currency(item.warrantyCost, currencyMode)}`} aria-label={`Xem phiếu chi phí ${item.month}`}><div className={`relative w-full overflow-hidden rounded-t-[4px] bg-[#E8EDF2] transition-transform duration-150 group-hover:-translate-y-1 ${isSelected ? "ring-2 ring-[#2666A8]" : ""}`} style={{ height: `${height}%` }}><div className="absolute inset-x-0 bottom-0 bg-[#E59B24]" style={{ height: `${repairHeight}%` }} /><div className="absolute inset-x-0 top-0 bg-[#2666A8]" style={{ height: `${100 - repairHeight}%`, opacity: item.warrantyCost > 0 ? 1 : 0 }} /></div></button>; })}</div><div className="mt-2 flex gap-2">{data.map((item, index) => <button type="button" key={item.month} onClick={() => selectMonth(index)} className={`h-7 min-w-9 flex-1 rounded-md text-center text-[10px] font-extrabold outline-none focus-visible:ring-2 focus-visible:ring-[#2666A8] ${selectedMonth === index ? "bg-[#EAF3FB] text-[#2666A8]" : "text-[#71869A] hover:bg-[#F0F6FA]"}`}>{item.month}</button>)}</div></div></div><p className="-mt-1 text-[10px] font-medium text-[#8AA0B6] sm:hidden">Vuốt ngang để xem đủ 12 tháng.</p>{selectedMonth !== null && <p className="mt-2 text-[10px] font-bold text-[#2666A8]">Đang chọn {data[selectedMonth]?.month || `tháng ${selectedMonth + 1}`} · Danh sách phiếu hiển thị bên dưới.</p>}{maxCost === 0 && <p className="mt-3 text-center text-xs text-[#8AA0B6]">Chưa có chi phí trong năm đã chọn.</p>}</div>;
}
