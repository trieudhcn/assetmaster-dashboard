...
export function ReportsManagementView() {
  ...
  const serviceCostReport = useMemo(() => {
    ...
    const rows = (maintenanceQuery.data || []).filter((ticket) => {
      ...
      const channel = ticket.serviceChannel === "warranty" ? "warranty" : "repair";
      return assetById.has(ticket.assetId) && Number.isFinite(cost) && cost > 0 && (serviceCostYear === "all" || String(ticketYear) === serviceCostYear) && (serviceCostChannel === "all" || channel === serviceCostChannel);
    }).map((ticket) => {
  ...
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    {hasOrgError ? <section className="rounded-xl border border-[#F2D596] bg-[#FFF9EB] p-5"><div className="font-bold text-[#A86B00]">Không thể tải bộ lọc cơ cấu tổ chức</div><button onClick={() => { void employeesQuery.refetch(); void departmentsQuery.refetch(); void divisionsQuery.refetch(); }} className="mt-3 rounded-lg border border-[#F2D596] bg-white px-3 py-2 text-xs font-bold text-[#A86B00]">Thử lại</button></section> : <>
      ...
      <section className={`mt-5 ${card} overflow-hidden`}><div className="flex flex-col gap-4 border-b border-[#E7EEF3] px-5 py-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex items-center gap-2 text-sm font-extrabold text-[#193B57]"><FileBarChart size={17} className="text-[#5B5BD6]" />Tổng chi phí Bảo hành/Sửa chữa</div><p className="mt-1 text-xs text-[#71869A]">Tổng hợp chi phí thực tế theo năm và kênh xử lý trong phạm vi đang chọn.</p></div><div className="flex flex-wrap items-center gap-3"><div className="w-44"><SearchableSelect value={serviceCostChannel} onChange={(value) => setServiceCostChannel(value as "all" | "warranty" | "repair")} options={[{ value: "all", label: "Bảo hành & Sửa chữa" }, { value: "warranty", label: "Chỉ Bảo hành" }, { value: "repair", label: "Chỉ Sửa chữa" }]} placeholder="Bảo hành & Sửa chữa" searchPlaceholder="Tìm kênh xử lý..." /></div><div className="w-40"><SearchableSelect value={serviceCostYear} onChange={setServiceCostYear} options={[{ value: "all", label: "Tất cả các năm" }, ...serviceCostYears.map((year) => ({ value: String(year), label: `Năm ${year}` }))]} placeholder="Tất cả các năm" searchPlaceholder="Tìm năm..." /></div><div className="rounded-lg bg-[#EEF0FF] px-3 py-2 text-right"><div className="text-[10px] font-extrabold uppercase tracking-[0.1em] text-[#5B5BD6]">Tổng chi phí</div><div className="mt-1 text-sm font-extrabold text-[#3730A3]">{currency(serviceCostReport.totalCost, currencyMode)}</div></div></div></div>{maintenanceQuery.isLoading ? <div className="grid min-h-28 place-items-center text-sm text-[#71869A]">Đang tổng hợp chi phí Bảo hành/Sửa chữa...</div> : serviceCostReport.rows.length ? <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="grid flex-1 gap-3 sm:grid-cols-3"><div className="rounded-lg border border-[#E0E3FF] bg-[#F9F9FF] px-3 py-3"><div className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#6B66B7]">Phiếu có chi phí</div><div className="mt-1 text-xl font-extrabold text-[#3730A3]">{serviceCostReport.ticketCount}</div></div><div className="rounded-lg border border-[#D9E8F3] bg-[#F8FCFF] px-3 py-3"><div className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#3278BD]">Tài sản phát sinh</div><div className="mt-1 text-xl font-extrabold text-[#193B57]">{serviceCostReport.assetCount}</div></div><div className="rounded-lg border border-[#D5ECE7] bg-[#F4FBFA] px-3 py-3"><div className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#087A6A]">Kênh xử lý</div><div className="mt-1 text-xl font-extrabold text-[#193B57]">{serviceCostChannel === "all" ? "Tất cả" : serviceCostChannel === "warranty" ? "Bảo hành" : "Sửa chữa"}</div><div className="mt-1 text-[10px] font-semibold text-[#60758A]">Năm {serviceCostYear === "all" ? "tất cả" : serviceCostYear}</div></div></div><button onClick={exportRepairCostExcel} disabled={!isAdmin || exporting !== null} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg border border-[#C9CCF4] bg-[#F7F7FF] px-4 py-2.5 text-xs font-bold text-[#4A45A5] hover:bg-white disabled:cursor-not-allowed disabled:opacity-60"><Download size={15} className={exporting === "repairCosts" ? "animate-pulse" : ""} />{exporting === "repairCosts" ? "Đang chuẩn bị..." : `Xem trước Excel (${serviceCostReport.ticketCount})`}</button></div> : <div className="px-5 py-9 text-center text-sm text-[#71869A]">Chưa có phiếu {serviceCostChannel === "warranty" ? "Bảo hành" : serviceCostChannel === "repair" ? "Sửa chữa" : "Bảo hành/Sửa chữa"} phát sinh chi phí trong năm và phạm vi đã chọn.</div>}</section>
