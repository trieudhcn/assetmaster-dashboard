...
export function ReportsManagementView() {
  ...
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    {hasOrgError ? <section className="rounded-xl border border-[#F2D596] bg-[#FFF9EB] p-5"><div className="font-bold text-[#A86B00]">Không thể tải bộ lọc cơ cấu tổ chức</div><button onClick={() => { void employeesQuery.refetch(); void departmentsQuery.refetch(); void divisionsQuery.refetch(); }} className="mt-3 rounded-lg border border-[#F2D596] bg-white px-3 py-2 text-xs font-bold text-[#A86B00]">Thử lại</button></section> : <>
      ...
      <CompactValueAllocation title="Phân bổ giá trị theo Bộ Phận" context={selectedDivision?.name || selectedDepartment?.name || "Tất cả cơ cấu"} data={divisionValueData} totalValue={selectedValue} isLoading={assetsQuery.isLoading || employeesQuery.isLoading || divisionsQuery.isLoading} currencyMode={currencyMode} accent="#7666B3" />
      <CompactValueAllocation title="Phân bổ giá trị theo Hãng" data={brandValueData} totalValue={selectedValue} isLoading={assetsQuery.isLoading || brandsQuery.isLoading} currencyMode={currencyMode} accent="#0F8C8C" />
      <CompactValueAllocation title="Phân bổ giá trị theo Nhà cung cấp" data={supplierValueData} totalValue={selectedValue} isLoading={assetsQuery.isLoading} currencyMode={currencyMode} accent="#2666A8" />
