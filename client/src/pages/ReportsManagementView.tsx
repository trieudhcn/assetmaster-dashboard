...
function ServiceCostTrendChart({ data, maxCost, yearLabel, currencyMode }: { data: Array<{ month: string; repairCost: number; warrantyCost: number; totalCost: number }>; maxCost: number; yearLabel: string; currencyMode: CurrencyDisplayMode }) {
  return <div className="rounded-xl border border-[#E0E8F0] bg-[#FBFCFE] p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-extrabold text-[#193B57]">Biến động chi phí theo tháng</div><div className="mt-1 text-[10px] font-semibold text-[#71869A]">{yearLabel}</div></div><div className="flex items-center gap-2 text-[10px] font-semibold text-[#60758A]"><span className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-[#E59B24]" />Sửa chữa</span><span className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-[#2666A8]" />Bảo hành</span></div></div><div className="mt-4 flex h-32 items-end gap-1.5 border-b border-[#DCE7EF] pb-1">{data.map((item) => { const height = maxCost > 0 ? Math.max(3, Math.round((item.totalCost / maxCost) * 100)) : 3; const repairHeight = item.totalCost > 0 ? Math.round((item.repairCost / item.totalCost) * 100) : 0; return <div key={item.month} className="group relative flex h-full min-w-0 flex-1 items-end" title={`${item.month}: ${currency(item.totalCost, currencyMode)} · Sửa chữa ${currency(item.repairCost, currencyMode)} · Bảo hành ${currency(item.warrantyCost, currencyMode)}`}><div className="relative w-full overflow-hidden rounded-t-[4px] bg-[#E8EDF2] transition-transform duration-150 group-hover:-translate-y-1" style={{ height: `${height}%` }}><div className="absolute inset-x-0 bottom-0 bg-[#E59B24]" style={{ height: `${repairHeight}%` }} /><div className="absolute inset-x-0 top-0 bg-[#2666A8]" style={{ height: `${100 - repairHeight}%`, opacity: item.warrantyCost > 0 ? 1 : 0 }} /></div></div>; })}</div><div className="mt-2 flex gap-1.5">{data.map((item) => <div key={item.month} className="min-w-0 flex-1 text-center text-[9px] font-semibold text-[#71869A]">{item.month}</div>)}</div>{maxCost === 0 && <p className="mt-3 text-center text-xs text-[#8AA0B6]">Chưa có chi phí trong năm đã chọn.</p>}</div>;
}
