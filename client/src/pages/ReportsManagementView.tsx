...
function ServiceCostTrendChart({ data, maxCost, yearLabel, currencyMode, selectedMonth, onSelectMonth }: { data: Array<{ month: string; repairCost: number; warrantyCost: number; totalCost: number }>; maxCost: number; yearLabel: string; currencyMode: CurrencyDisplayMode; selectedMonth: number | null; onSelectMonth: (monthIndex: number) => void }) {
  return <div className="rounded-xl border border-[#E0E8F0] bg-[#FBFCFE] p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-extrabold text-[#193B57]">Biến động chi phí theo tháng</div><div className="mt-1 text-[10px] font-semibold text-[#71869A]">{yearLabel} · Nhấn cột để xem phiếu</div></div><div className="flex items-center gap-2 text-[10px] font-semibold text-[#60758A]"><span className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-[#E59B24]" />Sửa chữa</span><span className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-[#2666A8]" />Bảo hành</span></div></div><div className="mt-4 flex h-32 items-end gap-1.5 border-b border-[#DCE7EF] pb-1">{data.map((item, index) => { const height = maxCost > 0 ? Math.max(3, Math.round((item.totalCost / maxCost) * 100)) : 3; const repairHeight = item.totalCost > 0 ? Math.round((item.repairCost / item.totalCost) * 100) : 0; const isSelected = selectedMonth === index; return <button type="button" key={item.month} onClick={() => onSelectMonth(index)} className={`group relative flex h-full min-w-0 flex-1 items-end rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-[#2666A8] ${isSelected ? "bg-[#EAF3FB]" : ""}`} title={`${item.month}: ${currency(item.totalCost, currencyMode)} · Sửa chữa ${currency(item.repairCost, currencyMode)} · Bảo hành ${currency(item.warrantyCost, currencyMode)}`} aria-label={`Xem phiếu chi phí ${item.month}`}><div className={`relative w-full overflow-hidden rounded-t-[4px] bg-[#E8EDF2] transition-transform duration-150 group-hover:-translate-y-1 ${isSelected ? "ring-2 ring-[#2666A8]" : ""}`} style={{ height: `${height}%` }}><div className="absolute inset-x-0 bottom-0 bg-[#E59B24]" style={{ height: `${repairHeight}%` }} /><div className="absolute inset-x-0 top-0 bg-[#2666A8]" style={{ height: `${100 - repairHeight}%`, opacity: item.warrantyCost > 0 ? 1 : 0 }} /></div></button>; })}</div><div className="mt-2 flex gap-1.5">{data.map((item, index) => <button type="button" key={item.month} onClick={() => onSelectMonth(index)} className={`min-w-0 flex-1 text-center text-[9px] font-semibold outline-none focus-visible:text-[#2666A8] ${selectedMonth === index ? "text-[#2666A8]" : "text-[#71869A]"}`}>{item.month}</button>)}</div>{maxCost === 0 && <p className="mt-3 text-center text-xs text-[#8AA0B6]">Chưa có chi phí trong năm đã chọn.</p>}</div>;
}
...
export function ReportsManagementView() {
  ...
  const monthlyServiceCostTrend = useMemo(() => {
    const months = Array.from({ length: 12 }, (_, index) => ({ month: `T${index + 1}`, repairCost: 0, warrantyCost: 0, totalCost: 0 }));
    serviceCostReport.allRows.forEach((item) => {
      const date = new Date(item.openedAt);
      if (!Number.isFinite(date.getTime())) return;
      const month = months[date.getMonth()];
      if (!month) return;
      if (item.channel === "Bảo hành") month.warrantyCost += item.actualCost;
      else month.repairCost += item.actualCost;
  ...
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    {hasOrgError ? <section className="rounded-xl border border-[#F2D596] bg-[#FFF9EB] p-5"><div className="font-bold text-[#A86B00]">Không thể tải bộ lọc cơ cấu tổ chức</div><button onClick={() => { void employeesQuery.refetch(); void departmentsQuery.refetch(); void divisionsQuery.refetch(); }} className="mt-3 rounded-lg border border-[#F2D596] bg-white px-3 py-2 text-xs font-bold text-[#A86B00]">Thử lại</button></section> : <>
      ...
      <section className={`mt-5 ${card} overflow-hidden`}><div className="flex flex-col gap-4 border-b border-[#E7EEF3] px-5 py-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex items-center gap-2 text-sm font-extrabold text-[#193B57]"><FileBarChart size={17} className="text-[#5B5BD6]" />Tổng chi phí Bảo hành/Sửa chữa</div><p className="mt-1 text-xs text-[#71869A]">Theo dõi tỷ trọng kênh xử lý và nhấn cột tháng để xem danh sách phiếu chi tiết.</p></div><div className="flex flex-wrap items-center gap-3"><div className="w-44"><SearchableSelect value={serviceCostChannel} onChange={(value) => setServiceCostChannel(value as "all" | "warranty" | "repair")} options={[{ value: "all", label: "Bảo hành & Sửa chữa" }, { value: "warranty", label: "Chỉ Bảo hành" }, { value: "repair", label: "Chỉ Sửa chữa" }]} placeholder="Bảo hành & Sửa chữa" searchPlaceholder="Tìm kênh xử lý..." /></div><div className="w-40"><SearchableSelect value={serviceCostYear} onChange={(value) => { setServiceCostYear(value); setSelectedServiceCostMonth(null); }} options={[{ value: "all", label: "Tất cả các năm" }, ...serviceCostYears.map((year) => ({ value: String(year), label: `Năm ${year}` }))]} placeholder="Tất cả các năm" searchPlaceholder="Tìm năm..." /></div></div></div>{maintenanceQuery.isLoading ? <div className="grid min-h-40 place-items-center text-sm text-[#71869A]">Đang tổng hợp chi phí Bảo hành/Sửa chữa...</div> : <div className="p-5"><div className="grid gap-4 xl:grid-cols-[minmax(0,1.16fr)_minmax(290px,0.84fr)]"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-lg border border-[#F0D9B8] bg-[#FFF9EB] px-3 py-3"><div className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#A86B00]">Sửa chữa</div><div className="mt-1 text-xl font-extrabold text-[#8F5A00]">{currency(serviceCostReport.repairCost, currencyMode)}</div><div className="mt-2 text-[10px] font-semibold text-[#8F6B2D]">{serviceCostReport.totalCost ? `${Math.round((serviceCostReport.repairCost / serviceCostReport.totalCost) * 1000) / 10}% tổng cộng` : "0% tổng cộng"}</div></div><div className="rounded-lg border border-[#D9E8F3] bg-[#F8FCFF] px-3 py-3"><div className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#3278BD]">Bảo hành</div><div className="mt-1 text-xl font-extrabold text-[#193B57]">{currency(serviceCostReport.warrantyCost, currencyMode)}</div><div className="mt-2 text-[10px] font-semibold text-[#507DA6]">{serviceCostReport.totalCost ? `${Math.round((serviceCostReport.warrantyCost / serviceCostReport.totalCost) * 1000) / 10}% tổng cộng` : "0% tổng cộng"}</div></div><div className="rounded-lg border border-[#E0E3FF] bg-[#F9F9FF] px-3 py-3"><div className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#6B66B7]">{serviceCostYear === "all" ? "Tổng cộng" : `Tổng cộng (năm ${serviceCostYear})`}</div><div className="mt-1 text-xl font-extrabold text-[#3730A3]">{currency(serviceCostReport.totalCost, currencyMode)}</div><div className="mt-2 text-[10px] font-semibold text-[#6560A8]">{serviceCostReport.totalCost ? "100% tổng cộng" : "0% tổng cộng"}</div></div></div><ServiceCostTrendChart data={monthlyServiceCostTrend} maxCost={monthlyServiceCostMax} yearLabel={serviceCostYear === "all" ? "Tất cả các năm" : `Năm ${serviceCostYear}`} currencyMode={currencyMode} selectedMonth={selectedServiceCostMonth} onSelectMonth={setSelectedServiceCostMonth} /></div>{selectedServiceCostMonth !== null && <div className="mt-4 overflow-hidden rounded-xl border border-[#D9E8F3]"><div className="flex flex-col gap-2 border-b border-[#D9E8F3] bg-[#F8FCFF] px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-xs font-extrabold text-[#193B57]">Phiếu chi phí tháng T{selectedServiceCostMonth + 1}</div><div className="mt-1 text-[10px] font-semibold text-[#71869A]">{serviceCostYear === "all" ? "Tất cả các năm" : `Năm ${serviceCostYear}`} · Theo kênh xử lý đang chọn</div></div><button type="button" onClick={() => setSelectedServiceCostMonth(null)} className="self-start rounded-md border border-[#C9DAE8] bg-white px-2.5 py-1.5 text-[10px] font-bold text-[#527187] hover:bg-[#F0F6FA]">Đóng danh sách</button></div>{selectedMonthlyServiceTickets.length ? <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-xs"><thead className="bg-[#FBFCFE] text-[10px] font-extrabold uppercase tracking-[.08em] text-[#71869A]"><tr><th className="px-4 py-2.5">Mã phiếu</th><th className="px-4 py-2.5">Tài sản</th><th className="px-4 py-2.5">Kênh</th><th className="px-4 py-2.5">Ngày mở</th><th className="px-4 py-2.5 text-right">Chi phí</th><th className="px-4 py-2.5">Trạng thái</th></tr></thead><tbody>{selectedMonthlyServiceTickets.map((item) => <tr key={item.ticketId} className="border-t border-[#E7EEF3]"><td className="px-4 py-3 font-mono text-[11px] font-bold text-[#2666A8]">{item.ticketCode}</td><td className="px-4 py-3"><div className="font-semibold text-[#193B57]">{item.assetName}</div><div className="mt-0.5 font-mono text-[10px] text-[#8AA0B6]">{item.assetCode}</div></td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${item.channel === "Bảo hành" ? "bg-[#EAF3FB] text-[#2666A8]" : "bg-[#FFF4DE] text-[#A86B00]"}`}>{item.channel}</span></td><td className="px-4 py-3 text-[#60758A]">{new Date(item.openedAt).toLocaleDateString("vi-VN")}</td><td className="px-4 py-3 text-right font-extrabold text-[#3730A3]">{currency(item.actualCost, currencyMode)}</td><td className="px-4 py-3 text-[#60758A]">{serviceTicketStatusLabel(item.status)}</td></tr>)}</tbody></table></div> : <div className="px-4 py-7 text-center text-xs text-[#8AA0B6]">Không có phiếu phát sinh chi phí trong tháng này theo bộ lọc hiện tại.</div>}</div>}<div className="mt-4 flex flex-col gap-3 border-t border-[#E7EEF3] pt-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-[#71869A]">Bản xem trước Excel áp dụng riêng bộ lọc kênh và năm hiện tại.</p><button onClick={exportRepairCostExcel} disabled={!isAdmin || !serviceCostReport.rows.length || exporting !== null} title={!serviceCostReport.rows.length ? "Không có phiếu phát sinh chi phí theo bộ lọc hiện tại" : "Mở bản xem trước Excel"} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg border border-[#C9CCF4] bg-[#F7F7FF] px-4 py-2.5 text-xs font-bold text-[#4A45A5] hover:bg-white disabled:cursor-not-allowed disabled:opacity-40"><Download size={15} className={exporting === "repairCosts" ? "animate-pulse" : ""} />{exporting === "repairCosts" ? "Đang chuẩn bị..." : `Xem trước Excel (${serviceCostReport.ticketCount})`}</button></div>{!serviceCostReport.rows.length && <p className="mt-3 text-xs text-[#8AA0B6]">Không có phiếu phát sinh chi phí theo bộ lọc kênh và năm hiện tại; nút xem trước Excel đang được khóa.</p>}</div>}</section>
