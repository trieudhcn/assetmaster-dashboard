...
import { buildDigitalExpiryAlertUrl, createDigitalExpiryAlertId } from "@/lib/digitalExpiryAlerts";
import { getNotificationDestination, getNotificationTargetLabel, type NotificationTarget } from "@/lib/notificationLinks";
...
import { LicensesServicesManagementView } from "./LicensesServicesManagementView";
...
import { FileStorageSettingsPanel } from "@/components/FileStorageSettingsPanel";
import { SelfHostedBackupRecoveryPanel } from "@/components/SelfHostedBackupRecoveryPanel";
import { SelfHostedServiceStatusPanel } from "@/components/SelfHostedServiceStatusPanel";
...
import { AssetQrCapture, findAssetByQrValue, type QrScannableAsset } from "@/components/AssetQrCapture";
...
const navItems = [
  ...
  { label: "Bản quyền & Dịch vụ", icon: ShieldCheck },
...
type AssetCatalogLicenseFilterState = { value: string; options: string[]; counts: Record<string, number>; loading: boolean; onChange: (value: string) => void; onOpenImportHistory: () => void };
let assetCatalogLicenseFilterState: AssetCatalogLicenseFilterState = { value: "Tất cả Bản quyền", options: ["Tất cả Bản quyền"], counts: { "Tất cả Bản quyền": 0 }, loading: false, onChange: () => undefined, onOpenImportHistory: () => undefined };
...
type HeaderNotification = {
  ...
  description: string;
  createdAt: Date;
  kind: "assignment" | "maintenance" | "return" | "supplyRequest";
...
function EmployeeManagementPage() {
  ...
  const activeLicenseAssignments = trpc.employees.activeLicenseAssignments.useQuery({ userId: selectedEmployeeId ?? 0 }, { enabled: Boolean(selectedEmployeeId) && isAdmin });
  ...
  const updateActiveStatus = trpc.employees.updateActiveStatus.useMutation({
    onSuccess: (result) => { utils.employees.list.invalidate(); utils.employees.activeLicenseAssignments.invalidate(); setAccountStatusDialogOpen(false); toast.success(result.revokedLicenseCount ? `Đã khóa tài khoản và thu hồi ${result.revokedLicenseCount} Bản quyền về kho.` : "Đã cập nhật trạng thái tài khoản."); },
...
export default function Home() {
  ...
  const [activeNav, setActiveNav] = useState(() => {
    ...
    const deepLinks: Record<string, string> = { assets: "Danh mục tài sản", supplies: "Phụ kiện", categories: "Phân loại tài sản", maintenance: "Bảo hành & Sửa chữa", audit: "Kiểm kê", retirement: "Khấu hao & Thanh lý", reports: "Báo Cáo", employees: "Quản lý nhân viên", organization: "Phòng Ban & Bộ Phận", vendors: "Nhà cung cấp & Hãng", contracts: "Hợp đồng & Hóa đơn", invoices: "Hợp đồng & Hóa đơn", licenses: "Bản quyền & Dịch vụ", handovers: "Bàn giao & Cấp phát", settings: "Cài đặt", help: "Trợ giúp & hướng dẫn" };
  ...
  const [vendorFilter, setVendorFilter] = usePersistedState("assetmaster.filters.assets.vendor", "Tất cả nhà cung cấp");
  const [brandFilter, setBrandFilter] = usePersistedState("assetmaster.filters.assets.brand", "Tất cả hãng");
  const [warrantyFilter, setWarrantyFilter] = usePersistedState("assetmaster.filters.assets.warranty", "Tất cả bảo hành");
  const [branchFilter, setBranchFilter] = usePersistedState("assetmaster.filters.assets.branch", "Tất cả chi nhánh");
  const [invoiceFilter, setInvoiceFilter] = usePersistedState("assetmaster.filters.assets.invoice", "Tất cả Hóa đơn");
  const [licenseFilter, setLicenseFilter] = usePersistedState("assetmaster.filters.assets.license", "Tất cả Bản quyền");
  const [isAssetFilterPending, startAssetFilterTransition] = useTransition();
  const [isAssetFilterApplying, setIsAssetFilterApplying] = useState(false);
  const assetFilterLoadingTimerRef = useRef<number | null>(null);
  const applyAssetFilter = (update: (value: string) => void, value: string) => {
    setIsAssetFilterApplying(true);
    if (assetFilterLoadingTimerRef.current) window.clearTimeout(assetFilterLoadingTimerRef.current);
    startAssetFilterTransition(() => update(value));
    assetFilterLoadingTimerRef.current = window.setTimeout(() => {
      setIsAssetFilterApplying(false);
      assetFilterLoadingTimerRef.current = null;
    }, 180);
  };
  const resetAssetFilters = () => {
    setIsAssetFilterApplying(true);
    if (assetFilterLoadingTimerRef.current) window.clearTimeout(assetFilterLoadingTimerRef.current);
    startAssetFilterTransition(() => {
      setQuery(""); setStatus("Tất cả trạng thái"); setDepartment("Tất cả phòng ban"); setVendorFilter("Tất cả nhà cung cấp"); setBrandFilter("Tất cả hãng"); setWarrantyFilter("Tất cả bảo hành"); setBranchFilter("Tất cả chi nhánh"); setInvoiceFilter("Tất cả Hóa đơn"); setLicenseFilter("Tất cả Bản quyền");
    });
    assetFilterLoadingTimerRef.current = window.setTimeout(() => {
      setIsAssetFilterApplying(false);
      assetFilterLoadingTimerRef.current = null;
  ...
  const isAssetFilterLoading = isAssetFilterPending || isAssetFilterApplying;
  useEffect(() => () => { if (assetFilterLoadingTimerRef.current) window.clearTimeout(assetFilterLoadingTimerRef.current); }, []);
  ...
  const [legacyDismissedDashboardAlertIds, setLegacyDismissedDashboardAlertIds] = useState<string[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("assetmaster-dismissed-dashboard-alert-ids") || "[]") as string[];
    } catch {
      return [];
    }
  });
  ...
  const assetEmployeesQuery = trpc.employees.list.useQuery(undefined, { enabled: isAuthenticated && isAdmin });
  const assetDepartmentsQuery = trpc.departments.listAll.useQuery(undefined, { enabled: isAuthenticated && isAdmin });
  const companyQuery = trpc.company.get.useQuery(undefined, { enabled: isAuthenticated && isAdmin });
  const assetFilterDataLoading = assetQuery.isLoading || assetDepartmentsQuery.isLoading || assetEmployeesQuery.isLoading || branchesQuery.isLoading || vendorsQuery.isLoading || brandsQuery.isLoading || dashboardSoftwareLicensesQuery.isLoading || assetLicenseAssignmentsQuery.isLoading;
  ...
  const overdueAuditReminders = useMemo(() => (operationalRemindersQuery.data || []).filter((reminder): reminder is Extract<OperationalReminder, { kind: "audit" }> => reminder.kind === "audit" && reminder.isOverdue).sort((left, right) => left.dueAt.getTime() - right.dueAt.getTime()), [operationalRemindersQuery.data]);
  useEffect(() => {
  ...
  const dismissDashboardAlertsMutation = trpc.notifications.dismissDashboardAlerts.useMutation({
    onMutate: async ({ alertIds }) => {
      await trpcUtils.notifications.dashboardAlertStates.cancel();
      const previous = trpcUtils.notifications.dashboardAlertStates.getData();
      trpcUtils.notifications.dashboardAlertStates.setData(undefined, { alertIds: Array.from(new Set([...(previous?.alertIds || []), ...alertIds])) });
      return { previous };
    },
  ...
  const restoreDashboardAlertsMutation = trpc.notifications.restoreDashboardAlerts.useMutation({
    onMutate: async ({ alertIds }) => {
      await trpcUtils.notifications.dashboardAlertStates.cancel();
      const previous = trpcUtils.notifications.dashboardAlertStates.getData();
      trpcUtils.notifications.dashboardAlertStates.setData(undefined, { alertIds: (previous?.alertIds || []).filter((alertId) => !alertIds.includes(alertId)) });
      return { previous };
    },
    onError: (error, _input, context) => {
      if (context?.previous) trpcUtils.notifications.dashboardAlertStates.setData(undefined, context.previous);
  ...
  const assetLicenseLabelsByAssetCode = useMemo(() => {
    const licensesById = new Map((dashboardSoftwareLicensesQuery.data || []).map((license) => [license.id, license]));
    const assetCodeById = new Map((assetQuery.data || []).map((asset) => [asset.id, asset.assetCode]));
    const labelsByCode = new Map<string, string[]>();
    (assetLicenseAssignmentsQuery.data || []).filter((assignment) => assignment.status === "active" && Boolean(assignment.assetId)).forEach((assignment) => {
      const assetCode = assetCodeById.get(assignment.assetId!);
      const licenseName = licensesById.get(assignment.softwareLicenseId)?.productName?.trim();
      if (!assetCode || !licenseName) return;
      labelsByCode.set(assetCode, [...(labelsByCode.get(assetCode) || []), licenseName]);
    });
  ...
  const filteredAssets = useMemo(() => assetRowsWithLicenses.filter((asset) => {
    ...
    const matchesDepartment = department === "Tất cả phòng ban" || asset.departmentName === department;
    ...
    const matchesLicense = licenseFilter === "Tất cả Bản quyền" || (asset.licenseLabels || []).some((licenseName) => matchesVietnameseSearch(licenseName, licenseFilter));
    return matchesQuery && matchesStatus && matchesDepartment && matchesVendor && matchesBrand && matchesWarranty && matchesBranch && matchesInvoice && matchesLicense;
  }), [assetRowsWithLicenses, query, status, department, vendorFilter, brandFilter, warrantyFilter, branchFilter, invoiceFilter, licenseFilter, branchesQuery.data]);
  ...
  const departmentFilterOptions = useMemo(() => ["Tất cả phòng ban", ...Array.from(new Set((assetDepartmentsQuery.data || []).map((item) => item.name.trim()).filter(Boolean))).sort((left, right) => left.localeCompare(right, "vi"))], [assetDepartmentsQuery.data]);
  const departmentOptionLabels = useMemo(() => Object.fromEntries((assetDepartmentsQuery.data || []).map((item) => [item.name, item.isActive ? item.name : `${item.name} · Ngừng hoạt động`])), [assetDepartmentsQuery.data]);
  const assetStatusFilterCounts = useMemo(() => getAssetStatusFilterCounts(assetRows), [assetRows]);
  const departmentFilterCounts = useMemo(() => {
    const counts: Record<string, number> = { "Tất cả phòng ban": assetRows.length };
    departmentFilterOptions.slice(1).forEach((departmentName) => { counts[departmentName] = assetRows.filter((asset) => asset.departmentName === departmentName).length; });
    return counts;
  }, [assetRows, departmentFilterOptions]);
  const vendorFilterCounts = useMemo(() => {
    const counts: Record<string, number> = { "Tất cả nhà cung cấp": assetRows.length };
    (vendorsQuery.data || []).forEach((vendor) => { counts[vendor.name] = assetRows.filter((asset) => asset.supplier === vendor.name).length; });
    return counts;
  }, [assetRows, vendorsQuery.data]);
  const brandFilterCounts = useMemo(() => {
    const counts: Record<string, number> = { "Tất cả hãng": assetRows.length };
    (brandsQuery.data || []).forEach((brand) => { counts[brand.name] = assetRows.filter((asset) => asset.brand === brand.name).length; });
    return counts;
  }, [assetRows, brandsQuery.data]);
  ...
  const headerNotifications = useMemo<HeaderNotification[]>(() => {
    const maintenanceNotifications = notificationPreferences.maintenanceEnabled ? assetRows.filter((asset) => asset.statusType === "maintenance").slice(0, 2).map((asset) => ({
      ...
      target: { type: "maintenance" as const, assetCode: asset.code },
    })) : [];
    ...
    const supplyRequestNotifications = notificationPreferences.handoverEnabled ? (supplyRequestNotificationsQuery.data || []).filter((request) => request.status === "pending").map((request) => {
      ...
      return {
        ...
        title: `${request.requestCode} đang chờ duyệt`,
        description: `${request.requesterName} yêu cầu ${request.items.length} loại phụ kiện: ${itemNames}${remainingItems ? ` và ${remainingItems} loại khác` : ""}.`,
        createdAt: request.createdAt,
        kind: "supplyRequest" as const,
        target: { type: "supplyRequest" as const, requestId: request.id },
      };
    }) : [];
  ...
  const openNotificationTarget = (notification: HeaderNotification) => {
    ...
    const destination = getNotificationDestination(target);
    if (destination.storageEntry)
      sessionStorage.setItem(
        destination.storageEntry.key,
        destination.storageEntry.value
      );
    navigateTo(destination.navigationLabel);
  };
  ...
  const lowStockSupplies = useMemo(() => (suppliesQuery.data || []).filter((supply) => Number(supply.stockQuantity) <= Number(supply.minimumQuantity)).sort((left, right) => Math.abs(Number(left.stockQuantity)) - Math.abs(Number(right.stockQuantity)) || Number(left.stockQuantity) - Number(right.stockQuantity)), [suppliesQuery.data]);
  const digitalExpiryAlerts = useMemo(() => {
    const now = Date.now();
    const withinThirtyDays = (value: Date | string | null) => { if (!value) return false; const days = Math.ceil((new Date(value).getTime() - now) / 86_400_000); return days >= 0 && days <= 30; };
    const licenseAlerts = (dashboardSoftwareLicensesQuery.data || []).filter((license) => license.status !== "retired" && withinThirtyDays(license.expiresAt)).map((license) => ({ kind: "license" as const, id: license.id, alertId: createDigitalExpiryAlertId("license", license.id), title: license.productName, detail: license.licenseCode, dueAt: new Date(license.expiresAt!), categoryLabel: "Bản quyền" }));
    const serviceAlerts = (dashboardTechnologyServicesQuery.data || []).filter((service) => service.status !== "cancelled" && withinThirtyDays(service.expiresAt)).map((service) => ({ kind: "service" as const, id: service.id, alertId: createDigitalExpiryAlertId("service", service.id), title: service.name, detail: service.domainName || service.serviceCode, dueAt: new Date(service.expiresAt!), categoryLabel: service.serviceType === "internet" ? "Internet" : service.serviceType === "domain" ? "Tên miền" : "SSL" }));
    return [...licenseAlerts, ...serviceAlerts].sort((left, right) => left.dueAt.getTime() - right.dueAt.getTime());
  }, [dashboardSoftwareLicensesQuery.data, dashboardTechnologyServicesQuery.data]);
  const dismissedDashboardAlertIds = useMemo(() => Array.from(new Set([...(dashboardAlertStatesQuery.data?.alertIds || []), ...legacyDismissedDashboardAlertIds])), [dashboardAlertStatesQuery.data?.alertIds, legacyDismissedDashboardAlertIds]);
  const visibleOverdueAuditReminders = useMemo(() => overdueAuditReminders.filter((reminder) => !dismissedDashboardAlertIds.includes(reminder.id)), [dismissedDashboardAlertIds, overdueAuditReminders]);
  ...
  const visibleDigitalExpiryAlerts = useMemo(() => digitalExpiryAlerts.filter((alert) => !dismissedDashboardAlertIds.includes(alert.alertId)), [digitalExpiryAlerts, dismissedDashboardAlertIds]);
  const lowStockSupplyPreview = visibleLowStockSupplies.slice(0, 3);
  const activeDashboardAlertIds = useMemo(() => [...overdueAuditReminders.map((reminder) => reminder.id), ...lowStockSupplies.map((supply) => "supply-" + supply.id), ...digitalExpiryAlerts.map((alert) => alert.alertId)], [digitalExpiryAlerts, lowStockSupplies, overdueAuditReminders]);
  const dashboardAlertHistory = useMemo(() => (dashboardAlertHistoryQuery.data || []).map((entry) => {
    ...
    if (kind === "license") {
      const license = (dashboardSoftwareLicensesQuery.data || []).find((item) => item.id === numericId);
      return { ...entry, kind, numericId, title: license?.productName || `Bản quyền #${rawId}`, detail: license?.licenseCode || entry.alertId };
    }
    if (kind === "service") {
      const service = (dashboardTechnologyServicesQuery.data || []).find((item) => item.id === numericId);
      return { ...entry, kind, numericId, title: service?.name || `Dịch vụ #${rawId}`, detail: service?.domainName || service?.serviceCode || entry.alertId };
    }
  ...
  }), [dashboardAlertHistoryQuery.data, dashboardSoftwareLicensesQuery.data, dashboardTechnologyServicesQuery.data, operationalRemindersQuery.data, suppliesQuery.data]);
  ...
  const restoreDashboardAlerts = () => {
    const alertIds = activeDashboardAlertIds.filter((alertId) => dismissedDashboardAlertIds.includes(alertId));
    if (alertIds.length) restoreDashboardAlertsMutation.mutate({ alertIds });
  };
  useEffect(() => {
    if (!dashboardAlertStatesQuery.data || !legacyDismissedDashboardAlertIds.length) return;
    const legacyAlertIds = legacyDismissedDashboardAlertIds;
    setLegacyDismissedDashboardAlertIds([]);
    dismissDashboardAlertsMutation.mutate({ alertIds: legacyAlertIds }, {
      onSuccess: () => localStorage.removeItem("assetmaster-dismissed-dashboard-alert-ids"),
      onError: () => setLegacyDismissedDashboardAlertIds(legacyAlertIds),
    });
  }, [dashboardAlertStatesQuery.data, dismissDashboardAlertsMutation, legacyDismissedDashboardAlertIds]);
  const overdueDays = (dueAt: Date) => Math.max(1, Math.ceil((Date.now() - dueAt.getTime()) / 86_400_000));
  ...
  const openDigitalExpiryAlert = (kind: "license" | "service", id: number, searchValue: string) => {
    const url = buildDigitalExpiryAlertUrl(window.location.href, kind, searchValue);
    window.history.pushState({}, "", url);
    setActiveNav("Bản quyền & Dịch vụ");
    setMobileNavOpen(false);
    window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: "smooth" }));
  };
  const openDashboardAlertHistoryItem = (item: typeof dashboardAlertHistory[number]) => {
    if (item.kind === "audit" && Number.isInteger(item.numericId) && item.numericId > 0) {
      ...
      navigateTo("Phụ kiện");
    } else if ((item.kind === "license" || item.kind === "service") && Number.isInteger(item.numericId) && item.numericId > 0) {
      openDigitalExpiryAlert(item.kind, item.numericId, item.detail);
    }
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] text-[#102A43] antialiased">
      ...
      <main className="min-h-screen lg:pl-[264px]" data-system-settings-layout={activeNav === "Cài đặt" ? "true" : undefined}>
        ...
        {activeNav === "Bản quyền & Dịch vụ" ? <LicensesServicesManagementView /> : null}
        {activeNav === "Trợ giúp & hướng dẫn" ? <HelpCenter /> : null}
        {activeNav === "Danh mục tài sản" ? assetQuery.isLoading && !assetQuery.data ? <AssetCatalogLoadingPanel /> : assetQuery.isError && !assetQuery.data ? <AssetCatalogUnavailablePanel onRetry={() => void assetQuery.refetch()} /> : <PaginatedAssetCatalogPage assets={filteredAssets} allAssets={assetRows} statusCounts={assetStatusFilterCounts} query={query} status={status} department={department} vendor={vendorFilter} brand={brandFilter} warranty={warrantyFilter} branch={branchFilter} invoice={invoiceFilter} invoiceOptions={invoiceFilterOptions} departmentOptions={departmentFilterOptions} vendorOptions={["Tất cả nhà cung cấp", ...(vendorsQuery.data || []).map((item) => item.name)]} brandOptions={["Tất cả hãng", ...(brandsQuery.data || []).map((item) => item.name)]} branchOptions={["Tất cả chi nhánh", ...(branchesQuery.data || []).map((item) => item.name)]} branchCounts={branchFilterCounts} departmentCounts={departmentFilterCounts} vendorCounts={vendorFilterCounts} brandCounts={brandFilterCounts} departmentOptionLabels={departmentOptionLabels} isFilterDataLoading={assetFilterDataLoading} isFiltering={isAssetFilterLoading} onQueryChange={(value) => applyAssetFilter(setQuery, value)} onStatusChange={(value) => applyAssetFilter(setStatus, value)} onDepartmentChange={(value) => applyAssetFilter(setDepartment, value)} onVendorChange={(value) => applyAssetFilter(setVendorFilter, value)} onBrandChange={(value) => applyAssetFilter(setBrandFilter, value)} onWarrantyChange={(value) => applyAssetFilter(setWarrantyFilter, value)} onBranchChange={(value) => applyAssetFilter(setBranchFilter, value)} onInvoiceChange={(value) => applyAssetFilter(setInvoiceFilter, value)} onReset={resetAssetFilters} onCreate={openCreateModal} onEdit={openEditModal} onOpenDetail={openDetailModal} onOpenQr={setQrAsset} onOpenMaintenance={(asset) => { sessionStorage.setItem("assetmaster-open-maintenance-asset-code", asset.code); navigateTo("Bảo hành & Sửa chữa"); }} onOpenInvoice={(asset) => { if (!asset.purchaseInvoiceId) return; const url = new URL(window.location.href); url.searchParams.set("view", "invoices"); url.searchParams.set("invoiceId", String(asset.purchaseInvoiceId)); window.history.pushState({}, "", `${url.pathname}${url.search}${url.hash}`); setActiveNav("Hợp đồng & Hóa đơn"); setMobileNavOpen(false); window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: "smooth" })); }} onAssign={(asset) => { if (asset.statusType !== "available") { toast.error("Chỉ có thể bàn giao tài sản đang sẵn có."); return; } setHandoverAssetCode(asset.code); }} canEditSectionLabels={isAdmin} /> : null}
        <div className={`px-4 py-7 sm:px-6 lg:px-9 lg:py-8 ${activeNav === "Tổng quan" ? "" : "hidden"}`}>
          <div className="mx-auto max-w-[1500px]">
            ...
            <div data-dashboard-alert-history-launcher className="mb-4 flex flex-col gap-3 rounded-xl border border-[#D9E4E8] bg-white px-4 py-3 shadow-[0_6px_18px_rgba(16,42,67,0.04)] sm:flex-row sm:items-center sm:justify-between"><div><div className="text-xs font-extrabold text-[#193B57]">Cảnh báo đã xem</div><p className="mt-0.5 text-[11px] text-[#71869A]">Mở lịch sử để xem lại, mở đúng nghiệp vụ hoặc hiện lại cảnh báo.</p></div><button type="button" onClick={() => { setDashboardAlertHistoryOpen(true); window.requestAnimationFrame(() => document.querySelector<HTMLElement>("[data-dashboard-alert-history]")?.scrollIntoView({ behavior: "smooth", block: "center" })); }} className="w-fit rounded-lg border border-[#CDE5E5] bg-[#F8FCFB] px-3 py-2 text-xs font-extrabold text-[#087A6A] transition hover:bg-[#E6F6F2]">Xem lịch sử cảnh báo</button></div>
            ...
            <section data-branch-asset-value-chart className="mt-5 overflow-hidden rounded-xl border border-[#CDE5E5] bg-white shadow-[0_8px_24px_rgba(16,42,67,0.045)]">
<div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#DCE9ED] bg-[#F4FBFA] px-4 py-3"><div><div className="text-sm font-extrabold text-[#087A6A]">Giá trị tài sản theo Chi nhánh</div><p className="mt-0.5 text-[11px] text-[#4B8884]">Tổng nguyên giá tài sản đang còn thuộc công ty, phân theo Chi nhánh.</p></div><span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-extrabold text-[#087A6A] ring-1 ring-[#BFE7E1]">{formatVnd(assetValueTotal)} VNĐ</span></div>
{branchAssetValueSummary.length ? <div className="mt-5 grid gap-3 lg:grid-cols-2">{branchAssetValueSummary.map((item) => { const maxValue = Math.max(...branchAssetValueSummary.map((entry) => entry.value), 1); const share = item.value ? Math.max(8, (item.value / maxValue) * 100) : 0; const percentage = assetValueTotal ? (item.value / assetValueTotal) * 100 : 0; const canFilter = item.code !== "CHƯA GÁN"; return <button key={item.code} type="button" disabled={!canFilter} onClick={() => { if (!canFilter) return; setBranchFilter(item.name); navigateTo("Danh mục tài sản"); }} className="group relative rounded-xl border border-[#E2ECEF] bg-[#FBFCFD] p-3.5 text-left transition hover:-translate-y-0.5 hover:border-[#8BCDC6] hover:bg-[#F4FBFA] hover:shadow-[0_8px_18px_rgba(15,140,140,0.10)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0F8C8C] disabled:cursor-default disabled:hover:translate-y-0 disabled:hover:border-[#E2ECEF] disabled:hover:bg-[#FBFCFD] disabled:hover:shadow-none"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-xs font-extrabold text-[#193B57]">{item.name}</div><div className="mt-0.5 font-mono text-[10px] font-semibold text-[#71869A]">{item.code} · {item.count} tài sản</div></div><div className="shrink-0 text-right text-xs font-extrabold tabular-nums text-[#087A6A]">{formatVnd(item.value)} <span className="text-[10px] text-[#71869A]">VNĐ</span></div></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#EAF2F3]"><div className="h-full rounded-full bg-[#0F8C8C]" style={{ width: String(share) + "%" }} /></div><span role="tooltip" className="pointer-events-none absolute -top-2 left-1/2 z-30 w-max max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-full rounded-lg bg-[#102A43] px-3 py-2 text-center text-[11px] font-semibold leading-5 text-white opacity-0 shadow-lg transition-opacity duration-150 group-hover:opacity-100 group-focus-visible:opacity-100"><span className="block">Tổng giá trị: {formatVnd(item.value)} VNĐ</span><span className="block text-[#B8E9DD]">Tỷ trọng: {percentage.toFixed(1)}% · {item.count} tài sản</span>{canFilter && <span className="mt-1 block text-[10px] text-[#D3F4EE]">Nhấp để xem danh mục đã lọc</span>}</span></button>; })}</div> : <div className="mt-4 rounded-lg border border-dashed border-[#CDE5E5] bg-[#F8FCFB] px-4 py-7 text-center text-xs text-[#71869A]">Chưa có tài sản đủ điều kiện để thống kê theo Chi nhánh.</div>}
            ...
            {visibleDigitalExpiryAlerts.length > 0 && <section data-digital-expiry-alert className="mt-5 overflow-hidden rounded-xl border border-[#E8C775] bg-white shadow-[0_8px_24px_rgba(16,42,67,0.045)]"><div className="flex min-h-[72px] flex-col gap-3 border-b border-[#F4E0AB] bg-[#FFF9EB] px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-white text-[#A86B00] shadow-sm"><ShieldCheck size={16} /></span><div><h2 className="text-sm font-extrabold text-[#8F5A00]">Bản quyền & Dịch vụ sắp hết hạn</h2><p className="mt-0.5 text-[11px] text-[#8F6A31]">Theo dõi license, Internet, Tên miền và SSL trong 30 ngày tới.</p></div></div><button type="button" onClick={() => openDigitalExpiryAlert("license", 0, "")} className="w-fit rounded-lg border border-[#F2D596] bg-white px-3 py-2 text-xs font-extrabold text-[#A86B00] transition hover:bg-[#FFF5DC]">Mở quản lý</button></div><div className="divide-y divide-[#F7E4B7]">{visibleDigitalExpiryAlerts.slice(0, 3).map((alert) => { const remainingDays = Math.max(0, Math.ceil((alert.dueAt.getTime() - Date.now()) / 86_400_000)); return <div key={alert.alertId} className="flex items-center justify-between gap-3 px-4 py-3"><button type="button" onClick={() => openDigitalExpiryAlert(alert.kind, alert.id, alert.detail)} className="min-w-0 flex-1 text-left transition hover:text-[#A86B00]"><div className="flex flex-wrap items-center gap-2"><span className="truncate text-xs font-extrabold text-[#193B57]">{alert.title}</span><span className="rounded-full bg-[#FFF0C9] px-1.5 py-0.5 text-[9px] font-extrabold text-[#A86B00]">{alert.categoryLabel}</span></div><div className="mt-0.5 flex flex-wrap items-center gap-1.5"><span className="font-mono text-[11px] text-[#8F6A31]">{alert.detail}</span><span className="rounded-full bg-[#FDEDEE] px-1.5 py-0.5 text-[9px] font-extrabold text-[#B44545]">{remainingDays === 0 ? "Hết hạn hôm nay" : `Còn ${remainingDays} ngày`}</span></div></button><div className="flex shrink-0 items-center gap-1.5"><button type="button" onClick={() => dismissDashboardAlert(alert.alertId)} className="rounded-md border border-[#F2D596] bg-white px-2 py-1.5 text-[10px] font-extrabold text-[#A86B00] transition hover:bg-[#FFF5DC]" aria-label="Đánh dấu đã xem cảnh báo Bản quyền hoặc Dịch vụ">Đã xem</button><button type="button" onClick={() => openDigitalExpiryAlert(alert.kind, alert.id, alert.detail)} className="text-[11px] font-extrabold text-[#A86B00]">Mở →</button></div></div>; })}</div></section>}
...
function AssetCatalogPage({ assets, totalAssets, statusCounts, branchCounts, departmentCounts, vendorCounts, brandCounts, departmentOptionLabels, isFilterDataLoading, isFiltering, query, status, department, vendor, brand, warranty, branch, invoice, invoiceOptions, departmentOptions, vendorOptions, brandOptions, branchOptions, onQueryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onWarrantyChange, onBranchChange, onInvoiceChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onOpenMaintenance, onOpenInvoice, onAssign, onExportFilteredAssets, onExportAllAssets, filteredAssetCount, allAssetCount, canExportFilteredAssets, canExportAllAssets, isExportingFilteredAssets, onOpenImport, onOpenImportHistory, canEditSectionLabels = false }: { assets: Asset[]; totalAssets: number; statusCounts: Record<string, number>; branchCounts: Record<string, number>; departmentCounts: Record<string, number>; vendorCounts: Record<string, number>; brandCounts: Record<string, number>; departmentOptionLabels: Record<string, string>; isFilterDataLoading: boolean; isFiltering: boolean; query: string; status: string; department: string; vendor: string; brand: string; warranty: string; branch: string; invoice: string; invoiceOptions: string[]; departmentOptions: string[]; vendorOptions: string[]; brandOptions: string[]; branchOptions: string[]; onQueryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onWarrantyChange: (value: string) => void; onBranchChange: (value: string) => void; onInvoiceChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onOpenMaintenance: (asset: Asset) => void; onOpenInvoice: (asset: Asset) => void; onAssign: (asset: Asset) => void; onExportFilteredAssets: () => void; onExportAllAssets: () => void; filteredAssetCount: number; allAssetCount: number; canExportFilteredAssets: boolean; canExportAllAssets: boolean; isExportingFilteredAssets: boolean; onOpenImport: () => void; onOpenImportHistory: () => void; canEditSectionLabels?: boolean }) {
  type AssetCatalogColumn = "code" | "name" | "holder" | "branch" | "status" | "license" | "location" | "invoice" | "value";
  type OptionalAssetColumn = Exclude<AssetCatalogColumn, "code" | "name">;
  const defaultVisibleColumns: Record<OptionalAssetColumn, boolean> = { holder: true, branch: true, status: true, license: false, location: true, invoice: false, value: true };
  const defaultColumnOrder: AssetCatalogColumn[] = ["code", "name", "holder", "branch", "status", "license", "location", "invoice", "value"];
  const columnLabels: Record<AssetCatalogColumn, string> = { code: "Mã TS", name: "Tên tài sản", holder: "Người giữ", branch: "Chi nhánh", status: "Trạng thái", license: "License", location: "Vị trí / SN", invoice: "Hóa đơn", value: "Giá trị" };
  ...
  useEffect(() => {
    if (!columnPickerOpen && !exportMenuOpen) return;
  ...
  }, [columnPickerOpen, exportMenuOpen]);
  useEffect(() => {
    try {
  ...
  const renderAssetCell = (asset: Asset, key: AssetCatalogColumn) => {
    ...
    if (key === "license") { const labels = asset.licenseLabels || []; return <td key={key} className="px-4 py-4"><span className="block max-w-[190px] truncate text-xs font-semibold text-[#087A6A]" title={labels.join(" · ") || "Chưa cấp License"}>{labels.length ? labels.join(" · ") : "—"}</span></td>; }
  ...
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    <section className="mt-5 overflow-hidden rounded-xl border border-[#DFE9F0] bg-white shadow-[0_8px_24px_rgba(16,42,67,0.045)]"><div className="flex flex-col gap-4 border-b border-[#E7EEF3] px-5 py-5 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="font-display text-[17px] font-extrabold tracking-[-0.025em] text-[#102A43]">Danh sách quản trị</h2><p className="mt-1 text-xs text-[#8AA0B6]">Tìm đúng tài sản trước khi xem hồ sơ, điều chỉnh thông tin, tạo QR hoặc cấp phát.</p></div><div className="flex flex-wrap items-center gap-2 lg:flex-nowrap"><div ref={columnPickerRef} className="relative shrink-0"><button type="button" onClick={() => setColumnPickerOpen((open) => !open)} aria-expanded={columnPickerOpen} aria-controls="asset-catalog-column-picker" className="inline-flex h-9 items-center justify-center gap-2 rounded-lg border border-[#CDE5E5] bg-white px-3 text-xs font-bold text-[#087A6A] transition hover:bg-[#ECF8F7]"><Columns3 size={14} />Cột</button>{columnPickerOpen && <div id="asset-catalog-column-picker" data-asset-column-picker className="absolute left-0 top-full z-40 mt-2 w-52 rounded-xl border border-[#D7E5EC] bg-white p-2 shadow-[0_16px_36px_rgba(16,42,67,0.16)]"><div className="px-2 pb-1.5 text-[10px] font-extrabold uppercase tracking-[.1em] text-[#71869A]">Cột hiển thị</div>{columnOptions.map((column) => <label key={column.key} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-xs font-semibold text-[#526779] hover:bg-[#F4FBFA]"><input type="checkbox" checked={visibleColumns[column.key]} onChange={() => setVisibleColumns((current) => ({ ...current, [column.key]: !current[column.key] }))} className="h-3.5 w-3.5 rounded border-[#9FC8C4] text-[#0F8C8C] focus:ring-[#0F8C8C]" />{column.label}</label>)}<button type="button" onClick={resetColumnLayout} className="mt-1 w-full rounded-lg border border-[#DDE7F0] px-2 py-1.5 text-[10px] font-extrabold text-[#60758A] transition hover:bg-[#F7FAFC]">Khôi phục mặc định</button></div>}</div><div ref={exportMenuRef} className="relative shrink-0"><button type="button" onClick={() => setExportMenuOpen((open) => !open)} disabled={isExportingFilteredAssets} aria-expanded={exportMenuOpen} aria-controls="asset-catalog-export-menu" className="inline-flex h-9 items-center justify-center gap-2 rounded-lg border border-[#C7DDF8] bg-white px-3 text-xs font-bold text-[#2666A8] transition hover:bg-[#EAF3FF] disabled:cursor-not-allowed disabled:opacity-50"><Download size={14} />{isExportingFilteredAssets ? "Đang xuất..." : "Xuất Excel"}<ChevronDown size={14} className={`transition-transform ${exportMenuOpen ? "rotate-180" : ""}`} /></button>{exportMenuOpen && <div id="asset-catalog-export-menu" data-asset-export-menu className="absolute right-0 top-full z-40 mt-2 w-60 overflow-hidden rounded-xl border border-[#C7DDF8] bg-white p-1.5 shadow-[0_16px_36px_rgba(16,42,67,0.16)]"><button type="button" onClick={() => { setExportMenuOpen(false); onExportFilteredAssets(); }} disabled={!canExportFilteredAssets || isExportingFilteredAssets} className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left text-xs font-bold text-[#2666A8] transition hover:bg-[#EAF3FF] disabled:cursor-not-allowed disabled:opacity-50"><span>Xuất theo bộ lọc</span><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-extrabold text-[#2666A8] ring-1 ring-[#C7DDF8]">{filteredAssetCount}</span></button><button type="button" onClick={() => { setExportMenuOpen(false); onExportAllAssets(); }} disabled={!canExportAllAssets || isExportingFilteredAssets} className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left text-xs font-bold text-[#60758A] transition hover:bg-[#F7FAFC] disabled:cursor-not-allowed disabled:opacity-50"><span>Xuất tất cả</span><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-extrabold text-[#60758A] ring-1 ring-[#DDE7F0]">{allAssetCount}</span></button></div>}</div><button type="button" onClick={onOpenImport} title="Tải template và import nhiều tài sản từ Excel" className="inline-flex h-9 shrink-0 items-center justify-center rounded-lg bg-[#0F8C8C] px-3 text-xs font-bold text-white hover:bg-[#087A6A]">Nhập Excel</button><button onClick={onReset} className="inline-flex h-9 items-center justify-center gap-2 rounded-lg border border-[#DDE7F0] px-3 text-xs font-bold text-[#60758A] hover:bg-[#F7FAFC]"><SlidersHorizontal size={14} />Đặt lại bộ lọc</button><button type="button" onClick={onOpenImportHistory} title="Lịch sử import thành công" aria-label="Mở lịch sử import" className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-[#CDE5E5] bg-white text-[#087A6A] transition hover:bg-[#ECF8F7]"><History size={15} /></button></div></div>
      <div className="flex flex-wrap items-center gap-2 border-b border-[#E7EEF3] bg-[#FBFCFD] px-5 py-4"><div className="relative min-w-0 flex-[1_1_260px]"><Search className="absolute left-3 top-2.5 text-[#9BAEC0]" size={16} /><input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="Tìm mã, tên hoặc người giữ..." className="h-9 w-full rounded-lg border border-[#DDE7F0] bg-white pl-9 pr-3 text-xs outline-none focus:border-[#0F8C8C]" /></div><FilterSelect value={status} onChange={onStatusChange} counts={statusCounts} options={["Tất cả trạng thái", "Sẵn có", "Đang cấp phát", "Bảo trì", "Trả nhà cung cấp"]} isLoading={isFilterDataLoading} /><FilterSelect value={department} onChange={onDepartmentChange} options={departmentOptions} counts={departmentCounts} optionLabels={departmentOptionLabels} isLoading={isFilterDataLoading} /><FilterSelect value={branch} onChange={onBranchChange} options={branchOptions} counts={branchCounts} isLoading={isFilterDataLoading} /><FilterSelect value={vendor} onChange={onVendorChange} options={vendorOptions} counts={vendorCounts} isLoading={isFilterDataLoading} /><FilterSelect value={brand} onChange={onBrandChange} options={brandOptions} counts={brandCounts} isLoading={isFilterDataLoading} /><FilterSelect value={warranty} onChange={onWarrantyChange} options={["Tất cả bảo hành", "Đang bảo hành", "Sắp hết hạn", "Đã hết hạn"]} isLoading={isFilterDataLoading} /><FilterSelect value={invoice} onChange={onInvoiceChange} options={invoiceOptions} isLoading={isFilterDataLoading} /><FilterSelect value={assetCatalogLicenseFilterState.value} onChange={assetCatalogLicenseFilterState.onChange} options={assetCatalogLicenseFilterState.options} counts={assetCatalogLicenseFilterState.counts} isLoading={assetCatalogLicenseFilterState.loading} /></div>
      <div className="relative mobile-table-scroll overflow-x-auto" aria-busy={isFiltering}>{isFiltering && <div data-asset-filter-loading role="status" aria-live="polite" className="absolute inset-x-0 top-0 z-20 flex items-center gap-2 border-b border-[#CDE5E5] bg-white/95 px-5 py-2 text-[11px] font-bold text-[#087A6A] shadow-sm"><Loader2 size={14} className="animate-spin" />Đang áp dụng bộ lọc...</div>}<table className="w-full border-collapse text-left" style={{ tableLayout: "auto" }}><thead><tr className="border-b border-[#E7EEF3] bg-[#FCFDFE] text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#8AA0B6]">{displayedColumns.map(renderColumnHeader)}<th className="px-5 py-3.5 text-right">Thao tác</th></tr></thead><tbody>{isFiltering ? Array.from({ length: 6 }, (_, index) => <tr key={`asset-filter-skeleton-${index}`} className="border-b border-[#EDF2F5]"><td colSpan={displayedColumns.length + 1} className="px-5 py-4"><div className="h-5 animate-pulse rounded bg-[#EAF0F4]" /></td></tr>) : assets.map((asset) => <tr key={asset.code} className="group border-b border-[#EDF2F5] transition hover:bg-[#F8FBFC]">{displayedColumns.map((key) => renderAssetCell(asset, key))}<td className="px-5 py-4"><div className="flex justify-end gap-1 opacity-70 transition group-hover:opacity-100"><button onClick={() => onOpenDetail(asset)} className="rounded-md p-2 text-[#60758A] hover:bg-[#F0F5F8] hover:text-[#193B57]" aria-label={`Hồ sơ ${asset.code}`}><FileText size={15} /></button>{asset.statusType === "maintenance" && <button onClick={() => onOpenMaintenance(asset)} className="rounded-md p-2 text-[#A86B00] hover:bg-[#FFF5DC]" aria-label={`Mở phiếu Bảo hành/Sửa chữa ${asset.code}`}><Wrench size={15} /></button>}<button onClick={() => onEdit(asset)} className="rounded-md p-2 text-[#60758A] hover:bg-[#EAF3FF] hover:text-[#2666A8]" aria-label={`Chỉnh sửa ${asset.code}`}><Settings2 size={15} /></button><button onClick={() => onOpenQr(asset)} className="rounded-md p-2 text-[#60758A] hover:bg-[#E8F7F5] hover:text-[#087A6A]" aria-label={`Mã QR ${asset.code}`}><QrCode size={15} /></button><button onClick={() => onAssign(asset)} className="rounded-md p-2 text-[#60758A] hover:bg-[#FFF5DC] hover:text-[#A86B00]" aria-label={`Cấp phát ${asset.code}`}><PackageCheck size={15} /></button></div></td></tr>)}</tbody></table>{!isFiltering && assets.length === 0 && <div data-asset-catalog-empty className="px-6 py-16 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#EAF7F6] text-[#0F8C8C]"><Search size={22} /></div><div className="mt-4 text-base font-extrabold text-[#193B57]">Không có tài sản phù hợp</div><p className="mx-auto mt-2 max-w-md text-xs leading-5 text-[#71869A]">Không có tài sản nào khớp với điều kiện lọc hiện tại. Bạn có thể xóa bộ lọc để xem lại toàn bộ danh mục.</p><button type="button" onClick={onReset} className="mt-4 inline-flex h-9 items-center justify-center rounded-lg border border-[#B8DDD9] bg-white px-3 text-xs font-bold text-[#087A6A] transition hover:bg-[#ECF8F7]">Xóa tất cả bộ lọc</button></div>}</div>
...
function PaginatedAssetCatalogPage({ assets, allAssets, statusCounts, branchCounts, departmentCounts, vendorCounts, brandCounts, departmentOptionLabels, isFilterDataLoading, isFiltering, query, status, department, vendor, brand, warranty, branch, invoice, invoiceOptions, departmentOptions, vendorOptions, brandOptions, branchOptions, onQueryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onWarrantyChange, onBranchChange, onInvoiceChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onOpenMaintenance, onOpenInvoice, onAssign, canEditSectionLabels = false }: { assets: Asset[]; allAssets: Asset[]; statusCounts: Record<string, number>;  branchCounts: Record<string, number>; departmentCounts: Record<string, number>; vendorCounts: Record<string, number>; brandCounts: Record<string, number>; departmentOptionLabels: Record<string, string>; isFilterDataLoading: boolean; isFiltering: boolean; query: string; status: string; department: string; vendor: string; brand: string; warranty: string; branch: string; invoice: string; invoiceOptions: string[]; departmentOptions: string[]; vendorOptions: string[]; brandOptions: string[]; branchOptions: string[]; onQueryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onWarrantyChange: (value: string) => void; onBranchChange: (value: string) => void; onInvoiceChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onOpenMaintenance: (asset: Asset) => void; onOpenInvoice: (asset: Asset) => void; onAssign: (asset: Asset) => void; canEditSectionLabels?: boolean }) {
  ...
  const allAssetExportRows = useMemo(() => buildFilteredAssetExportRows(allAssets), [allAssets]);
  ...
  const exportAssetRowsExcel = (rows: ReturnType<typeof buildFilteredAssetExportRows>, scope: "filtered" | "all") => {
    if (!rows.length) { toast.info(scope === "filtered" ? "Không có tài sản phù hợp với bộ lọc hiện tại để xuất Excel." : "Chưa có tài sản để xuất Excel."); return; }
    if (isExportingFilteredAssets) return;
    setIsExportingFilteredAssets(true);
    const isFiltered = scope === "filtered";
    const loadingToast = toast.loading(isFiltered ? "Đang tạo danh sách tài sản theo bộ lọc..." : "Đang tạo toàn bộ danh sách tài sản...");
    window.setTimeout(async () => {
      try {
        ...
        await writeBrandedWorkbook(workbook, {
          documentTitle: isFiltered ? "DANH SÁCH TÀI SẢN THEO BỘ LỌC" : "DANH SÁCH TOÀN BỘ TÀI SẢN",
          fileName: `assetmaster-danh-sach-tai-san-${isFiltered ? "da-loc" : "tat-ca"}-${new Date().toISOString().slice(0, 10)}.xlsx`,
          description: isFiltered ? `Danh sách ${rows.length} tài sản theo toàn bộ bộ lọc hiện tại.` : `Danh sách toàn bộ ${rows.length} tài sản hiện có trong danh mục.`,
        });
        toast.success(isFiltered ? `Đã xuất ${rows.length} tài sản theo bộ lọc ra Excel.` : `Đã xuất toàn bộ ${rows.length} tài sản ra Excel.`, { id: loadingToast });
      } catch (error) {
        console.error(error);
        toast.error(isFiltered ? "Không thể xuất danh sách tài sản theo bộ lọc." : "Không thể xuất toàn bộ danh sách tài sản.", { id: loadingToast });
      } finally {
        setIsExportingFilteredAssets(false);
      }
    }, 180);
  };
  const exportFilteredAssetsExcel = () => exportAssetRowsExcel(filteredAssetExportRows, "filtered");
  const exportAllAssetsExcel = () => exportAssetRowsExcel(allAssetExportRows, "all");
  ...
  useEffect(() => { setPage(1); }, [query, status, department, vendor, brand, warranty, branch, invoice, pageSize]);
...
function AssetQuickHandoverModal({ assetCode, onClose }: { assetCode: string; onClose: () => void }) {
  ...
  const [selectedAssets, setSelectedAssets] = useState<HandoverAssetSelection[]>([]);
  useEffect(() => {
    if (!selectedAsset) return;
    setForm((current) => ({ ...current, assetCode: selectedAsset.assetCode, assetName: selectedAsset.name }));
    setSelectedAssets((current) => current.length ? current : [selectedAsset]);
  }, [selectedAsset?.id, selectedAsset?.assetCode, selectedAsset?.name]);
  ...
  const save = () => {
    ...
    if (!primaryAsset || selectedAssets.some((asset) => asset.status !== "available") || !form.recipientUserId || !form.recipient.trim()) { toast.error("Vui lòng chọn nhân viên nhận hợp lệ và kiểm tra các tài sản còn sẵn có."); return; }
    createHandover.mutate({ assetId: primaryAsset.id, assetIds: selectedAssets.map((asset) => asset.id), recipientName: form.recipient, recipientDepartmentName: form.department || null, handedOverAt: Date.now(), dueBackAt: normalizePurchaseDate(form.dueBackAt), conditionOut: form.condition, accessories: form.accessories || null, supplyItems, note: form.note || null, recipientUserId: form.recipientUserId, recipientDepartmentId: form.recipientDepartmentId || null });
  };
  ...
  return <PersistedHandoverCreateModal form={form} assets={assetsQuery.data || []} selectedAssets={selectedAssets} onSelectedAssetsChange={setSelectedAssets} employees={recipientsQuery.data || []} departments={departmentsQuery.data || []} supplies={suppliesQuery.data || []} supplyItems={supplyItems} onSupplyItemsChange={setSupplyItems} update={update} onRecipientChange={chooseRecipient} onClose={onClose} onSave={save} saving={createHandover.isPending} />;
}
...
function AssignmentsPage({ showComingSoon, companyInfo }: { showComingSoon: (label: string) => void; companyInfo: CompanyInfo }) {
  ...
  const [query, setQuery] = useState("");
  const [statusFilter, setStatusFilter] = usePersistedState("assetmaster.filters.handovers.status", "Tất cả trạng thái");
  const [handoverYearFilter, setHandoverYearFilter] = usePersistedState("assetmaster.filters.handovers.year", "Tất cả các năm");
  const [handoverDepartmentFilter, setHandoverDepartmentFilter] = usePersistedState("assetmaster.filters.handovers.department", "Tất cả phòng ban");
  const [handoverBranchFilter, setHandoverBranchFilter] = usePersistedState("assetmaster.filters.handovers.branch", "Tất cả chi nhánh");
  const [handoverRecipientFilter, setHandoverRecipientFilter] = usePersistedState("assetmaster.filters.handovers.recipient", "Tất cả người nhận");
  const [handoverLicenseFilter, setHandoverLicenseFilter] = usePersistedState("assetmaster.filters.handovers.license", "Tất cả Bản quyền");
  ...
  const [supplyItems, setSupplyItems] = useState<HandoverSupplyItem[]>([]);
  const [selectedHandoverAssets, setSelectedHandoverAssets] = useState<HandoverAssetSelection[]>([]);
  useEffect(() => {
  ...
  useEffect(() => { if (!handoversQuery.data) return; setHandovers(handoversQuery.data.map((item) => { const assetItems = item.assetItems?.length ? item.assetItems : []; const primaryAsset = assetItems[0] || assignmentAssetsQuery.data?.find((asset) => asset.id === item.assetId); return ({ id: item.id, referenceCode: item.referenceCode, assetCode: primaryAsset && "assetCode" in primaryAsset ? primaryAsset.assetCode : `TS-${item.assetId}`, assetName: primaryAsset ? ("assetName" in primaryAsset ? primaryAsset.assetName : primaryAsset.name) : "Tài sản", assetItems: assetItems.map((assetItem) => ({ id: assetItem.id, assetId: assetItem.assetId, assetCode: assetItem.assetCode, assetName: assetItem.assetName, conditionOut: assetItem.conditionOut, conditionIn: assetItem.conditionIn })), recipient: item.recipientName, department: item.recipientDepartmentName || "Chưa xác định", date: new Date(item.handedOverAt).toLocaleDateString("vi-VN"), status: item.status === "active" ? "Đã bàn giao" : item.status === "pending_signature" ? "Chờ ký" : item.status === "returned" ? "Đã hoàn trả" : "Nháp", condition: item.conditionOut || "Tốt", handoverBy: item.handoverByName || "Quản trị viên", note: item.note || "", accessories: item.accessories || "", recipientSignatureUrl: item.recipientSignatureUrl, recoveryCertificateNumber: item.recoveryCertificateNumber, recoveryCertificateYear: item.recoveryCertificateYear, recoveryCertificateMonth: item.recoveryCertificateMonth, recoveryCertificateSequence: item.recoveryCertificateSequence }); })); }, [handoversQuery.data, assignmentAssetsQuery.data]);
  ...
  const update = (key: keyof Handover, value: string) => setForm((current) => ({ ...current, [key]: value }));
  const createHandover = () => { const primaryAsset = selectedHandoverAssets[0]; if (!form.recipient.trim() || !form.recipientUserId || !primaryAsset || selectedHandoverAssets.some((asset) => asset.status !== "available")) { toast.error("Vui lòng chọn ít nhất một tài sản sẵn có và nhân viên nhận hợp lệ."); return; } createHandoverMutation.mutate({ assetId: primaryAsset.id, assetIds: selectedHandoverAssets.map((asset) => asset.id), recipientName: form.recipient, recipientDepartmentName: form.department || null, handedOverAt: Date.now(), dueBackAt: normalizePurchaseDate(form.dueBackAt), conditionOut: form.condition, accessories: form.accessories || null, supplyItems, note: form.note || null, recipientUserId: form.recipientUserId, recipientDepartmentId: form.recipientDepartmentId || null }); setModal(null); };
  const openCreateForAssets = (assets: HandoverAssetSelection[]) => {
    const uniqueAssets = Array.from(new Map(assets.map((asset) => [asset.id, asset])).values());
    const unavailableAsset = uniqueAssets.find((asset) => asset.status !== "available");
    if (!uniqueAssets.length || unavailableAsset) {
      toast.error(unavailableAsset ? `Không thể bàn giao ${unavailableAsset.assetCode}: tài sản không ở trạng thái Sẵn có.` : "Chưa có tài sản nào được chọn.");
      return;
    }
    const primaryAsset = uniqueAssets[0];
    setSelectedHandoverAssets(uniqueAssets);
    setForm({ id: 0, referenceCode: "", assetCode: primaryAsset.assetCode, assetName: primaryAsset.name, recipient: "", department: "", date: new Date().toLocaleDateString("vi-VN"), status: "Nháp", condition: "Tốt", handoverBy: "", note: "", accessories: "", recipientUserId: null, recipientDepartmentId: null });
    setSupplyItems([]);
    setModal("create");
  };
  const openCreateForAsset = (asset: HandoverAssetSelection) => {
    if (asset.status !== "available") {
      toast.error(`Không thể bàn giao ${asset.assetCode}: tài sản không ở trạng thái Sẵn có.`);
      return;
    }
    openCreateForAssets([asset]);
  };
  const openCreate = () => {
    const firstAsset = assignmentAssetsQuery.data?.find((asset) => asset.status === "available");
    if (!firstAsset) {
      toast.error("Cần có ít nhất một tài sản sẵn có trước khi lập phiếu bàn giao.");
      return;
    }
    openCreateForAsset(firstAsset);
  };
...
function PersistedHandoverCreateModal({ form, assets, selectedAssets, onSelectedAssetsChange, employees, departments, supplies, supplyItems, onSupplyItemsChange, update, onRecipientChange, onClose: dismiss, onSave, saving }: { form: Handover; assets: HandoverAssetSelection[]; selectedAssets: HandoverAssetSelection[]; onSelectedAssetsChange: (assets: HandoverAssetSelection[]) => void; employees: Array<{ id: number; name: string | null; email: string | null; departmentId: number | null; branchId: number | null; isActive: boolean }>; departments: Array<{ id: number; name: string }>; supplies: Array<{ id: number; code: string; name: string; unit: string; stockQuantity: string | number; isActive: boolean }>; supplyItems: HandoverSupplyItem[]; onSupplyItemsChange: (items: HandoverSupplyItem[]) => void; update: (key: keyof Handover, value: string) => void; onRecipientChange: (userId: number) => void; onClose: () => void; onSave: () => void; saving: boolean }) {
  const availableAssets = assets.filter((asset) => asset.status === "available");
  const selectedAssetIds = new Set(selectedAssets.map((asset) => asset.id));
  const selectableAssets = availableAssets.filter((asset) => !selectedAssetIds.has(asset.id));
  const addSelectedAsset = (assetCode: string) => {
    const asset = availableAssets.find((candidate) => candidate.assetCode === assetCode);
    if (!asset || selectedAssetIds.has(asset.id)) return;
    const nextAssets = [...selectedAssets, asset];
    onSelectedAssetsChange(nextAssets);
    if (!form.assetCode) {
      update("assetCode", asset.assetCode);
      update("assetName", asset.name);
    }
  };
  const removeSelectedAsset = (assetId: number) => {
    const nextAssets = selectedAssets.filter((asset) => asset.id !== assetId);
    onSelectedAssetsChange(nextAssets);
    if (selectedAssets[0]?.id === assetId) {
      update("assetCode", nextAssets[0]?.assetCode || "");
      update("assetName", nextAssets[0]?.name || "");
    }
  };
  ...
  const initialFormRef = useRef(JSON.stringify({ form, selectedAssets }));
  const addSupplyItem = () => {
  ...
  const onClose = () => {
    if (initialFormRef.current !== JSON.stringify({ form, selectedAssets })) { toast.warning("Đóng phiếu chưa lưu?", { description: "Thông tin bàn giao đang nhập sẽ bị hủy.", action: { label: "Bỏ thay đổi", onClick: dismiss } }); return; }
    dismiss();
  };
  useModalDismiss(onClose);
  useEffect(() => {
...
const loadImageData = loadPdfImageData;

async function loadHandoverPdfFont() {
  try {
    const response = await fetch(handoverPdfFontUrl);
    if (!response.ok) return null;
    return await response.arrayBuffer();
  } catch {
    // Keep PDF generation available when a reverse proxy or an old Docker
    // image cannot serve the optional bundled font asset.
    return null;
  }
}
...
async function downloadHandoverPdf(item: Handover, signature: string | undefined, companyInfo: CompanyInfo) {
  ...
  if (fontBuffer) registerVietnamesePdfFont(doc, fontBuffer);
...
function HandoverDetailModal({ item: listItem, companyInfo, onClose, onDataChanged, autoOpenRecoveryCertificate, onRecoveryPdfHandled }: { item: Handover; companyInfo: CompanyInfo; onClose: () => void; onDataChanged: () => void; autoOpenRecoveryCertificate: string | null; onRecoveryPdfHandled: () => void }) {
  ...
  }, [autoOpenRecoveryCertificate, handoverDetailQuery.isLoading, handoverDetailQuery.data, item.id, item.status, item.recoveryCertificateNumber, companyInfo, onRecoveryPdfHandled, onClose]);
  useModalDismiss(onClose);
  const preparePdf = (kind: "handover" | "recovery") => {
    ...
    void pdf.then(() => toast.success("Đã mở xem trước PDF.")).catch((error) => {
      console.error("AssetMaster PDF preview failed", error);
      const detail = error instanceof Error && error.message ? `: ${error.message}` : "";
      toast.error(`Không thể chuẩn bị bản xem trước PDF${detail}`);
  ...
  return <div onMouseDown={(event) => { if (event.target === event.currentTarget && !isBusy) onClose(); }} className="fixed inset-0 z-[60] flex items-center justify-center bg-[#102A43]/40 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Biên bản bàn giao">
    <div className="max-h-[92vh] w-full max-w-[760px] overflow-y-auto rounded-2xl border border-[#DDE7F0] bg-white shadow-[0_24px_70px_rgba(16,42,67,0.22)]">
      ...
      <div className="space-y-5 p-6"><div className="flex flex-col justify-between gap-4 rounded-xl bg-[#102A43] p-5 text-white sm:flex-row sm:items-center"><div><div className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#A5C3D2]">Tài sản cấp phát</div><div className="mt-1 font-display text-lg font-extrabold">{item.assetName}</div><div className="mt-1 font-mono text-[11px] text-[#71D6CE]">{item.assetCode}</div></div><span className="inline-flex items-center gap-1.5 self-start rounded-full bg-white/10 px-3 py-1.5 text-[10px] font-extrabold text-[#BDF1E8]"><CheckCircle2 size={13} />{item.status}</span></div>
        ...
        <section data-handover-license-detail className="rounded-xl border border-[#D6E5F8] bg-[#F5F9FF] p-4"><div className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#2666A8]">Bản quyền liên quan đến phiếu</div><p className="mt-1 text-[11px] leading-5 text-[#527089]">Dùng cùng dữ liệu với cột Bản quyền của phiếu. Chỉ Bản quyền gắn đúng tài sản và người nhận mới được thu hồi khi hoàn trả tài sản.</p>{handoverLicenseAllocationsQuery.isLoading ? <p className="mt-2 text-xs text-[#71869A]">Đang tải Bản quyền...</p> : licensesForRecipient.length ? <div className="mt-3 space-y-2">{licensesForRecipient.map((allocation) => { const returnsWithAsset = allocation.assetId === handoverDetailQuery.data?.assetId; return <div key={allocation.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-white bg-white/80 px-3 py-2"><span className="text-xs font-extrabold text-[#193B57]" title={allocation.licenseCode || allocation.productName}>{allocation.productName}</span><span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${returnsWithAsset ? "bg-[#E6F6F2] text-[#087A6A]" : "bg-[#EAF0F7] text-[#527089]"}`}>{returnsWithAsset ? "Sẽ thu hồi cùng tài sản" : "Cấp theo người nhận"}</span></div>; })}</div> : <p className="mt-2 text-xs font-semibold text-[#71869A]">Người nhận chưa có Bản quyền đang cấp.</p>}</section>
...
function AssetPurchaseInvoiceField({ value, invoices, loading, onChange }: { value: number | null | undefined; invoices: Array<{ id: number; invoiceKey: string; status: string }>; loading: boolean; onChange: (value: string) => void }) {
  const options = invoices.filter((invoice) => invoice.status !== "cancelled").map((invoice) => ({ value: String(invoice.id), label: invoice.invoiceKey, searchText: invoice.invoiceKey }));
  return <div className="min-w-0"><label className="field-label">Hóa đơn mua bán</label><SearchableSelect value={value ? String(value) : ""} onChange={onChange} options={options} placeholder="Chưa liên kết Hóa đơn" searchPlaceholder="Tìm mã hoặc số hóa đơn..." emptyText="Không tìm thấy hóa đơn phù hợp" loading={loading} /></div>;
}
...
function AssetModal({ mode, asset, formData, setFormData, isSaving, onClose: dismiss, onSave, onEdit, onStartHandover, onReturnToInvoice }: { mode: "create" | "edit" | "detail"; asset: Asset | null; formData: Asset; setFormData: React.Dispatch<React.SetStateAction<Asset>>; isSaving: boolean; onClose: () => void; onSave: (attachments?: AssetSaveAttachments) => void; onEdit: () => void; onStartHandover: (assetCode: string) => void; onReturnToInvoice?: () => void }) {
  ...
  }, [isDetail, asset, retirementCompanyQuery.data, dismiss]);
  useEffect(() => {
    if (!isDetail || asset?.statusType !== "retired") return;
    const dialog = document.querySelector('[role="dialog"][aria-label="Chi tiết tài sản"]');
  ...
  useEffect(() => {
    ...
    const openLicenseDetail = (assignment: typeof activeAssignments[number]) => {
      ...
      overlay.dataset.assetLicenseQuickView = "true";
      overlay.className = "fixed inset-0 z-[130] grid place-items-center p-4";
      const backdrop = document.createElement("button");
      backdrop.type = "button";
      backdrop.className = "absolute inset-0 bg-[#102A43]/40 backdrop-blur-[1px]";
      backdrop.setAttribute("aria-label", "Đóng chi tiết License");
      const panel = document.createElement("section");
      panel.className = "relative z-[1] w-full max-w-md rounded-2xl border border-[#CDE5E5] bg-white p-5 shadow-[0_24px_70px_rgba(16,42,67,.24)]";
      panel.setAttribute("role", "dialog");
      panel.setAttribute("aria-modal", "true");
      panel.setAttribute("aria-label", `Chi tiết License ${license?.productName || assignment.softwareLicenseId}`);
      ...
      closeButton.className = "absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-lg text-[#71869A] transition hover:bg-[#EEF5F7] hover:text-[#193B57]";
      closeButton.textContent = "×";
      closeButton.setAttribute("aria-label", "Đóng chi tiết License");
      closeButton.addEventListener("click", close);
      const eyebrow = document.createElement("div");
      eyebrow.className = "text-[10px] font-extrabold uppercase tracking-[.14em] text-[#0F8C8C]";
      eyebrow.textContent = "License cấp theo thiết bị";
      const title = document.createElement("h3");
      title.className = "mt-2 pr-8 text-lg font-extrabold text-[#102A43]";
      title.textContent = license?.productName || `Bản quyền #${assignment.softwareLicenseId}`;
      const grid = document.createElement("div");
      grid.className = "mt-4 grid gap-2 sm:grid-cols-2";
      const accessLabel = assignment.softwareLicenseKeyId ? "Key riêng" : assignment.softwareLicenseActivationAccountId ? "Tài khoản chủ" : "Chỗ dùng";
      [["Mã License", license?.licenseCode || "Chưa có mã"], ["Phương thức", accessLabel], ["Thiết bị", `${persistedAsset.assetCode} · ${persistedAsset.name}`], ["Thời gian cấp", dateFormatter.format(new Date(assignment.assignedAt))], ["Trạng thái", "Đang cấp phát"]].forEach(([label, value]) => {
        const row = document.createElement("div");
        row.className = "min-w-0 rounded-lg border border-[#E7EEF3] bg-[#FBFCFD] px-3 py-2.5";
        const rowLabel = document.createElement("div");
        rowLabel.className = "text-[9px] font-extrabold uppercase tracking-[.1em] text-[#8AA0B6]";
        rowLabel.textContent = label;
        const rowValue = document.createElement("div");
        rowValue.className = "mt-1 break-words text-xs font-bold leading-5 text-[#193B57]";
        rowValue.textContent = value;
        row.append(rowLabel, rowValue);
        grid.append(row);
      });
      panel.append(closeButton, eyebrow, title, grid);
    ...
    uniqueAssignments.forEach((assignment) => {
      ...
      const assignedAt = document.createElement("span");
      assignedAt.className = "mt-0.5 font-sans text-[9px] font-semibold text-[#60758A]";
      assignedAt.textContent = `Cấp: ${dateFormatter.format(new Date(assignment.assignedAt))}`;
      item.append(name, assignedAt);
      item.addEventListener("click", () => openLicenseDetail(assignment));
      values.append(item);
    });
...
function QrLookupModal({ assets, onClose, onOpenAsset }: { assets: Asset[]; onClose: () => void; onOpenAsset: (asset: Asset) => void }) {
  ...
  const scannableAssets = useMemo(
    () => assets.map((asset) => ({
      assetCode: asset.code,
      name: asset.name,
      qrToken: asset.qrToken,
      status: asset.statusType,
      source: asset,
    })),
    [assets]
  );

  const findAsset = (rawValue: string) => {
    const matched = findAssetByQrValue(scannableAssets, rawValue);
    setMatchedAsset(matched?.source || null);
...
function HandoverQrScannerModal({ assets, onClose, onConfirm }: { assets: HandoverQrAsset[]; onClose: () => void; onConfirm: (assets: HandoverQrAsset[]) => void }) {
  ...
  const addScannedAsset = (rawValue: string) => {
    ...
    if (selectedAssets.some((selectedAsset) => selectedAsset.id === asset.id)) {
      setMessage(`Tài sản ${asset.assetCode} đã có trong phiếu; hệ thống không thêm trùng.`);
      return;
    }
    if (selectedAssets.length >= 50) {
      setMessage("Mỗi phiếu được chứa tối đa 50 tài sản.");
      return;
    }
    setSelectedAssets((current) => [...current, asset]);
    setMessage("");
    toast.success(`Đã thêm ${asset.assetCode} vào phiếu (${selectedAssets.length + 1} tài sản).`);
  };

  return <div onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className="fixed inset-0 z-[80] flex items-center justify-center bg-[#102A43]/50 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Quét QR để bàn giao">
    <div className="max-h-[calc(100dvh-32px)] w-full max-w-2xl overflow-y-auto rounded-2xl border border-[#DDE7F0] bg-white p-6 shadow-[0_24px_70px_rgba(16,42,67,0.24)]">
      <div className="flex items-start justify-between"><div><div className="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#0F8C8C]"><QrCode size={14} />Bàn giao bằng QR</div><h2 className="mt-2 font-display text-xl font-extrabold text-[#102A43]">Quét liên tiếp các tài sản</h2><p className="mt-1 text-xs leading-5 text-[#71869A]">Camera tiếp tục hoạt động sau mỗi mã. Tài sản hợp lệ được gom vào cùng một phiếu.</p></div><button onClick={onClose} className="rounded-lg p-2 text-[#8AA0B6] hover:bg-[#F0F5F8]" aria-label="Đóng"><X size={18} /></button></div>
      <div className="mt-5"><AssetQrCapture value={input} onValueChange={(value) => { setInput(value); setMessage(""); }} onScan={addScannedAsset} continuousCamera submitLabel="Kiểm tra & thêm tài sản" helperText="Quét liên tiếp bằng camera hoặc máy quét keyboard/HID gửi Enter. Mã trùng sẽ không được thêm lần hai." /></div>
      {message && <p role="alert" className="mt-3 rounded-lg border border-[#F2D596] bg-[#FFF9EB] px-3 py-2 text-xs font-semibold leading-5 text-[#A86B00]">{message}</p>}
      <section className="mt-4 rounded-xl border border-[#DDE7F0] bg-[#FBFCFD] p-4" data-handover-qr-queue>
        <div className="flex items-center justify-between gap-3"><div><div className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#0F8C8C]">Danh sách tài sản</div><p className="mt-1 text-xs text-[#71869A]">Có thể xóa mã quét nhầm trước khi tiếp tục.</p></div><span className="rounded-full bg-[#E6F6F2] px-3 py-1 text-xs font-extrabold text-[#087A6A]">{selectedAssets.length} tài sản</span></div>
        {selectedAssets.length ? <div className="mt-3 max-h-64 space-y-2 overflow-y-auto pr-1">{selectedAssets.map((asset, index) => <div key={asset.id} className="flex items-center justify-between gap-3 rounded-lg border border-[#CDE5E5] bg-white px-3 py-2"><div className="min-w-0"><div className="truncate text-xs font-extrabold text-[#193B57]">{asset.name}</div><div className="mt-0.5 font-mono text-[10px] font-bold text-[#0F8C8C]">{index + 1}. {asset.assetCode}</div></div><button type="button" onClick={() => setSelectedAssets((current) => current.filter((item) => item.id !== asset.id))} className="grid h-8 w-8 shrink-0 place-items-center rounded-md text-[#B44545] hover:bg-[#FFF0F0]" aria-label={`Xóa ${asset.assetCode}`}><X size={14} /></button></div>)}</div> : <div className="mt-3 rounded-lg border border-dashed border-[#C8D7E1] bg-white px-3 py-5 text-center text-xs font-semibold text-[#8AA0B6]">Chưa quét tài sản nào.</div>}
      </section>
      <div className="mt-5 flex flex-col-reverse justify-end gap-2 border-t border-[#E7EEF3] pt-4 sm:flex-row"><button onClick={onClose} className="modal-close-action">Hủy</button><button type="button" onClick={() => onConfirm(selectedAssets)} disabled={!selectedAssets.length} className="rounded-lg bg-[#0F8C8C] px-4 py-2.5 text-xs font-bold text-white transition hover:bg-[#087A6A] disabled:cursor-not-allowed disabled:opacity-50">Tiếp tục lập phiếu ({selectedAssets.length})</button></div>
    </div>
  </div>;
}

function FilterSelect({ value, onChange, options, counts, optionLabels, isLoading = false }: { value: string; onChange: (value: string) => void; options: string[]; counts?: Record<string, number>; optionLabels?: Record<string, string>; isLoading?: boolean }) {
