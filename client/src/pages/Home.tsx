...
function HandoverDetailModal({ item: listItem, companyInfo, onClose, onDataChanged }: { item: Handover; companyInfo: CompanyInfo; onClose: () => void; onDataChanged: () => void }) {
  ...
  const updateHandoverStatus = trpc.handovers.updateStatus.useMutation({
    onSuccess: (result, variables) => {
      onDataChanged();
      toast.success(variables.status === "returned" ? `Đã ghi nhận hoàn trả, trả tài sản về trạng thái sẵn có${result.returnedAccessoryCount ? ` và hoàn ${result.returnedAccessoryCount} loại phụ kiện vào kho.` : "."}` : "Đã cập nhật trạng thái phiếu bàn giao.");
      onClose();
    },
