  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] text-[#102A43] antialiased">
      ...
      <main className="min-h-screen lg:pl-[264px]">
        ...
        {activeNav === "Cài đặt" ? <CompanySettingsPage companyInfo={companyInfo} onSave={(next) => { setCompanyInfo(next); localStorage.setItem("assetmaster-company-info", JSON.stringify(next)); toast.success("Đã lưu thông tin công ty."); }} /> : null}
        <div className={`relative overflow-hidden px-4 py-7 sm:px-6 lg:px-9 lg:py-8 ${activeNav === "Bàn giao & Cấp phát" || activeNav === "Cài đặt" ? "hidden" : ""}`}>
