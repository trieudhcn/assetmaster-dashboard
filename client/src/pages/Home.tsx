...
{branchAssetValueSummary.length ? <div className="mt-5 grid gap-3 lg:grid-cols-2">{branchAssetValueSummary.map((item) => { const maxValue = Math.max(...branchAssetValueSummary.map((entry) => entry.value), 1); const share = item.value ? Math.max(8, (item.value / maxValue) * 100) : 0; return <article key={item.code} className="rounded-xl border border-[#E2ECEF] bg-[#FBFCFD] p-3.5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-xs font-extrabold text-[#193B57]">{item.name}</div><div className="mt-0.5 font-mono text-[10px] font-semibold text-[#71869A]">{item.code} · {item.count} tài sản</div></div><div className="shrink-0 text-right text-xs font-extrabold tabular-nums text-[#087A6A]">{formatVnd(item.value)} <span className="text-[10px] text-[#71869A]">VNĐ</span></div></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#EAF2F3]"><div className="h-full rounded-full bg-[#0F8C8C]" style={{ width: String(share) + "%" }} /></div></article>; })}</div> : <div className="mt-4 rounded-lg border border-dashed border-[#CDE5E5] bg-[#F8FCFB] px-4 py-7 text-center text-xs text-[#71869A]">Chưa có tài sản đủ điều kiện để thống kê theo Chi nhánh.</div>}
</section>
...
function PaginatedAssetCatalogPage({ assets, statusCounts, branchCounts, query, category, status, department, vendor, brand, warranty, branch, vendorOptions, brandOptions, branchOptions, onQueryChange, onCategoryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onWarrantyChange, onBranchChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onOpenMaintenance, onAssign, canEditSectionLabels = false }: { assets: Asset[]; statusCounts: Record<string, number>; branchCounts: Record<string, number>; query: string; category: string; status: string; department: string; vendor: string; brand: string; warranty: string; branch: string; vendorOptions: string[]; brandOptions: string[]; branchOptions: string[]; onQueryChange: (value: string) => void; onCategoryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onWarrantyChange: (value: string) => void; onBranchChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onOpenMaintenance: (asset: Asset) => void; onAssign: (asset: Asset) => void; canEditSectionLabels?: boolean }) {
...
function AssetBranchSelector({ value, branches, isLoading, isError, onRetry, onChange }: { value: number | null; branches: Array<{ id: number; code: string; name: string; isActive: boolean }>; isLoading: boolean; isError: boolean; onRetry: () => void; onChange: (branchId: number | null) => void }) {
  ...
  return <div data-asset-branch className="min-w-0"><label className="field-label">Chi nhánh</label>{isLoading ? <div className="field-input flex items-center text-xs text-[#71869A]">Đang tải Chi nhánh...</div> : isError ? <div className="field-input flex items-center gap-2 text-xs text-[#B44545]">Không thể tải <button type="button" onClick={onRetry} className="font-bold underline">Thử lại</button></div> : activeBranches.length === 0 ? <div className="field-input flex items-center text-xs text-[#A86B00]">Chưa có Chi nhánh hoạt động</div> : <SearchableSelect value={selectedValue} onChange={(next) => onChange(next ? Number(next) : null)} placeholder="Chọn Chi nhánh" searchPlaceholder="Tìm tên hoặc mã Chi nhánh..." options={activeBranches.map((branch) => ({ value: String(branch.id), label: `${branch.name} · ${branch.code}`, searchText: `${branch.name} ${branch.code}` }))} />}</div>;
}
