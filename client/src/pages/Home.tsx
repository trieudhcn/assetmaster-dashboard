...
export default function Home() {
  ...
  const resetAssetFilters = () => {
    ...
    startAssetFilterTransition(() => {
      setQuery(""); setCategory("Tất cả loại tài sản"); setStatus("Tất cả trạng thái"); setDepartment("Tất cả phòng ban"); setVendorFilter("Tất cả nhà cung cấp"); setBrandFilter("Tất cả hãng"); setWarrantyFilter("Tất cả bảo hành"); setBranchFilter("Tất cả chi nhánh"); setInvoiceFilter("Tất cả Hóa đơn"); setLicenseFilter("Tất cả Bản quyền");
    });
    assetFilterLoadingTimerRef.current = window.setTimeout(() => {
      setIsAssetFilterApplying(false);
      assetFilterLoadingTimerRef.current = null;
  ...
  const filteredAssets = useMemo(() => assetRowsWithLicenses.filter((asset) => {
    ...
    return matchesQuery && matchesCategory && matchesStatus && matchesDepartment && matchesVendor && matchesBrand && matchesWarranty && matchesBranch && matchesInvoice && matchesLicense;
  }), [assetRowsWithLicenses, query, category, status, department, vendorFilter, brandFilter, warrantyFilter, branchFilter, invoiceFilter, licenseFilter, branchesQuery.data]);
