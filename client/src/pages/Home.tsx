...
function AssetCatalogPage({ assets, totalAssets, statusCounts, query, category, status, department, vendor, brand, warranty, vendorOptions, brandOptions, onQueryChange, onCategoryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onWarrantyChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onOpenMaintenance, onAssign }: { assets: Asset[]; totalAssets: number; statusCounts: Record<string, number>; query: string; category: string; status: string; department: string; vendor: string; brand: string; warranty: string; vendorOptions: string[]; brandOptions: string[]; onQueryChange: (value: string) => void; onCategoryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onWarrantyChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onOpenMaintenance: (asset: Asset) => void; onAssign: (asset: Asset) => void }) {
  ...
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.16em] text-[#0F8C8C]"><span className="h-1.5 w-1.5 rounded-full bg-[#0F8C8C]" />Danh mục tài sản</div><h1 className="font-display text-[28px] font-extrabold tracking-[-0.045em] text-[#102A43] sm:text-[34px]">Danh mục tài sản</h1><p className="mt-1.5 text-sm text-[#71869A]">Tra cứu, phân loại và thực hiện các thao tác quản trị trên từng tài sản.</p></div><button onClick={onCreate} className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-[#0F8C8C] px-4 text-xs font-bold text-white shadow-[0_5px_14px_rgba(15,140,140,0.22)] hover:bg-[#087A6A]"><Plus size={16} />Thêm tài sản mới</button></div>
...
function PaginatedAssetCatalogPage({ assets, statusCounts, query, category, status, department, vendor, brand, warranty, vendorOptions, brandOptions, onQueryChange, onCategoryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onWarrantyChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onOpenMaintenance, onAssign }: { assets: Asset[]; statusCounts: Record<string, number>; query: string; category: string; status: string; department: string; vendor: string; brand: string; warranty: string; vendorOptions: string[]; brandOptions: string[]; onQueryChange: (value: string) => void; onCategoryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onWarrantyChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onOpenMaintenance: (asset: Asset) => void; onAssign: (asset: Asset) => void }) {
...
function PartialAccessoryReturnPanel({ supplyItems, quantities, onQuantityChange }: { supplyItems: NonNullable<Handover["supplyItems"]>; quantities: Record<number, string>; onQuantityChange: (id: number, quantity: string) => void }) {
  ...
  return <div className="mt-4 rounded-xl border border-[#F0DFC0] bg-[#FFFDF7] p-3"><div className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#A86B00]">Số lượng phụ kiện thực tế hoàn về kho</div><p className="mt-1 text-[11px] leading-5 text-[#8A7140]">Kiểm tra từng phụ kiện trước khi xác nhận. Có thể hoàn một phần; số còn lại sẽ được ghi nhận là chưa hoàn.</p><div className="mt-3 space-y-2">{outstandingItems.map((supplyItem) => <div key={supplyItem.id} className="grid grid-cols-[minmax(0,1fr)_116px] items-center gap-3 rounded-lg border border-[#F0E1BF] bg-white px-3 py-2"><div className="min-w-0"><div className="truncate text-xs font-bold text-[#193B57]">{supplyItem.supplyName} <span className="font-mono text-[10px] text-[#8AA0B6]">{supplyItem.supplyCode}</span></div><div className="mt-1 text-[10px] text-[#71869A]">Đang giữ: <b className="text-[#A86B00]">{supplyItem.outstanding} {supplyItem.unit}</b></div></div><label className="text-[10px] font-bold text-[#60758A]">Hoàn kho<input type="number" inputMode="decimal" min="0" max={supplyItem.outstanding} step="0.01" value={quantities[supplyItem.id] ?? String(supplyItem.outstanding)} onChange={(event) => onQuantityChange(supplyItem.id, event.target.value)} className="mt-1 h-8 w-full rounded-md border border-[#DDE7F0] bg-white px-2 text-right text-xs font-extrabold text-[#193B57] outline-none focus:border-[#0F8C8C]" aria-label={`Số lượng hoàn kho ${supplyItem.supplyName}`} /></label></div>)}</div>{missingItems.length > 0 && <div className="mt-3 rounded-lg border border-[#F2C6A2] bg-[#FFF3E8] px-3 py-2 text-[11px] font-semibold leading-5 text-[#A94B12]">Cảnh báo: còn {missingItems.length} loại phụ kiện chưa hoàn đủ. Số còn thiếu sẽ tiếp tục hiển thị là đang giữ sau khi thu hồi.</div>}</div>;
}
...
async function downloadAssetRecoveryPdf(item: Handover, companyInfo: CompanyInfo) {
  ...
  const watermark = await createPdfLogoWatermark(companyInfo.logoUrl).catch(() => null);
  applyPdfLogoWatermark(doc, watermark);
  openPdfPreview(doc, `${item.referenceCode}-bien-ban-thu-hoi-tai-san.pdf`, `Biên bản thu hồi tài sản ${item.referenceCode}`);
}
