...
function AssignmentsPage({ showComingSoon, companyInfo }: { showComingSoon: (label: string) => void; companyInfo: CompanyInfo }) {
  ...
  const hasActiveHandoverFilters = Boolean(query.trim()) || statusFilter !== "Tất cả trạng thái" || handoverYearFilter !== "Tất cả các năm" || handoverDepartmentFilter !== "Tất cả phòng ban" || handoverRecipientFilter !== "Tất cả người nhận";
  const resetHandoverFilters = () => { setQuery(""); setStatusFilter("Tất cả trạng thái"); setHandoverYearFilter("Tất cả các năm"); setHandoverDepartmentFilter("Tất cả phòng ban"); setHandoverRecipientFilter("Tất cả người nhận"); };
