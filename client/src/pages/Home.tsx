...
export default function Home() {
  ...
  const [assetRows, setAssetRows] = useState<Asset[]>(assets);
  ...
  const saveAsset = () => { if (!formData.name.trim() || !formData.value.trim()) { toast.error("Vui lòng nhập tên tài sản và giá trị."); return; } if (assetModal === "edit") { setAssetRows((rows) => rows.map((row) => row.code === formData.code ? formData : row)); toast.success("Đã cập nhật tài sản."); } else { setAssetRows((rows) => [formData, ...rows]); toast.success("Đã thêm tài sản mới."); } setAssetModal(null); };
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] text-[#102A43] antialiased">
      ...
      <main className="min-h-screen lg:pl-[264px]">
        ...
        {activeNav === "Bàn giao & Cấp phát" ? <AssignmentsPage showComingSoon={showComingSoon} /> : null}
        <div className={`relative overflow-hidden px-4 py-7 sm:px-6 lg:px-9 lg:py-8 ${activeNav === "Bàn giao & Cấp phát" ? "hidden" : ""}`}>
...
function AssignmentsPage({ showComingSoon }: { showComingSoon: (label: string) => void }) {
...
function HandoverDetailModal({ item, onClose, onPrint, onHistory }: { item: Handover; onClose: () => void; onPrint: () => void; onHistory: () => void }) {
