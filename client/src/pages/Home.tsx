      ...
      <main className="min-h-screen lg:pl-[264px]">
        ...
        <div className={`px-4 py-7 sm:px-6 lg:px-9 lg:py-8 ${activeNav === "Tổng quan" ? "" : "hidden"}`}>
          <div className="mx-auto max-w-[1500px]">
            ...
            <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 min-[1450px]:grid-cols-5">
              {dashboardKpis.map((kpi, index) => { const Icon = kpi.icon; const toneMap: Record<string, string> = { teal: "bg-[#E6F6F2] text-[#0F8C8C]", blue: "bg-[#EAF3FF] text-[#3278BD]", amber: "bg-[#FFF5DC] text-[#D38A00]", navy: "bg-[#EAF0F7] text-[#193B57]" }; const valueClass = `font-display font-extrabold tracking-[-0.04em] ${kpi.alertValue ? "text-[#C75419]" : "text-[#102A43]"} ${kpi.compactValue ? "whitespace-nowrap text-[20px]" : "text-[26px]"}`; return <div key={kpi.label} className="animate-kpi group rounded-xl border border-[#DFE9F0] bg-white p-5 shadow-[0_8px_24px_rgba(16,42,67,0.045)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(16,42,67,0.08)]" style={{ animationDelay: `${index * 45}ms` }}><div className="flex items-start justify-between"><div className={`grid h-10 w-10 place-items-center rounded-[11px] ${toneMap[kpi.tone]}`}><Icon size={19} /></div></div><div className="mt-5 text-[12px] font-semibold text-[#7890A5]">{kpi.label}</div><div className="mt-1 flex items-baseline gap-2">{kpi.alertValue ? <button type="button" onClick={() => { sessionStorage.setItem("assetmaster-maintenance-status-filter", "needs_attention"); navigateTo("Bảo hành & Sửa chữa"); }} className={`${valueClass} cursor-pointer rounded-md outline-none transition hover:text-[#A83F12] focus-visible:ring-2 focus-visible:ring-[#F0A516]`} aria-label="Xem phiếu Bảo hành/Sửa chữa cần xử lý" title="Xem phiếu cần xử lý">{kpi.value}</button> : <span className={valueClass}>{kpi.value}</span>}</div>{kpi.detail && <div className="mt-2 text-[11px] font-medium text-[#9AAEBD]">{kpi.detail}</div>}{kpi.subDetail && <div className="mt-2 text-[10px] font-extrabold text-[#A86B00]">{kpi.subDetail}</div>}</div>; })}
            </section>
