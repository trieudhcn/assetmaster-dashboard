...
function AssetModal({ mode, asset, formData, setFormData, onClose: dismiss, onSave, onEdit, onStartHandover }: { mode: "create" | "edit" | "detail"; asset: Asset | null; formData: Asset; setFormData: React.Dispatch<React.SetStateAction<Asset>>; onClose: () => void; onSave: (attachment?: SupplierReturnAttachment) => void; onEdit: () => void; onStartHandover: (assetCode: string) => void }) {
...
function FilterSelect({ value, onChange, options }: { value: string; onChange: (value: string) => void; options: string[] }) {
  return <SearchableSelect value={value} onChange={onChange} options={options.map((option) => ({ value: option, label: option }))} placeholder={options[0] || "Chọn một giá trị"} searchPlaceholder="Tìm trong dropdown..." className="w-full sm:w-[180px]" />;
}
