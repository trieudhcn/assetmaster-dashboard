...
function PaginatedAssetCatalogPage({ assets, allAssets, statusCounts, branchCounts, departmentCounts, vendorCounts, brandCounts, departmentOptionLabels, isFilterDataLoading, isFiltering, query, status, department, vendor, brand, warranty, branch, invoice, invoiceOptions, departmentOptions, vendorOptions, brandOptions, branchOptions, onQueryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onWarrantyChange, onBranchChange, onInvoiceChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onOpenMaintenance, onOpenInvoice, onAssign, canEditSectionLabels = false }: { assets: Asset[]; allAssets: Asset[]; statusCounts: Record<string, number>;  branchCounts: Record<string, number>; departmentCounts: Record<string, number>; vendorCounts: Record<string, number>; brandCounts: Record<string, number>; departmentOptionLabels: Record<string, string>; isFilterDataLoading: boolean; isFiltering: boolean; query: string; status: string; department: string; vendor: string; brand: string; warranty: string; branch: string; invoice: string; invoiceOptions: string[]; departmentOptions: string[]; vendorOptions: string[]; brandOptions: string[]; branchOptions: string[]; onQueryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onWarrantyChange: (value: string) => void; onBranchChange: (value: string) => void; onInvoiceChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onOpenMaintenance: (asset: Asset) => void; onOpenInvoice: (asset: Asset) => void; onAssign: (asset: Asset) => void; canEditSectionLabels?: boolean }) {
  ...
  const [isExportingQrLabels, setIsExportingQrLabels] = useState(false);
  const [isExportingMaintenance, setIsExportingMaintenance] = useState(false);
  const exportFilteredQrLabelsPdf = async () => {
    if (!allFilteredAssets.length) {
      toast.info("Không có tài sản phù hợp với bộ lọc hiện tại để in tem QR.");
      return;
    }
    if (isExportingQrLabels) return;
    setIsExportingQrLabels(true);
    const loadingToast = toast.loading(`Đang tạo ${allFilteredAssets.length} tem QR...`);
    try {
      ...
      for (let index = 0; index < allFilteredAssets.length; index += 1) {
        ...
        const asset = allFilteredAssets[index];
        const payload = `ASSETMASTER|${asset.qrToken || asset.code}`;
        const qrDataUrl = await QRCodeGenerator.toDataURL(payload, {
          width: 320,
          margin: 1,
        ...
        doc.roundedRect(x, y, cellWidth, cellHeight, 1.5, 1.5, "S");
        doc.addImage(qrDataUrl, "PNG", x + 3, y + 3, 28, 28, undefined, "FAST");
        doc.setTextColor(16, 42, 67);
        doc.setFontSize(8.5);
        doc.setFont(undefined, "bold");
        doc.text(asset.code, x + 34, y + 9);
        doc.setFont(undefined, "normal");
        doc.setFontSize(7);
        const nameLines = doc.splitTextToSize(asset.name || "Tài sản", cellWidth - 37).slice(0, 3);
        doc.text(nameLines, x + 34, y + 15);
        doc.setFontSize(6.2);
        doc.setTextColor(96, 117, 138);
        const branchName = (asset.branchLabel || "Chưa gán chi nhánh").split(" · ")[0];
        doc.text(doc.splitTextToSize(branchName, cellWidth - 37).slice(0, 2), x + 34, y + 31);
        doc.setTextColor(15, 140, 140);
        doc.setFontSize(5.5);
        doc.text("ASSETMASTER", x + 3, y + cellHeight - 3);
      }
      openPdfPreview(
        doc,
        `assetmaster-tem-qr-${new Date().toISOString().slice(0, 10)}.pdf`,
        `Tem QR tài sản (${allFilteredAssets.length})`
      );
      toast.success(`Đã tạo ${allFilteredAssets.length} tem QR theo bộ lọc hiện tại.`, { id: loadingToast });
    } catch (error) {
      console.error(error);
      toast.error("Không thể tạo file tem QR. Vui lòng thử lại.", { id: loadingToast });
    } finally {
      setIsExportingQrLabels(false);
    }
  };
...
function QrLookupModal({ assets, onClose, onOpenAsset }: { assets: Asset[]; onClose: () => void; onOpenAsset: (asset: Asset) => void }) {
  ...
  const findAsset = (rawValue = input) => {
    const candidate = rawValue.trim().replace(/^ASSETMASTER\|/i, "");
    if (!candidate) {
      setMessage("Hãy quét hoặc nhập mã QR tài sản.");
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        const preferredCamera = devices.find((device) => /back|rear|environment|sau/i.test(device.label)) || devices[0];
        if (!videoRef.current || cancelled) return;
        const controls = await reader.decodeFromVideoDevice(preferredCamera.deviceId, videoRef.current, (result) => {
          if (!result || decoded) return;
          decoded = true;
          const scannedValue = result.getText();
          setInput(scannedValue);
          const matched = findAsset(scannedValue);
          controls.stop();
          scannerControlsRef.current = null;
          setCameraOpen(false);
          if (matched) toast.success(`Đã nhận diện ${matched.code} bằng camera.`);
        });
        ...
        scannerControlsRef.current = null;
        setCameraOpen(false);
        const reason = error instanceof Error ? error.message : "Không thể truy cập camera.";
        setCameraError(
