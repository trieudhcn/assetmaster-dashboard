...
export default function Home() {
  ...
  const [activeNav, setActiveNav] = useState(() => {
    ...
    const deepLinks: Record<string, string> = { assets: "Danh mục tài sản", maintenance: "Bảo trì & Báo hỏng", audit: "Kiểm kê", reports: "Báo cáo", employees: "Quản lý nhân viên", organization: "Phòng Ban & Bộ Phận", handovers: "Bàn giao & Cấp phát" };
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] text-[#102A43] antialiased">
      ...
      <main className="min-h-screen lg:pl-[264px]">
        ...
        {activeNav === "Danh mục tài sản" ? <AssetCatalogPage assets={filteredAssets} totalAssets={assetRows.length} query={query} category={category} status={status} department={department} onQueryChange={setQuery} onCategoryChange={setCategory} onStatusChange={setStatus} onDepartmentChange={setDepartment} onReset={() => { setQuery(""); setCategory("Tất cả loại tài sản"); setStatus("Tất cả trạng thái"); setDepartment("Tất cả phòng ban"); }} onCreate={openCreateModal} onEdit={openEditModal} onOpenDetail={openDetailModal} onOpenQr={setQrAsset} onAssign={(asset) => { navigateTo("Bàn giao & Cấp phát"); toast.info(`Chọn ${asset.code} trong form tạo phiếu để tiếp tục cấp phát.`); }} /> : null}
...
function AssetCatalogPage({ assets, totalAssets, query, category, status, department, onQueryChange, onCategoryChange, onStatusChange, onDepartmentChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onAssign }: { assets: Asset[]; totalAssets: number; query: string; category: string; status: string; department: string; onQueryChange: (value: string) => void; onCategoryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onAssign: (asset: Asset) => void }) {
