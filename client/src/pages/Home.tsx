...
function AssignmentsPage({ showComingSoon, companyInfo }: { showComingSoon: (label: string) => void; companyInfo: CompanyInfo }) {
  ...
  const hasActiveHandoverFilters = Boolean(query.trim()) || statusFilter !== "Tất cả trạng thái" || handoverYearFilter !== "Tất cả các năm" || handoverDepartmentFilter !== "Tất cả phòng ban" || handoverRecipientFilter !== "Tất cả người nhận" || hideDepartmentsWithoutHandovers;
  const resetHandoverFilters = () => { setQuery(""); setStatusFilter("Tất cả trạng thái"); setHandoverYearFilter("Tất cả các năm"); setHandoverDepartmentFilter("Tất cả phòng ban"); setHandoverRecipientFilter("Tất cả người nhận"); setHideDepartmentsWithoutHandovers(false); };
...
function AssetModal({ mode, asset, formData, setFormData, isSaving, onClose: dismiss, onSave, onEdit, onStartHandover }: { mode: "create" | "edit" | "detail"; asset: Asset | null; formData: Asset; setFormData: React.Dispatch<React.SetStateAction<Asset>>; isSaving: boolean; onClose: () => void; onSave: (attachments?: AssetSaveAttachments) => void; onEdit: () => void; onStartHandover: (assetCode: string) => void }) {
  ...
  return <div data-asset-modal onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className="fixed inset-0 z-[60] flex items-center justify-center bg-[#102A43]/40 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={title}>
    <div className={`max-h-[92vh] w-full overflow-y-auto rounded-2xl border border-[#DDE7F0] bg-white shadow-[0_24px_70px_rgba(16,42,67,0.22)] ${isDetail ? "max-w-[560px]" : "max-w-[720px]"}`}>
      {!isDetail && <AssetBranchSelector value={formData.branchId ?? null} branches={branchesQuery.data || []} isLoading={branchesQuery.isLoading} isError={branchesQuery.isError} onRetry={() => { void branchesQuery.refetch(); }} onChange={(branchId) => setFormData((current) => ({ ...current, branchId }))} />}
...
function AssetBranchSelector({ value, branches, isLoading, isError, onRetry, onChange }: { value: number | null; branches: Array<{ id: number; code: string; name: string; isActive: boolean }>; isLoading: boolean; isError: boolean; onRetry: () => void; onChange: (branchId: number | null) => void }) {
  ...
  return <section data-asset-branch className="border-b border-[#E7EEF3] bg-[#F8FCFB] px-6 py-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><div className="text-xs font-extrabold text-[#193B57]">Chi nhánh quản lý</div><p className="mt-1 text-[11px] text-[#71869A]">Tài sản mới mặc định gán vào HO-Head Office.</p></div>{defaultBranch && <span className="rounded-full bg-[#E6F6F2] px-2 py-1 text-[10px] font-extrabold text-[#087A6A]">Mặc định: {defaultBranch.code}</span>}</div>{isLoading ? <p className="mt-3 text-xs text-[#71869A]">Đang tải Chi nhánh...</p> : isError ? <p className="mt-3 text-xs text-[#B44545]">Không thể tải Chi nhánh. <button type="button" onClick={onRetry} className="font-bold underline">Thử lại</button></p> : activeBranches.length === 0 ? <p className="mt-3 text-xs text-[#A86B00]">Chưa có Chi nhánh hoạt động. Hãy tạo HO-Head Office trong Cài đặt hệ thống.</p> : <select value={value ? String(value) : String(defaultBranch?.id || "")} onChange={(event) => onChange(event.target.value ? Number(event.target.value) : null)} className="field-input mt-3 h-10 w-full text-sm" aria-label="Chi nhánh quản lý tài sản"><option value="">Chọn Chi nhánh</option>{activeBranches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name} · {branch.code}</option>)}</select>}</section>;
}
