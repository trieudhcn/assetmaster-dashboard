...
export default function Home() {
  ...
  const filteredAssets = useMemo(() => assetRows.filter((asset) => {
    ...
    return matchesQuery && matchesCategory && matchesStatus && matchesDepartment && matchesVendor && matchesBrand && matchesWarranty && matchesBranch;
  }), [assetRows, query, category, status, department, vendorFilter, brandFilter, warrantyFilter, branchFilter, branchesQuery.data]);
  ...
  const refreshDashboardData = async () => {
    ...
    try {
      await Promise.all([assetQuery.refetch(), maintenanceTicketsQuery.refetch(), suppliesQuery.refetch()]);
...
function AssetCatalogPage({ assets, totalAssets, statusCounts, query, category, status, department, vendor, brand, warranty, branch, vendorOptions, brandOptions, branchOptions, onQueryChange, onCategoryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onWarrantyChange, onBranchChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onOpenMaintenance, onAssign, canEditSectionLabels = false }: { assets: Asset[]; totalAssets: number; statusCounts: Record<string, number>; query: string; category: string; status: string; department: string; vendor: string; brand: string; warranty: string; branch: string; vendorOptions: string[]; brandOptions: string[]; branchOptions: string[]; onQueryChange: (value: string) => void; onCategoryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onWarrantyChange: (value: string) => void; onBranchChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onOpenMaintenance: (asset: Asset) => void; onAssign: (asset: Asset) => void; canEditSectionLabels?: boolean }) {
...
function PaginatedAssetCatalogPage({ assets, statusCounts, query, category, status, department, vendor, brand, warranty, branch, vendorOptions, brandOptions, branchOptions, onQueryChange, onCategoryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onWarrantyChange, onBranchChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onOpenMaintenance, onAssign, canEditSectionLabels = false }: { assets: Asset[]; statusCounts: Record<string, number>; query: string; category: string; status: string; department: string; vendor: string; brand: string; warranty: string; branch: string; vendorOptions: string[]; brandOptions: string[]; branchOptions: string[]; onQueryChange: (value: string) => void; onCategoryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onWarrantyChange: (value: string) => void; onBranchChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onOpenMaintenance: (asset: Asset) => void; onAssign: (asset: Asset) => void; canEditSectionLabels?: boolean }) {
  ...
  useEffect(() => { setPage(1); }, [query, category, status, department, vendor, brand, warranty, branch, pageSize]);
...
function AssignmentsPage({ showComingSoon, companyInfo }: { showComingSoon: (label: string) => void; companyInfo: CompanyInfo }) {
  ...
  const [hideDepartmentsWithoutHandovers, setHideDepartmentsWithoutHandovers] = useState(false);
  ...
  const visibleHandoverDepartments = hideDepartmentsWithoutHandovers ? handoverDepartments.filter((department) => handoverDepartmentCounts[department] > 0 || department === handoverDepartmentFilter) : handoverDepartments;
  ...
  const hasActiveHandoverFilters = Boolean(query.trim()) || statusFilter !== "Tất cả trạng thái" || handoverYearFilter !== "Tất cả các năm" || handoverDepartmentFilter !== "Tất cả phòng ban" || handoverBranchFilter !== "Tất cả chi nhánh" || handoverRecipientFilter !== "Tất cả người nhận" || hideDepartmentsWithoutHandovers;
  const resetHandoverFilters = () => { setQuery(""); setStatusFilter("Tất cả trạng thái"); setHandoverYearFilter("Tất cả các năm"); setHandoverDepartmentFilter("Tất cả phòng ban"); setHandoverBranchFilter("Tất cả chi nhánh"); setHandoverRecipientFilter("Tất cả người nhận"); setHideDepartmentsWithoutHandovers(false); };
...
async function downloadAssetRecoveryPdf(item: Handover, companyInfo: CompanyInfo) {
  const doc = new jsPDF({ unit: "mm", format: "a4" });
  ...
  doc.setTextColor(15, 140, 140);
  doc.setFontSize(10);
  doc.text(companyInfo.name || "Doanh nghiệp", left + 24, y - 4);
  doc.setFontSize(8);
  doc.setTextColor(112, 134, 154);
...
function AssetModal({ mode, asset, formData, setFormData, isSaving, onClose: dismiss, onSave, onEdit, onStartHandover }: { mode: "create" | "edit" | "detail"; asset: Asset | null; formData: Asset; setFormData: React.Dispatch<React.SetStateAction<Asset>>; isSaving: boolean; onClose: () => void; onSave: (attachments?: AssetSaveAttachments) => void; onEdit: () => void; onStartHandover: (assetCode: string) => void }) {
  ...
  const branchesQuery = trpc.branches.list.useQuery(undefined, { enabled: !isDetail });
...
function AssetBranchSelector({ value, branches, isLoading, isError, onRetry, onChange }: { value: number | null; branches: Array<{ id: number; code: string; name: string; isActive: boolean }>; isLoading: boolean; isError: boolean; onRetry: () => void; onChange: (branchId: number | null) => void }) {
  ...
  return <section data-asset-branch className="sm:col-span-2 rounded-xl border border-[#CDE5E5] bg-[#F8FCFB] p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><div className="text-xs font-extrabold text-[#193B57]">Chi nhánh quản lý</div><p className="mt-1 text-[11px] text-[#71869A]">Tài sản mới mặc định gán vào HO-Head Office.</p></div>{defaultBranch && <span className="rounded-full bg-[#E6F6F2] px-2 py-1 text-[10px] font-extrabold text-[#087A6A]">Mặc định: {defaultBranch.code}</span>}</div>{isLoading ? <p className="mt-3 text-xs text-[#71869A]">Đang tải Chi nhánh...</p> : isError ? <p className="mt-3 text-xs text-[#B44545]">Không thể tải Chi nhánh. <button type="button" onClick={onRetry} className="font-bold underline">Thử lại</button></p> : activeBranches.length === 0 ? <p className="mt-3 text-xs text-[#A86B00]">Chưa có Chi nhánh hoạt động. Hãy tạo Chi nhánh trong Cài đặt hệ thống.</p> : <div className="mt-3"><SearchableSelect value={selectedValue} onChange={(next) => onChange(next ? Number(next) : null)} placeholder="Chọn Chi nhánh" searchPlaceholder="Tìm tên hoặc mã Chi nhánh..." options={activeBranches.map((branch) => ({ value: String(branch.id), label: `${branch.name} · ${branch.code}`, searchText: `${branch.name} ${branch.code}` }))} /></div>}</section>;
}
