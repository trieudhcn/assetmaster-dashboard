...
export default function Home() {
  ...
  const filteredAssets = useMemo(() => assetRows.filter((asset) => {
    ...
    return matchesQuery && matchesCategory && matchesStatus && matchesDepartment && matchesVendor && matchesBrand;
  }), [assetRows, query, category, status, department, vendorFilter, brandFilter]);
      ...
      <main className="min-h-screen lg:pl-[264px]">
        <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between gap-4 border-b border-[#DDE7F0] bg-[#FFFFFF]/95 px-4 shadow-[0_5px_20px_rgba(16,42,67,0.03)] backdrop-blur-xl sm:px-6 lg:px-9">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="relative"><button onClick={() => { setNotificationsOpen((current) => !current); setHeaderProfileOpen(false); }} className="relative rounded-lg p-2 text-[#60758A] hover:bg-[#F0F5F8]" aria-label="Thông báo" aria-expanded={notificationsOpen}><Bell size={19} /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-[#F0A516] ring-2 ring-white" /></button>{notificationsOpen && <div className="absolute right-0 top-[calc(100%+10px)] z-50 w-[320px] overflow-hidden rounded-xl border border-[#DDE7F0] bg-white shadow-[0_18px_42px_rgba(16,42,67,0.18)]"><div className="flex items-center justify-between border-b border-[#E7EEF3] px-4 py-3"><div><div className="text-xs font-extrabold text-[#193B57]">Thông báo</div><div className="mt-0.5 text-[10px] text-[#8AA0B6]">Cập nhật hoạt động gần đây</div></div><button onClick={() => setNotificationsOpen(false)} className="text-[10px] font-bold text-[#087A6A]">Đóng</button></div><div className="p-4"><div className="flex gap-3 rounded-lg bg-[#F7FAFC] p-3"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#E6F6F2] text-[#087A6A]"><Bell size={15} /></div><div><div className="text-xs font-bold text-[#193B57]">Chưa có thông báo mới</div><p className="mt-1 text-[11px] leading-5 text-[#71869A]">Các cập nhật bàn giao, bảo trì và import sẽ xuất hiện tại đây.</p></div></div></div></div>}</div>
          ...
          <div className="relative mx-auto max-w-[1500px]"><div className="mb-5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.15em] text-[#A86B00]"><span className="h-px w-8 bg-[#F0A516]" /><span className="h-1.5 w-1.5 rounded-full bg-[#F0A516]" />System pulse · live inventory signal</div>
            ...
            <section className="mt-6 grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
              <div className="rounded-xl border border-[#DFE9F0] bg-white p-5 shadow-[0_8px_24px_rgba(16,42,67,0.045)]">
                ...
                <div className="mt-5 space-y-4">{[{ label: "Đang cấp phát", count: assetRows.filter((asset) => asset.statusType === "active").length, tone: "bg-[#3278BD]" }, { label: "Sẵn có", count: assetRows.filter((asset) => asset.statusType === "available").length, tone: "bg-[#0F8C8C]" }, { label: "Bảo trì / hỏng", count: assetRows.filter((asset) => asset.statusType === "maintenance").length, tone: "bg-[#F0A516]" }].map((item) => <div key={item.label}><div className="flex justify-between text-xs"><span className="font-bold text-[#60758A]">{item.label}</span><span className="font-extrabold text-[#193B57]">{item.count} tài sản</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-[#EDF2F5]"><div className={`h-full rounded-full ${item.tone}`} style={{ width: `${assetRows.length ? Math.max((item.count / assetRows.length) * 100, item.count ? 8 : 0) : 0}%` }} /></div></div>)}</div>
              </div>
...
function AssetCatalogPage({ assets, totalAssets, query, category, status, department, vendor, brand, vendorOptions, brandOptions, onQueryChange, onCategoryChange, onStatusChange, onDepartmentChange, onVendorChange, onBrandChange, onReset, onCreate, onEdit, onOpenDetail, onOpenQr, onAssign }: { assets: Asset[]; totalAssets: number; query: string; category: string; status: string; department: string; vendor: string; brand: string; vendorOptions: string[]; brandOptions: string[]; onQueryChange: (value: string) => void; onCategoryChange: (value: string) => void; onStatusChange: (value: string) => void; onDepartmentChange: (value: string) => void; onVendorChange: (value: string) => void; onBrandChange: (value: string) => void; onReset: () => void; onCreate: () => void; onEdit: (asset: Asset) => void; onOpenDetail: (asset: Asset) => void; onOpenQr: (asset: Asset) => void; onAssign: (asset: Asset) => void }) {
...
function AssignmentsPage({ showComingSoon, companyInfo }: { showComingSoon: (label: string) => void; companyInfo: CompanyInfo }) {
  ...
  useEffect(() => {
    ...
    pendingReturnRequests.forEach((item) => {
      ...
      rejectButton.onclick = () => { if (window.confirm(`Từ chối yêu cầu hoàn trả ${item.assetCode}?`)) resolveReturnRequest.mutate({ id: item.id, decision: "rejected", resolution: null }); };
...
function PersistedHandoverCreateModal({ form, assets, employees, departments, update, onRecipientChange, onClose, onSave, saving }: { form: Handover; assets: Array<{ assetCode: string; name: string; status: string }>; employees: Array<{ id: number; name: string | null; email: string | null; departmentId: number | null; isActive: boolean }>; departments: Array<{ id: number; name: string }>; update: (key: keyof Handover, value: string) => void; onRecipientChange: (userId: number) => void; onClose: () => void; onSave: () => void; saving: boolean }) {
  ...
  useEffect(() => {
    const saveButton = Array.from(document.querySelectorAll<HTMLButtonElement>("button")).find((button) => button.textContent?.trim() === "Lưu phiếu nháp");
    if (!saveButton) return;
    const confirmBeforeSave = (event: MouseEvent) => {
      event.preventDefault();
      event.stopImmediatePropagation();
      const dueBackLabel = form.dueBackAt ? new Date(form.dueBackAt).toLocaleDateString("vi-VN") : "Chưa thiết lập";
      const summary = ["Xác nhận tạo phiếu bàn giao", "", `Tài sản: ${form.assetCode} · ${form.assetName}`, `Người nhận: ${form.recipient || "Chưa chọn"}`, `Phòng ban: ${form.department || "Chưa gán"}`, `Tình trạng: ${form.condition}`, `Hạn dự kiến hoàn trả: ${dueBackLabel}`, "", "Bạn đã kiểm tra đúng thông tin trên chưa?"].join("\n");
      if (window.confirm(summary)) onSave();
    };
    saveButton.addEventListener("click", confirmBeforeSave, true);
    return () => saveButton.removeEventListener("click", confirmBeforeSave, true);
  }, [form.assetCode, form.assetName, form.recipient, form.department, form.condition, form.dueBackAt, onSave]);
...
async function downloadHandoverPdf(item: Handover, signature: string | undefined, companyInfo: CompanyInfo, output: "download" | "print" = "download") {
  ...
  if (output === "print") {
    ...
    if (!printWindow) {
      ...
      throw new Error("Trình duyệt đã chặn cửa sổ in. PDF đã được tải xuống để bạn in thủ công.");
    }
...
function AssetModal({ mode, asset, formData, setFormData, onClose, onSave, onEdit, onStartHandover }: { mode: "create" | "edit" | "detail"; asset: Asset | null; formData: Asset; setFormData: React.Dispatch<React.SetStateAction<Asset>>; onClose: () => void; onSave: () => void; onEdit: () => void; onStartHandover: (assetCode: string) => void }) {
  ...
  useEffect(() => {
    ...
    const statusLabel = formData.statusType === "available" ? "Chưa bàn giao" : formData.statusType === "maintenance" ? "Bảo trì" : "";
    if (statusLabel && formData.holder !== statusLabel) {
      setFormData((current) => ({ ...current, holder: statusLabel }));
      return;
    }
    if (formData.statusType === "active" && ["Chưa bàn giao", "Bảo trì"].includes(formData.holder)) {
