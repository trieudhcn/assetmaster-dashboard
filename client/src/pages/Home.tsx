...
function EmployeeManagementPage() {
  ...
  const updateActiveStatus = trpc.employees.updateActiveStatus.useMutation({ onSuccess: () => { utils.employees.list.invalidate(); toast.success("Đã cập nhật trạng thái tài khoản."); } });
  const employees = (usersQuery.data || []).filter((employee) => (roleFilter === "all" || employee.role === roleFilter) && `${employee.name || ""} ${employee.email || ""}`.toLowerCase().includes(searchTerm.toLowerCase()));
  const selectedEmployee = employees.find((employee) => employee.id === selectedEmployeeId);
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]"><h1 className="font-display text-3xl font-extrabold text-[#102A43]">Quản lý nhân viên</h1><p className="mt-1 text-sm text-[#71869A]">Tài khoản và phân quyền truy cập hệ thống.</p><div className="mt-5 flex gap-3"><input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="field-input flex-1" placeholder="Tìm tên hoặc email..." /><select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="field-input w-44"><option value="all">Tất cả vai trò</option><option value="admin">Quản trị viên</option><option value="user">Nhân viên</option></select></div><div className="mt-4 overflow-hidden rounded-xl border border-[#DFE9F0] bg-white"><table className="w-full text-left text-xs"><thead className="bg-[#FBFCFD] text-[#8AA0B6]"><tr><th className="p-4">Nhân viên</th><th>Email</th><th>Vai trò</th><th className="p-4">Thao tác</th></tr></thead><tbody>{employees.map((employee) => <tr key={employee.id} className="border-t border-[#EDF2F5]"><td className="p-4 font-bold text-[#193B57]">{employee.name || "Chưa đặt tên"}</td><td>{employee.email || "—"}</td><td>{employee.role === "admin" ? "Quản trị viên" : "Nhân viên"}</td><td className="p-4"><button onClick={() => setSelectedEmployeeId(employee.id)} className="mr-2 rounded-md border border-[#DDE7F0] px-3 py-1.5 font-bold text-[#2666A8]">Xem thông tin</button><button onClick={() => updateRole.mutate({ id: employee.id, role: employee.role === "admin" ? "user" : "admin" })} className="rounded-md border border-[#CDE5E5] px-3 py-1.5 font-bold text-[#087A6A]">Đổi vai trò</button></td></tr>)}</tbody></table></div></div>{selectedEmployee && <><button onClick={() => setSelectedEmployeeId(null)} className="fixed inset-0 z-40 bg-[#102A43]/20" /><aside className="fixed inset-y-0 right-0 z-50 w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl"><button onClick={() => setSelectedEmployeeId(null)} className="float-right text-[#71869A]">Đóng</button><h2 className="font-display text-2xl font-extrabold text-[#102A43]">{selectedEmployee.name || "Nhân viên"}</h2><p className="mt-1 text-sm text-[#71869A]">{selectedEmployee.email || "Chưa có email"}</p><div className="mt-5 border-t pt-4"><h3 className="font-bold text-[#193B57]">Lịch sử tài sản</h3>{assetHistory.data?.map((item) => <div key={item.id} className="mt-3 rounded-lg border border-[#E7EEF3] p-3 text-sm"><div className="font-bold text-[#193B57]">{item.assetCode} · {item.assetName}</div><div className="mt-1 text-xs text-[#71869A]">{item.status === "returned" ? `Đã hoàn trả · ${item.returnedAt ? new Date(item.returnedAt).toLocaleDateString("vi-VN") : "—"}` : `Đang cấp phát · ${new Date(item.handedOverAt).toLocaleDateString("vi-VN")}`}</div></div>)}{!assetHistory.data?.length && <p className="mt-3 text-sm text-[#8AA0B6]">Chưa có lịch sử tài sản.</p>}</div></aside></>}</div>;
}
...
function AssignmentsPage({ showComingSoon, companyInfo }: { showComingSoon: (label: string) => void; companyInfo: CompanyInfo }) {
  ...
  const createHandoverMutation = trpc.handovers.create.useMutation({ onSuccess: () => handoversQuery.refetch() });
  const [query, setQuery] = useState("");
  ...
  const createHandover = () => { const asset = assignmentAssetsQuery.data?.find((item) => item.assetCode === form.assetCode); if (!form.recipient.trim() || !form.department.trim() || !asset) { toast.error("Vui lòng chọn tài sản có trong hệ thống, người nhận và phòng ban."); return; } createHandoverMutation.mutate({ assetId: asset.id, recipientName: form.recipient, recipientDepartmentName: form.department, handedOverAt: Date.now(), dueBackAt: null, conditionOut: form.condition, accessories: form.note || null, note: form.note || null, recipientUserId: null, recipientDepartmentId: null }); setModal(null); toast.success("Đã gửi phiếu bàn giao nháp để lưu vào hệ thống."); };
  const openCreate = () => { const firstAsset = assignmentAssetsQuery.data?.[0]; if (!firstAsset) { toast.error("Hãy tạo ít nhất một tài sản trước khi lập phiếu bàn giao."); return; } setForm({ id: "", assetCode: firstAsset.assetCode, assetName: firstAsset.name, recipient: "", department: "", date: new Date().toLocaleDateString("vi-VN"), status: "Nháp", condition: "Tốt", handoverBy: "Minh Anh", note: "" }); setModal("create"); };
