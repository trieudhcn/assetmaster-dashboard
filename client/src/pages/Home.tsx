...
import { EntraSettingsPanel } from "@/components/EntraSettingsPanel";
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] text-[#102A43] antialiased">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[264px] flex-col overflow-hidden border-r border-[#DDE7F0] bg-[#102A43] px-4 py-5 shadow-[8px_0_30px_rgba(16,42,67,0.16)] transition-transform duration-200 lg:translate-x-0 ${mobileNavOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="mb-3 shrink-0 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-[#7FA0B8]">Không gian làm việc</div>
      ...
      <main className="min-h-screen lg:pl-[264px]" data-system-settings-layout={activeNav === "Cài đặt" ? "true" : undefined}>
        <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between gap-4 border-b border-[#DDE7F0] bg-[#FFFFFF]/95 px-4 shadow-[0_5px_20px_rgba(16,42,67,0.03)] backdrop-blur-xl sm:px-6 lg:px-9">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="hidden items-center gap-2 text-sm text-[#8AA0B6] sm:flex"><span>Không gian làm việc</span><span className="text-[#C2D0DC]">/</span><span className="font-semibold text-[#193B57]">{activeNav}</span></div>
          </div>
...
function AssignmentsPage({ showComingSoon, companyInfo }: { showComingSoon: (label: string) => void; companyInfo: CompanyInfo }) {
  ...
  const openCreateForAsset = (asset: { assetCode: string; name: string; status: string }) => {
    if (asset.status !== "available") {
      toast.error(`Không thể bàn giao ${asset.assetCode}: tài sản không ở trạng thái Sẵn có.`);
      return;
    }
    setForm({ id: 0, referenceCode: "", assetCode: asset.assetCode, assetName: asset.name, recipient: "", department: "", date: new Date().toLocaleDateString("vi-VN"), status: "Nháp", condition: "Tốt", handoverBy: "", note: "", accessories: "", recipientUserId: null, recipientDepartmentId: null });
    setSupplyItems([]);
    setModal("create");
  };
  const openCreate = () => {
...
function AssetModal({ mode, asset, formData, setFormData, isSaving, onClose: dismiss, onSave, onEdit, onStartHandover, onReturnToInvoice }: { mode: "create" | "edit" | "detail"; asset: Asset | null; formData: Asset; setFormData: React.Dispatch<React.SetStateAction<Asset>>; isSaving: boolean; onClose: () => void; onSave: (attachments?: AssetSaveAttachments) => void; onEdit: () => void; onStartHandover: (assetCode: string) => void; onReturnToInvoice?: () => void }) {
  ...
  return <div data-asset-modal onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className="fixed inset-0 z-[60] flex items-center justify-center bg-[#102A43]/40 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={title}>
    <div className={`max-h-[92vh] w-full overflow-y-auto rounded-2xl border border-[#DDE7F0] bg-white shadow-[0_24px_70px_rgba(16,42,67,0.22)] ${isDetail ? "max-w-[560px]" : "max-w-[720px]"}`}>
      <div className="assetmaster-modal-header shrink-0 relative border-b border-[#E7EEF3] px-6 pt-5 pb-6"><div className="min-w-0 pr-14"><div className="mb-1 text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#0F8C8C]">Asset catalog</div><h2 className="font-display text-xl font-extrabold tracking-[-0.03em] text-[#102A43]">{title}</h2><p className="mt-1 max-w-full truncate whitespace-nowrap text-xs leading-5 text-[#8AA0B6]" title={isDetail ? "Thông tin định danh và vòng đời của tài sản." : "Cập nhật dữ liệu để hệ thống luôn chính xác."}>{isDetail ? "Thông tin định danh và vòng đời của tài sản." : "Cập nhật dữ liệu để hệ thống luôn chính xác."}</p>{isDetail && <div className="mt-2 inline-flex max-w-full items-center gap-1.5 rounded-md bg-[#F5F9FB] px-2 py-1 text-[10px] font-semibold text-[#71869A]" title="Thời điểm bản ghi tài sản được cập nhật gần nhất"><Clock3 size={12} className="shrink-0 text-[#0F8C8C]" aria-hidden="true" /><span className="truncate">Lần cập nhật gần nhất: {persistedAsset?.updatedAt ? new Date(persistedAsset.updatedAt).toLocaleString("vi-VN", { dateStyle: "short", timeStyle: "short" }) : assetsQuery.isLoading ? "Đang tải..." : "Chưa ghi nhận"}</span></div>}</div><button onClick={onClose} className="absolute right-6 top-5 shrink-0 rounded-lg p-2 text-[#8AA0B6] hover:bg-[#F0F5F8]" aria-label="Đóng"><X size={18} /></button></div>
...
function HandoverQrScannerModal({ assets, onClose, onSelect }: { assets: HandoverQrAsset[]; onClose: () => void; onSelect: (asset: HandoverQrAsset) => void }) {
  const [input, setInput] = useState("");
  const [message, setMessage] = useState("");
  const [matchedAsset, setMatchedAsset] = useState<HandoverQrAsset | null>(null);

  const selectScannedAsset = (rawValue: string) => {
    ...
    setMessage("");
    toast.success(`Đã kiểm tra ${asset.assetCode}: tài sản sẵn có.`);
    onSelect(asset);
  };
  ...
  return <div onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className="fixed inset-0 z-[80] flex items-center justify-center bg-[#102A43]/50 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Quét QR để bàn giao">
    <div className="max-h-[calc(100dvh-32px)] w-full max-w-lg overflow-y-auto rounded-2xl border border-[#DDE7F0] bg-white p-6 shadow-[0_24px_70px_rgba(16,42,67,0.24)]"><div className="flex items-start justify-between"><div><div className="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#0F8C8C]"><QrCode size={14} />Bàn giao bằng QR</div><h2 className="mt-2 font-display text-xl font-extrabold text-[#102A43]">Quét tài sản cần cấp phát</h2><p className="mt-1 text-xs leading-5 text-[#71869A]">Hệ thống sẽ kiểm tra trạng thái trước khi mở phiếu và điền sẵn đúng tài sản.</p></div><button onClick={onClose} className="rounded-lg p-2 text-[#8AA0B6] hover:bg-[#F0F5F8]" aria-label="Đóng"><X size={18} /></button></div><div className="mt-5"><AssetQrCapture value={input} onValueChange={(value) => { setInput(value); setMessage(""); setMatchedAsset(null); }} onScan={selectScannedAsset} submitLabel="Kiểm tra & thêm vào phiếu" /></div>{message && <p role="alert" className="mt-3 rounded-lg border border-[#F2D596] bg-[#FFF9EB] px-3 py-2 text-xs font-semibold leading-5 text-[#A86B00]">{message}</p>}{matchedAsset && matchedAsset.status !== "available" && <div className="mt-3 rounded-xl border border-[#F3C4C4] bg-[#FFF8F8] p-4"><div className="font-mono text-xs font-extrabold text-[#B44545]">{matchedAsset.assetCode}</div><div className="mt-1 text-sm font-bold text-[#193B57]">{matchedAsset.name}</div></div>}<div className="mt-5 flex justify-end border-t border-[#E7EEF3] pt-4"><button onClick={onClose} className="modal-close-action">Đóng</button></div></div>
  </div>;
}
