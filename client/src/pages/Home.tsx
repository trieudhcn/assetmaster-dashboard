      ...
      <main className="min-h-screen lg:pl-[264px]">
        ...
        <div className={`relative overflow-hidden px-4 py-7 sm:px-6 lg:px-9 lg:py-8 ${activeNav === "Tổng quan" ? "" : "hidden"}`}>
          ...
          <div className="relative mx-auto max-w-[1500px]"><div className="mb-5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.15em] text-[#A86B00]"><span className="h-px w-8 bg-[#F0A516]" /><span className="h-1.5 w-1.5 rounded-full bg-[#F0A516]" />System pulse · live inventory signal</div>
            <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.16em] text-[#0F8C8C]"><span className="h-1.5 w-1.5 rounded-full bg-[#F0A516] shadow-[0_0_0_4px_rgba(240,165,22,0.12)]" />Asset Operations</div><h1 className="font-display text-[28px] font-extrabold tracking-[-0.045em] text-[#102A43] sm:text-[34px]">Tổng quan tài sản</h1><p className="mt-1.5 text-sm text-[#71869A]">Theo dõi, quản lý và tối ưu toàn bộ tài sản doanh nghiệp.</p></div><div data-dashboard-freshness className="flex items-center justify-between gap-3 rounded-xl border border-[#DCE9ED] bg-white/90 px-3 py-2.5 shadow-[0_8px_20px_rgba(16,42,67,0.045)] backdrop-blur-sm sm:min-w-[285px]"><div className="flex min-w-0 items-center gap-2.5"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#E6F6F2] text-[#087A6A]"><CalendarDays size={15} /></span><div className="min-w-0"><div className="text-[10px] font-extrabold uppercase tracking-[.12em] text-[#7890A5]">Trạng thái dữ liệu</div><div className="mt-0.5 truncate text-xs font-bold text-[#193B57]">Đồng bộ lúc {dashboardSyncLabel}</div></div></div><button type="button" onClick={() => void refreshDashboardData()} disabled={assetQuery.isFetching || maintenanceTicketsQuery.isFetching} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-[#CDE5E5] text-[#087A6A] transition hover:bg-[#E6F6F2] disabled:cursor-not-allowed disabled:opacity-55" aria-label="Đồng bộ lại dữ liệu dashboard" title="Đồng bộ lại dữ liệu"><ArrowDownUp size={15} className={assetQuery.isFetching || maintenanceTicketsQuery.isFetching ? "animate-spin" : ""} /></button></div></div>
