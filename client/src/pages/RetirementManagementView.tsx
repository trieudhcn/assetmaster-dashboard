...
import { ArchiveRestore, Download, FileBarChart as FileBarChart3, FileCheck2, FileText, Landmark } from "lucide-react";
...
export function RetirementManagementView() {
  ...
  const yearlySummary = useMemo(() => {
    const totals = new Map<string, { year: string; assetCount: number; purchaseValue: number; certificateCount: number }>();
    certificateGroups.forEach((group) => {
      const yearLabel = group.retiredAt ? String(new Date(group.retiredAt).getFullYear()) : "Chưa ghi nhận";
      const current = totals.get(yearLabel) || { year: yearLabel, assetCount: 0, purchaseValue: 0, certificateCount: 0 };
      current.assetCount += group.assets.length;
      current.purchaseValue += group.assets.reduce((total, asset) => total + Number(asset.purchaseValue || 0), 0);
  ...
  return <main className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    <section className="rounded-xl border border-[#E7D9B9] bg-white p-4 shadow-[0_8px_24px_rgba(16,42,67,0.045)] sm:p-5"><div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-extrabold text-[#8F5A00]"><Landmark size={17} />Tổng quan thanh lý</div><p className="mt-1 text-xs text-[#71869A]">Theo dõi giá trị nguyên giá đã chuyển trạng thái Khấu hao/Thanh lý theo năm.</p></div><div className="w-full sm:w-52"><label className="mb-1 block text-[10px] font-extrabold uppercase tracking-[.1em] text-[#8A7140]">Năm thanh lý</label><SearchableSelect value={year} onChange={setYear} options={[{ value: "all", label: "Tất cả năm" }, ...yearOptions.map((item) => ({ value: String(item), label: `Năm ${item}` }))]} placeholder="Tất cả năm" searchPlaceholder="Tìm năm thanh lý..." /></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><OverviewMetric label="Biên bản trong phạm vi" value={certificateGroups.length} icon={FileText} tone="amber" /><OverviewMetric label="Tài sản đã thanh lý" value={retiredAssets.length} icon={ArchiveRestore} tone="blue" /><OverviewMetric label="Tổng nguyên giá" value={`${formatVnd(totalPurchaseValue)} VNĐ`} icon={Landmark} tone="navy" /><OverviewMetric label="Đã chọn xuất PDF" value={selectedAssets.length} icon={FileCheck2} tone="teal" /></div>{yearlySummary.length > 0 && <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{yearlySummary.map((item) => <div key={item.year} className="rounded-lg border border-[#F0DFC0] bg-[#FFFDF7] px-3 py-2.5"><div className="flex items-center justify-between gap-3"><span className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#A86B00]">Năm {item.year}</span><span className="text-[10px] font-bold text-[#8A7140]">{item.certificateCount} biên bản</span></div><div className="mt-1.5 text-sm font-extrabold text-[#193B57]">{formatVnd(item.purchaseValue)} VNĐ</div><div className="mt-1 text-[10px] text-[#71869A]">{item.assetCount} tài sản</div></div>)}</div>}</section>
