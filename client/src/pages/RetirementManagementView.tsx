...
import { Download, FileBarChart as FileBarChart3, Landmark } from "lucide-react";
...
export function RetirementManagementView() {
  ...
  const retirementServiceCostSummary = useMemo(() => retiredAssets.reduce((summary, asset) => {
    ...
    return summary;
  }, { yearLabel: year === "all" ? "Tất cả năm" : `Năm ${year}`, warrantyCost: 0, repairCost: 0 }), [retiredAssets, serviceCostByAssetId, year]);
  ...
  return <main className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    <section className="rounded-xl border border-[#E7D9B9] bg-white p-4 shadow-[0_8px_24px_rgba(16,42,67,0.045)] sm:p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><OverviewMetric label="Tổng nguyên giá" value={`${formatVnd(totalPurchaseValue)} VNĐ`} icon={Landmark} tone="navy" /><RecordedSalvageMetric summary={recordedSalvageSummary} empty={!retiredAssets.length} /><RetirementServiceCostMetric summary={retirementServiceCostSummary} empty={!retiredAssets.length} /></div>
    </section>
