...
export function RetirementManagementView() {
  ...
  return <main className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    <section className="rounded-xl border border-[#E7D9B9] bg-white p-4 shadow-[0_8px_24px_rgba(16,42,67,0.045)] sm:p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><OverviewMetric label="Biên bản trong phạm vi" value={certificateGroups.length} icon={FileText} tone="amber" /><OverviewMetric label="Tài sản đã thanh lý" value={retiredAssets.length} icon={ArchiveRestore} tone="blue" /><OverviewMetric label="Tổng nguyên giá" value={`${formatVnd(totalPurchaseValue)} VNĐ`} icon={Landmark} tone="navy" /></div>
      {yearlySummary.length > 0 && <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4" data-retirement-recorded-salvage>{yearlySummary.map((item) => <div key={item.year} className="rounded-lg border border-[#F0DFC0] bg-[#FFFDF7] px-3 py-2.5"><div className="flex items-center justify-between gap-3"><span className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#A86B00]">Năm {item.year}</span><span className="text-[10px] font-bold text-[#8A7140]">{item.certificateCount} biên bản</span></div><div className="mt-1.5 text-sm font-extrabold text-[#193B57]">{formatVnd(item.salvageValue)} VNĐ</div><div className="mt-1 text-[10px] text-[#71869A]">Giá trị thanh lý đã ghi nhận · {item.assetCount} tài sản</div></div>)}</div>}
    </section>
