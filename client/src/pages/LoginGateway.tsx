...
import { Building2, ChevronRight, CircleAlert, Cloud, KeyRound, Loader2, LockKeyhole, ShieldCheck, Wrench } from "lucide-react";
...
function entraLoginIssue(code: string): LoginIssue {
  if (code === "cancelled")
    return {
      title: "Đăng nhập Microsoft đã được hủy",
      description: "Microsoft Entra ID không trả về phiên đăng nhập.",
      steps: ["Chọn lại Đăng nhập bằng Microsoft.", "Hoàn tất xác thực và MFA nếu được yêu cầu."],
    };
  if (code === "configuration")
    return {
      title: "Microsoft Entra ID chưa sẵn sàng",
      description: "Cấu hình Entra ID trên máy chủ chưa đầy đủ hoặc secret không đọc được.",
      steps: ["Kiểm tra Tenant ID, Client ID và Redirect URI.", "Kiểm tra Client Secret rồi khởi động lại ứng dụng.", "Liên hệ quản trị viên Entra ID nếu lỗi vẫn còn."],
    };
  return {
    title: "Không thể đăng nhập bằng Microsoft",
    description: "Phản hồi Entra ID không hợp lệ hoặc tài khoản chưa được cấp quyền AssetMaster.",
    steps: ["Kiểm tra tài khoản đã được gán App Role AssetMaster.", "Thử lại sau khi quản trị viên cấp quyền.", "Dùng LDAPS hoặc Admin cục bộ nếu cần truy cập dự phòng."],
  };
}
...
export function LoginGateway({ onLogin }: { onLogin: () => void }) {
  ...
  const entraEnabled = directoryQuery.data?.entraEnabled === true;
  const directoryEnabled = directoryQuery.data?.enabled === true;
  const isSubmitting = localLogin.isPending || directoryLogin.isPending;

  useEffect(() => {
    const errorCode = new URLSearchParams(window.location.search).get("entra_error");
    if (!errorCode) return;
    const nextIssue = entraLoginIssue(errorCode);
    setIssue(nextIssue);
