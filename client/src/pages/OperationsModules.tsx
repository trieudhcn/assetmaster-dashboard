      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        <section className={`${card} p-5`}>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <CurrencyInput value={estimatedCost} onChange={setEstimatedCost} placeholder="Chi phí dự kiến" aria-label="Chi phí dự kiến" />
                  ...
                  {!ticketsQuery.isLoading && tickets.map((ticket) => {
                    ...
                    return (
                      <tr key={ticket.id} className="border-t border-[#EDF2F5] align-top">
                        ...
                        <td className="px-4 py-4">
                          <div className="space-y-2"><label className="block text-[10px] font-bold text-[#8AA0B6]">Dự kiến<CurrencyInput disabled={!isAdmin || updateMutation.isPending} value={draft.estimatedCost} onChange={(value) => updateDraft(ticket, { estimatedCost: value })} placeholder="0" aria-label="Chi phí dự kiến" className="mt-1 h-8 w-28 text-xs" /></label><label className="block text-[10px] font-bold text-[#8AA0B6]">Thực tế<CurrencyInput disabled={!isAdmin || updateMutation.isPending} value={draft.actualCost} onChange={(value) => updateDraft(ticket, { actualCost: value })} placeholder="0" aria-label="Chi phí thực tế" className="mt-1 h-8 w-28 text-xs" /></label></div>
                        </td>
                        <td className="px-4 py-4">
