...
import { AssetQrCapture } from "@/components/AssetQrCapture";
...
function OperationalReminderPanel({ onCreateWarrantyTicket }: { onCreateWarrantyTicket?: (assetId: number) => void }) {
  ...
  const reminders = (remindersQuery.data || []).filter((reminder) => !(reminder.kind === "audit" && reminder.isOverdue));
  const openAuditSession = (sessionId: number) => window.dispatchEvent(new CustomEvent("assetmaster:open-audit-session", { detail: { sessionId } }));
  return <section className={`mb-5 ${card} overflow-hidden`}>
    <div className="flex items-center justify-between gap-3 border-b border-[#E7EEF3] px-4 py-3"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-extrabold text-[#193B57]"><BellRing size={16} className="text-[#A86B00]" />Nhắc việc vận hành</div><p className="mt-0.5 text-[11px] text-[#71869A]">Hạn xử lý và các đợt Kiểm kê sắp đến.</p></div><span className="shrink-0 rounded-full bg-[#FFF9EB] px-2.5 py-1 text-[10px] font-extrabold text-[#A86B00]">{reminders.length} việc</span></div>
    {remindersQuery.isLoading ? <div className="px-4 py-4 text-xs text-[#71869A]">Đang tải nhắc việc...</div> : remindersQuery.isError ? <div className="px-4 py-4 text-xs text-[#B44545]">Không thể tải nhắc việc. <button onClick={() => remindersQuery.refetch()} className="font-bold underline">Thử lại</button></div> : reminders.length === 0 ? <div className="px-4 py-4 text-xs text-[#71869A]">Chưa có việc vận hành cần theo dõi.</div> : <div className="divide-y divide-[#EDF2F5]">{reminders.slice(0, 3).map((reminder) => { const isWarrantyExpiry = reminder.kind === "warranty"; const canCreateWarrantyTicket = Boolean(onCreateWarrantyTicket) && isWarrantyExpiry && typeof reminder.assetId === "number"; return <div key={reminder.id} className={`flex flex-col gap-2 px-4 py-2.5 sm:flex-row sm:items-center sm:justify-between ${isWarrantyExpiry ? "bg-[#FFF9EB]" : ""}`}><div className="flex min-w-0 items-start gap-2"><CalendarClock size={15} className={`mt-0.5 shrink-0 ${isWarrantyExpiry ? "text-[#A86B00]" : "text-[#0F8C8C]"}`} /><div className="min-w-0"><div className="truncate text-xs font-bold text-[#193B57]">{reminder.title}</div><div className="mt-0.5 truncate text-[11px] text-[#71869A]">{isWarrantyExpiry ? "Bảo hành sắp hết hạn" : reminder.kind === "maintenance" ? "Bảo hành/Sửa chữa" : "Kiểm kê"} · {reminder.detail}</div></div></div><div className="flex shrink-0 items-center gap-2"><span className={`w-fit rounded-full px-2 py-1 text-[10px] font-extrabold ${isWarrantyExpiry ? "bg-[#FFE7A4] text-[#8A5900]" : "bg-[#FFF9EB] text-[#A86B00]"}`}>{isWarrantyExpiry ? `Còn ${Math.max(0, Math.ceil((new Date(reminder.dueAt).getTime() - Date.now()) / 86_400_000))} ngày` : `Hạn ${new Date(reminder.dueAt).toLocaleDateString("vi-VN")}`}</span>{reminder.kind === "audit" && typeof reminder.auditSessionId === "number" && <button type="button" onClick={() => openAuditSession(reminder.auditSessionId)} className="rounded-md border border-[#CDE5E5] bg-white px-2.5 py-1.5 text-[10px] font-extrabold text-[#087A6A] transition hover:bg-[#ECF8F7]">Mở đợt</button>}{canCreateWarrantyTicket && <button type="button" onClick={() => onCreateWarrantyTicket?.(reminder.assetId!)} className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg bg-[#0F8C8C] px-3 text-[10px] font-extrabold text-white shadow-[0_4px_10px_rgba(15,140,140,0.2)] transition hover:bg-[#087A6A]"><Plus size={13} />Tạo phiếu</button>}</div></div>; })}</div>}
  </section>;
}
          ...
          {ticketsQuery.isError ? (
            ...
            <>
              <div className="space-y-3 p-3 md:hidden">
                {ticketsQuery.isLoading && <div className="rounded-xl border border-[#E7EEF3] bg-[#FBFCFE] px-4 py-8 text-center text-xs font-semibold text-[#8AA0B6]">Đang tải phiếu Bảo hành/Sửa chữa...</div>}
                {!ticketsQuery.isLoading && pagedTickets.map((ticket) => {
                  ...
                  return <article key={`mobile-${ticket.id}`} className={`overflow-hidden rounded-xl border border-[#E1EAF1] bg-white shadow-[0_4px_14px_rgba(16,42,67,0.04)] ${isClosed ? "bg-[#FBFCFD]" : ""}`}>
                    <div className="p-3.5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="font-mono text-[11px] font-extrabold text-[#0F8C8C]">{ticket.ticketCode}</div><div className="mt-1 flex items-start gap-1.5 text-sm font-extrabold leading-5 text-[#193B57]"><Wrench size={14} className="mt-0.5 shrink-0 text-[#A86B00]" />{asset?.name || `Tài sản #${ticket.assetId}`}</div></div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-extrabold ${draft.status === "closed" ? "bg-[#F0F5F8] text-[#60758A]" : draft.status === "resolved" ? "bg-[#ECF8F7] text-[#087A6A]" : "bg-[#FFF5DC] text-[#A86B00]"}`}>{maintenanceStatusLabels[draft.status]}</span></div><p className="mt-2 line-clamp-2 text-[11px] leading-5 text-[#71869A]">{ticket.description || "Chưa có mô tả yêu cầu."}</p><div className="mt-3 flex flex-wrap gap-2"><span className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[10px] font-extrabold ${ticket.serviceChannel === "warranty" ? "border-[#8BCDC6] bg-[#ECF8F7] text-[#087A6A]" : "border-[#F2D596] bg-[#FFF9EB] text-[#A86B00]"}`}><LockKeyhole size={11} />{serviceChannelLabels[(ticket.serviceChannel || "repair") as keyof typeof serviceChannelLabels]}</span><span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${priorityTone}`}>{priorityLabels[ticket.priority]}</span></div><div className="mt-3 grid grid-cols-2 gap-2 rounded-lg bg-[#F7FAFC] p-2.5 text-[10px]"><div><div className="text-[#8AA0B6]">Người xử lý</div><div className="mt-0.5 truncate font-bold text-[#193B57]">{assignee?.name || assignee?.email || "Chưa phân công"}</div></div><div><div className="text-[#8AA0B6]">Chi phí thực tế</div><div className="mt-0.5 font-extrabold text-[#193B57]">{actualCostLabel}</div></div></div></div><div className="flex flex-wrap gap-2 border-t border-[#E7EEF3] bg-[#FBFCFD] p-3"><button type="button" onClick={() => setHistoryTicket(ticket)} className="inline-flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg border border-[#CDE5E5] bg-white px-3 text-[10px] font-extrabold text-[#087A6A]"><History size={13} />Lịch sử</button>{ticket.serviceChannel === "repair" && <button type="button" disabled={repairPdfTicketId === ticket.id} onClick={() => void previewRepairTicketPdf(ticket, asset)} className="inline-flex h-9 items-center justify-center rounded-lg border border-[#C7DDF8] bg-[#EFF7FF] px-3 text-[#2666A8] disabled:opacity-60" aria-label={`Xem trước PDF phiếu Sửa chữa ${ticket.ticketCode}`}><Printer size={14} /></button>}<button type="button" onClick={() => setExpandedTicketId((current) => current === ticket.id ? null : ticket.id)} className="inline-flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg bg-[#0F8C8C] px-3 text-[10px] font-extrabold text-white"><span>{isExpanded ? "Thu gọn" : "Cập nhật"}</span><ChevronDown size={13} className={isExpanded ? "rotate-180 transition-transform" : "transition-transform"} /></button></div>{isExpanded && <div className="space-y-3 border-t border-[#DCEDEF] bg-[#F7FBFB] p-3.5"><div><label className="field-label">Người xử lý</label><SearchableSelect value={draft.assigneeUserId} onChange={(value) => updateDraft(ticket, { assigneeUserId: value })} disabled={!canEditTicket || updateMutation.isPending || employeesQuery.isLoading} placeholder="Chưa phân công" searchPlaceholder="Tìm người xử lý..." options={[{ value: "", label: "Chưa phân công" }, ...employees.map((employee) => ({ value: String(employee.id), label: `${employee.name || employee.email || `Nhân viên #${employee.id}`}${employee.isActive ? "" : " · Đã khóa"}`, searchText: employee.email || "" }))]} /></div><div><label className="field-label">Trạng thái</label><SearchableSelect value={draft.status} onChange={(value) => updateDraft(ticket, { status: value as TicketDraft["status"] })} disabled={!canEditTicket || updateMutation.isPending} searchPlaceholder="Tìm trạng thái..." options={Object.entries(maintenanceStatusLabels).map(([value, label]) => ({ value, label }))} /></div><div className="grid grid-cols-2 gap-3"><label className="field-label">Dự kiến<CurrencyInput disabled={!canEditTicket || updateMutation.isPending} value={draft.estimatedCost} onChange={(value) => updateDraft(ticket, { estimatedCost: value })} placeholder={canEditTicket ? "0" : "Chưa nhập"} aria-label="Chi phí dự kiến" showWords className="mt-1 h-9 !w-full text-xs" /></label><label className="field-label">Thực tế<CurrencyInput disabled={!canEditTicket || updateMutation.isPending} value={draft.actualCost} onChange={(value) => updateDraft(ticket, { actualCost: value })} placeholder={canEditTicket ? "0" : "Chưa nhập"} aria-label="Chi phí thực tế" showWords className="mt-1 h-9 !w-full text-xs" /></label></div><div><label className="field-label">Kết quả xử lý</label><textarea disabled={!canEditTicket || updateMutation.isPending} value={draft.resolution} onChange={(event) => updateDraft(ticket, { resolution: event.target.value })} placeholder="Nhập kết quả hoặc hướng xử lý..." className="mt-1 min-h-[74px] w-full resize-y rounded-md border border-[#DDE7F0] p-2 text-xs leading-5 text-[#193B57] outline-none focus:border-[#0F8C8C] disabled:cursor-not-allowed disabled:opacity-60" /></div><div className="flex flex-col gap-2"><div>{ticket.attachmentUrl ? <a href={ticket.attachmentUrl} target="_blank" rel="noreferrer" className="text-[10px] font-bold text-[#087A6A] underline" title={ticket.attachmentName || "Mở chứng từ"}>{ticket.attachmentName || "Mở chứng từ"}</a> : <span className="text-[10px] text-[#8AA0B6]">Chưa có chứng từ</span>}</div>{isAdmin && <label className={`inline-flex h-9 items-center justify-center rounded-md border border-[#CDE5E5] px-3 text-[10px] font-bold ${canEditTicket ? "cursor-pointer text-[#087A6A] hover:bg-[#ECF8F7]" : "cursor-not-allowed text-[#8AA0B6] opacity-70"}`}><input type="file" accept="application/pdf,image/png,image/jpeg,image/webp" className="sr-only" disabled={!canEditTicket || uploadAttachmentMutation.isPending} onChange={(event) => { uploadAttachment(ticket, event.target.files?.[0]); event.currentTarget.value = ""; }} />{uploadAttachmentMutation.isPending ? "Đang tải" : isClosed ? "Phiếu đã đóng" : "Tải chứng từ"}</label>}<button disabled={!canEditTicket || updateMutation.isPending} onClick={() => saveTicket(ticket)} className="inline-flex h-10 items-center justify-center gap-1.5 rounded-md bg-[#0F8C8C] px-3 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-60"><Save size={13} />{updateMutation.isPending ? "Đang lưu" : isClosed ? "Đã đóng" : "Lưu cập nhật"}</button></div></div>}</article>;
                })}
              ...
              <div className="hidden md:block mobile-table-scroll overflow-x-auto">
                    ...
                    return (
                      <Fragment key={ticket.id}>
                        <tr className={`border-t border-[#EDF2F5] align-middle transition hover:bg-[#FAFDFD] ${isClosed ? "bg-[#FBFCFD]" : ""}`}>
                          ...
                          <td className="px-5 py-4 text-right"><div className="flex justify-end gap-1.5"><button type="button" onClick={() => setHistoryTicket(ticket)} className="rounded-md border border-[#CDE5E5] p-2 text-[#087A6A] transition hover:bg-[#ECF8F7]" aria-label={`Xem lịch sử ${ticket.ticketCode}`} title="Xem lịch sử"><History size={13} /></button><button type="button" disabled={repairPdfTicketId === ticket.id} onClick={() => void previewRepairTicketPdf(ticket, asset)} className="rounded-md border border-[#C7DDF8] bg-[#EFF7FF] p-2 text-[#2666A8] transition hover:bg-[#EAF3FF] disabled:cursor-wait disabled:opacity-60" aria-label={`Xem trước PDF phiếu ${serviceChannelLabels[(ticket.serviceChannel || "repair") as keyof typeof serviceChannelLabels]} ${ticket.ticketCode}`} title="PDF / In"><Printer size={13} /></button><button type="button" onClick={() => setExpandedTicketId((current) => current === ticket.id ? null : ticket.id)} className="inline-flex items-center gap-1 rounded-md bg-[#0F8C8C] px-2.5 py-2 text-[10px] font-extrabold text-white transition hover:bg-[#087A6A]" aria-expanded={isExpanded}><span>{isExpanded ? "Thu gọn" : "Cập nhật"}</span><ChevronDown size={13} className={isExpanded ? "rotate-180 transition-transform" : "transition-transform"} /></button></div></td>
                        </tr>
...
export function AuditPage() {
  ...
  const [isExportingFilteredAudit, setIsExportingFilteredAudit] = useState(false);
  ...
  const [auditKeyboardMode, setAuditKeyboardMode] = useState(false);
  const [isAuditFullscreen, setIsAuditFullscreen] = useState(false);
  const [auditDeviceMode, setAuditDeviceMode] = useState<"desktop" | "tablet" | "scanner">("desktop");
  ...
  const auditNextFocusIdRef = useRef<number | null>(null);
  ...
  const recordItemMutation = trpc.audits.recordItem.useMutation({
    onSuccess: async (_, variables) => {
      const nextFocusId = auditNextFocusIdRef.current;
      auditNextFocusIdRef.current = null;
      ...
      if (nextFocusId !== null) {
        window.requestAnimationFrame(() => {
          const nextRow = document.querySelector<HTMLTableRowElement>(`[data-audit-item-id="${nextFocusId}"]`)
            || document.querySelector<HTMLTableRowElement>('[data-audit-quick-row="true"]');
          nextRow?.focus();
        });
      }
    ...
    onError: (error) => {
      auditNextFocusIdRef.current = null;
      toast.error(error.message || "Không thể lưu kết quả kiểm kê.");
    },
  });
  ...
  const matchesAuditBranch = (asset: (typeof assets)[number]) => auditBranchFilter === "all" || asset.branchId === Number(auditBranchFilter);
  const selectableScopedAssets = availableAssets.filter((asset) => (exportDepartmentId === "all" || asset.departmentId === Number(exportDepartmentId)) && (exportCategoryId === "all" || asset.categoryId === Number(exportCategoryId)) && matchesAuditBranch(asset));
  ...
  const auditBranchOptions = [{ value: "all", label: "Tất cả Chi nhánh" }, ...(branchesQuery.data || []).filter((branch) => branch.isActive).map((branch) => ({ value: String(branch.id), label: `${branch.name} · ${branch.code}`, searchText: `${branch.name} ${branch.code}` }))];
  const scopedExportAssets = auditableAssets.filter((asset) => (exportDepartmentId === "all" || asset.departmentId === Number(exportDepartmentId)) && (exportCategoryId === "all" || asset.categoryId === Number(exportCategoryId)) && matchesAuditBranch(asset));
  const scopedAuditItems = auditItemsForCurrentSession.filter((item) => {
    const asset = assetById.get(item.assetId);
    if (!asset) return false;
    return (exportDepartmentId === "all" || asset.departmentId === Number(exportDepartmentId)) && (exportCategoryId === "all" || asset.categoryId === Number(exportCategoryId)) && matchesAuditBranch(asset);
  });
  ...
  const filteredAuditItems = scopedAuditItems.filter((item) => {
    ...
    return matchesSearch && matchesActualStatus && matchesResult && (asset ? matchesAuditBranch(asset) : false);
  });
  ...
  const auditPriority = (item: (typeof auditItems)[number]) => item.result === "missing" || item.actualStatus === "lost" ? 0 : item.result === "mismatch" ? 1 : 2;
  const prioritizedAuditItems = filteredAuditItems
    .map((item, originalIndex) => ({ item, originalIndex }))
    .sort((left, right) => auditPriority(left.item) - auditPriority(right.item) || left.originalIndex - right.originalIndex)
    .map(({ item }) => item);
  const auditItemsPageCount = Math.max(1, Math.ceil(prioritizedAuditItems.length / AUDIT_ITEMS_PAGE_SIZE));
  const activeAuditItemsPage = Math.min(auditItemsPage, auditItemsPageCount);
  const pagedAuditItems = prioritizedAuditItems.slice((activeAuditItemsPage - 1) * AUDIT_ITEMS_PAGE_SIZE, activeAuditItemsPage * AUDIT_ITEMS_PAGE_SIZE);

  useEffect(() => setAuditItemsPage(1), [selectedSessionId, auditAssetSearch, auditActualStatusFilter, auditResultFilter, exportDepartmentId, exportCategoryId, auditBranchFilter]);
  ...
  useEffect(() => {
    ...
    const exitFullscreenOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") setIsAuditFullscreen(false);
    };
    document.body.style.overflow = "hidden";
    window.addEventListener("keydown", exitFullscreenOnEscape);
    return () => {
      document.body.style.overflow = previousBodyOverflow;
      window.removeEventListener("keydown", exitFullscreenOnEscape);
    };
  }, [isAuditFullscreen]);
  ...
  const auditCompletionPercentage = summary.total > 0 ? Math.round(((summary.total - summary.pending) / summary.total) * 100) : 0;
  ...
  const scanAuditQr = async (rawValue = qrScanInput) => {
    if (!selectedAudit || addItemMutation.isPending || recordItemMutation.isPending) return;
  ...
  const filteredAuditExportRows = filteredAuditItems.map((item, index) => {
    const asset = assetById.get(item.assetId);
    const department = departmentsQuery.data?.find((entry) => entry.id === asset?.departmentId);
    return {
      "STT": index + 1,
      "Mã tài sản": asset?.assetCode || `#${item.assetId}`,
      "Tên tài sản": asset?.name || "Tài sản đã bị lưu trữ",
    ...
    window.setTimeout(async () => {
      try {
        ...
        const summarySheet = XLSX.utils.aoa_to_sheet([
          ...
          ["Phạm vi", `${exportDepartmentOptions.find((option) => option.value === exportDepartmentId)?.label || "Tất cả Phòng ban"} · ${exportCategoryOptions.find((option) => option.value === exportCategoryId)?.label || "Tất cả Phân loại"} · ${auditBranchOptions.find((option) => option.value === auditBranchFilter)?.label || "Tất cả Chi nhánh"} · ${totalAssetRows.length} tài sản`],
  ...
  const exportFilteredAuditExcel = async () => {
    if (!selectedAudit || !filteredAuditExportRows.length) { toast.info("Không có tài sản Kiểm kê phù hợp để xuất."); return; }
    setIsExportingFilteredAudit(true);
    try {
      const workbook = XLSX.utils.book_new();
      ...
      await writeBrandedWorkbook(workbook, {
        documentTitle: "DANH SÁCH KIỂM KÊ THEO BỘ LỌC",
        fileName: `assetmaster-danh-sach-kiem-ke-${selectedAudit.referenceCode}.xlsx`,
        description: `Đợt ${selectedAudit.name} · ${filteredAuditExportRows.length} tài sản · ${selectedBranchLabel}.`,
      });
      toast.success(`Đã xuất ${filteredAuditExportRows.length} tài sản theo bộ lọc hiện tại.`);
    } catch (error) {
      console.error("[AuditPage] Filtered audit Excel export failed", error);
      toast.error("Không thể xuất danh sách Kiểm kê.");
    } finally { setIsExportingFilteredAudit(false); }
  };
  ...
  return (
    <div className={shell}>
      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        {selectedAudit && <section role={isAuditFullscreen ? "dialog" : undefined} aria-modal={isAuditFullscreen ? true : undefined} aria-label={isAuditFullscreen ? `Kiểm kê toàn màn hình: ${selectedAudit.name}` : undefined} className={isAuditFullscreen ? "fixed inset-0 z-[100] m-0 h-[100dvh] overflow-y-auto bg-white" : `relative z-0 mt-5 overflow-hidden ${card}`}>
          <div className={`flex flex-col gap-4 border-b border-[#E7EEF3] p-5 lg:flex-row lg:items-end lg:justify-between ${isAuditFullscreen ? "sticky top-0 z-50 bg-white shadow-[0_8px_20px_rgba(16,42,67,0.08)]" : ""}`}>
            <div><div className="flex flex-wrap items-center gap-2"><div className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#0F8C8C]">Chi tiết kiểm kê</div>{isAuditFullscreen && <span className="rounded-full bg-[#E6F6F2] px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.1em] text-[#087A6A]">Toàn màn hình · Esc để thoát</span>}</div><h2 className="mt-1 font-display text-xl font-extrabold text-[#102A43]">{selectedAudit.name}</h2><p className="mt-1 text-xs text-[#71869A]">{selectedAudit.referenceCode} · Đối chiếu trạng thái hệ thống với thực tế kiểm kê.</p></div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              {isAuditFullscreen && <div className="col-span-2 rounded-lg border border-[#CDE5E5] bg-[#F4FBFA] px-3 py-2 sm:col-span-4"><div className="flex items-center justify-between gap-3 text-[10px] font-extrabold text-[#087A6A]"><span>Tiến độ kiểm kê</span><span>{auditCompletionPercentage}% · {summary.total - summary.pending}/{summary.total}</span></div><Progress value={auditCompletionPercentage} className="mt-2 h-2 bg-[#DDEBEA]" /></div>}
            </div>
            <div className="flex flex-wrap justify-end gap-2">{isAuditFullscreen && <div role="group" aria-label="Thiết bị kiểm kê" className="inline-flex min-h-10 overflow-hidden rounded-lg border border-[#D7E5F5] bg-white"><button type="button" aria-pressed={auditDeviceMode === "desktop"} onClick={() => { setAuditDeviceMode("desktop"); setIsQrScanOpen(false); }} className={`inline-flex items-center gap-1.5 px-2.5 text-[10px] font-extrabold transition ${auditDeviceMode === "desktop" ? "bg-[#EAF3FF] text-[#2666A8]" : "text-[#60758A] hover:bg-[#F7FAFC]"}`}><Monitor size={13} />Máy tính</button><button type="button" aria-pressed={auditDeviceMode === "tablet"} onClick={() => { setAuditDeviceMode("tablet"); setIsQrScanOpen(false); }} className={`inline-flex items-center gap-1.5 border-l border-[#D7E5F5] px-2.5 text-[10px] font-extrabold transition ${auditDeviceMode === "tablet" ? "bg-[#EAF3FF] text-[#2666A8]" : "text-[#60758A] hover:bg-[#F7FAFC]"}`}><Tablet size={13} />Máy tính bảng</button><button type="button" aria-pressed={auditDeviceMode === "scanner"} onClick={() => { setAuditDeviceMode("scanner"); setIsQrScanOpen(true); }} className={`inline-flex items-center gap-1.5 border-l border-[#D7E5F5] px-2.5 text-[10px] font-extrabold transition ${auditDeviceMode === "scanner" ? "bg-[#E6F6F2] text-[#087A6A]" : "text-[#60758A] hover:bg-[#F7FAFC]"}`}><QrCode size={13} />Máy quét</button></div>}<button type="button" aria-pressed={isAuditFullscreen} onClick={() => setIsAuditFullscreen((current) => !current)} className={`inline-flex min-h-10 items-center gap-2 rounded-lg border px-3 py-2 text-xs font-bold transition ${isAuditFullscreen ? "border-[#8BCDC6] bg-[#E6F6F2] text-[#087A6A] hover:bg-[#DDF3F0]" : "border-[#D7E5F5] bg-white text-[#2666A8] hover:bg-[#EAF3FF]"}`}>{isAuditFullscreen ? <Minimize2 size={15} /> : <Maximize2 size={15} />}{isAuditFullscreen ? "Thoát toàn màn hình" : "Kiểm kê toàn màn hình"}</button>{selectedAudit.status === "draft" && <button type="button" disabled={!isAdmin || deleteDraftAuditMutation.isPending} onClick={() => setIsDeleteDraftAuditOpen(true)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[#F3C4C4] bg-white px-3 py-2 text-xs font-bold text-[#B44545] hover:bg-[#FDEDEE] disabled:cursor-not-allowed disabled:opacity-60"><Trash2 size={15} />Xóa đợt nháp</button>}<button disabled={!isAdmin || isAuditLocked} onClick={() => setIsQrScanOpen((current) => !current)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[#CDE5E5] bg-white px-3 py-2 text-xs font-bold text-[#087A6A] transition hover:bg-[#ECF8F7] disabled:cursor-not-allowed disabled:opacity-60"><QrCode size={15} />{isQrScanOpen ? "Ẩn quét QR" : "Quét QR hàng loạt"}</button><button disabled={!filteredAuditExportRows.length || isExportingFilteredAudit || isExportingDiscrepancy !== null} onClick={() => void exportFilteredAuditExcel()} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[#D7E5F5] bg-white px-3 py-2 text-xs font-bold text-[#2666A8] transition hover:bg-[#EAF3FF] disabled:cursor-not-allowed disabled:opacity-60"><Download size={15} />{isExportingFilteredAudit ? "Đang xuất..." : "Xuất Excel"}</button><button disabled={!discrepancyRows.length || isExportingDiscrepancy !== null} onClick={exportDiscrepancyPdf} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-[#102A43] px-3 py-2 text-xs font-bold text-white transition hover:bg-[#193B57] disabled:cursor-not-allowed disabled:opacity-60"><FileText size={15} />{isExportingDiscrepancy === "pdf" ? "Đang chuẩn bị PDF..." : "Xuất PDF"}</button></div>
          </div>
          ...
          {auditItemsQuery.isError ? <div className={`m-5 rounded-xl border border-[#F2D596] bg-[#FFF9EB] p-4 text-sm ${isAuditFullscreen && auditDeviceMode === "scanner" ? "hidden" : ""}`}><div className="flex items-center gap-2 font-bold text-[#A86B00]"><AlertTriangle size={16} />Không thể tải chi tiết kiểm kê</div><p className="mt-1 text-xs text-[#71869A]">{auditItemsQuery.error.message || "Vui lòng thử lại."}</p><button onClick={() => auditItemsQuery.refetch()} className="mt-3 rounded-lg border border-[#F2D596] px-3 py-2 text-xs font-bold text-[#A86B00] hover:bg-white">Thử lại</button></div> : <div className={`mobile-table-scroll overflow-x-auto ${isAuditFullscreen && auditDeviceMode === "scanner" ? "hidden" : ""}`}><table className={`w-full table-fixed text-left text-xs ${isAuditFullscreen && auditDeviceMode === "tablet" ? "min-w-[1080px]" : "min-w-[1460px]"}`}><colgroup><col className="w-[210px]" />{!(isAuditFullscreen && auditDeviceMode === "tablet") && <><col className="w-[165px]" /><col className="w-[135px]" /></>}<col className="w-[230px]" /><col className="w-[190px]" /><col className="w-[350px]" /><col className="w-[180px]" /></colgroup><thead className="bg-[#F1F3F5] text-[10px] uppercase tracking-[.12em] text-[#526779]"><tr><th className="sticky left-0 z-30 w-[210px] border-r border-[#DDE7F0] bg-[#F1F3F5] px-5 py-3 shadow-[6px_0_10px_-8px_rgba(16,42,67,0.35)]">Tài sản</th>{!(isAuditFullscreen && auditDeviceMode === "tablet") && <><th className="px-4 py-3">Chi nhánh</th><th className="px-4 py-3">Trạng thái dự kiến</th></>}<th className="px-4 py-3">Trạng thái thực tế</th><th className="px-4 py-3">Kết quả</th><th className="px-4 py-3">Ghi chú / lịch sử</th><th className="sticky right-0 z-20 w-[180px] border-l border-[#DDE7F0] bg-[#F1F3F5] px-5 py-3 text-right">Lưu</th></tr></thead><tbody>{auditItemsQuery.isLoading && <tr><td colSpan={isAuditFullscreen && auditDeviceMode === "tablet" ? 5 : 7}><ModalTableSkeleton rows={5} columns={isAuditFullscreen && auditDeviceMode === "tablet" ? 5 : 7} /></td></tr>}{!auditItemsQuery.isLoading && pagedAuditItems.map((item) => { const asset = assetById.get(item.assetId); const draft = draftFor(item); const isDiscrepancy = draft.result === "missing" || draft.result === "mismatch"; const badgeTone = draft.result === "matched" ? "bg-[#E6F6F2] text-[#087A6A]" : isDiscrepancy ? "bg-[#FDEDEE] text-[#B44545]" : "bg-[#FFF5DC] text-[#A86B00]"; return <tr key={item.id} data-audit-item-id={item.id} data-audit-quick-row={auditKeyboardMode && isAdmin ? "true" : undefined} tabIndex={auditKeyboardMode && isAdmin ? 0 : -1} title={auditKeyboardMode && isAdmin ? "Phím 1: Khớp · 2: Chênh lệch · 3: Không tìm thấy · Enter: Lưu · ↑/↓: Chuyển dòng" : undefined} onClick={(event) => { if (auditKeyboardMode && isAdmin && !(event.target as HTMLElement).closest("input, textarea, button, [role='combobox'], [contenteditable='true']")) event.currentTarget.focus(); }} onKeyDown={(event) => { if (!auditKeyboardMode || !isAdmin || recordItemMutation.isPending || event.ctrlKey || event.metaKey || event.altKey) return; const target = event.target as HTMLElement; if (target.closest("input, textarea, button, [role='combobox'], [contenteditable='true']")) return; const quickRows = Array.from(document.querySelectorAll<HTMLTableRowElement>('[data-audit-quick-row="true"]')); const currentIndex = quickRows.indexOf(event.currentTarget); if (event.key === "ArrowDown" || event.key === "ArrowUp") { event.preventDefault(); const direction = event.key === "ArrowDown" ? 1 : -1; quickRows[Math.min(Math.max(currentIndex + direction, 0), quickRows.length - 1)]?.focus(); return; } if (event.key === "1") { event.preventDefault(); updateDraft(item, { actualStatus: item.expectedStatus || "", result: "matched" }); return; } if (event.key === "2") { event.preventDefault(); updateDraft(item, { result: "mismatch" }); return; } if (event.key === "3") { event.preventDefault(); updateDraft(item, { actualStatus: "", result: "missing", note: draft.note || "Không tìm thấy tại vị trí kiểm kê." }); return; } if (event.key === "Enter") { event.preventDefault(); const nextQuickRow = quickRows[currentIndex + 1]; auditNextFocusIdRef.current = Number(nextQuickRow?.dataset.auditItemId) || null; recordItem(item); } }} className={`border-t border-[#EDF2F5] align-top outline-none ${auditKeyboardMode ? "cursor-pointer focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0F8C8C]" : ""} ${draft.result === "missing" ? "bg-[#FFF1F3] ring-1 ring-inset ring-[#F3B5BD]" : isDiscrepancy ? "bg-[#FFF9EB] ring-1 ring-inset ring-[#F2D596]" : ""}`}><td className={`sticky left-0 z-10 w-[210px] border-r border-[#E7EEF3] px-5 py-4 shadow-[6px_0_10px_-8px_rgba(16,42,67,0.35)] ${draft.result === "missing" ? "border-l-4 border-l-[#D04A5A] bg-[#FFF1F3]" : draft.result === "mismatch" ? "border-l-4 border-l-[#D99000] bg-[#FFF9EB]" : "bg-white"}`}><div className="font-bold text-[#193B57]">{asset?.name || `Tài sản #${item.assetId}`}</div><div className="mt-1 font-mono text-[10px] text-[#0F8C8C]">{asset?.assetCode || "Tài sản đã bị lưu trữ"}</div><div className="mt-1 text-[10px] text-[#8AA0B6]">Trạng thái hệ thống: {auditAssetStatusLabel(asset?.status)}</div>{draft.result === "missing" ? <div className="mt-2 inline-flex items-center gap-1 rounded-full bg-[#FADDE1] px-2 py-1 text-[10px] font-extrabold text-[#B44545]"><AlertTriangle size={11} />Ưu tiên · Không tìm thấy</div> : draft.result === "mismatch" ? <div className="mt-2 inline-flex items-center gap-1 rounded-full bg-[#FFECC4] px-2 py-1 text-[10px] font-extrabold text-[#A86B00]"><AlertTriangle size={11} />Ưu tiên · Chênh lệch</div> : null}</td>{!(isAuditFullscreen && auditDeviceMode === "tablet") && <><td className="px-4 py-4"><span className="inline-flex max-w-[180px] truncate rounded-full bg-[#F0F5F8] px-2 py-1 text-[10px] font-bold text-[#60758A]" title={branchNameForAsset(asset)}>{branchNameForAsset(asset)}</span></td><td className="px-4 py-4"><span className="rounded-full bg-[#F0F5F8] px-2 py-1 text-[10px] font-bold text-[#60758A]">{auditAssetStatusLabel(item.expectedStatus)}</span></td></>}<td className="px-4 py-4"><SearchableSelect value={draft.actualStatus} onChange={(value) => updateDraft(item, { actualStatus: value })} disabled={!isAdmin || recordItemMutation.isPending} className="min-w-[185px]" placeholder="Chưa ghi nhận" searchPlaceholder="Tìm trạng thái thực tế..." options={[{ value: "", label: "Chưa ghi nhận" }, { value: "available", label: "Sẵn có" }, { value: "assigned", label: "Đang cấp phát" }, { value: "maintenance", label: "Bảo trì" }, { value: "retired", label: "Ngừng sử dụng" }, { value: "lost", label: "Thất lạc" }, { value: "damaged", label: "Hư hỏng" }]} /></td><td className="px-4 py-4"><SearchableSelect value={draft.result} onChange={(value) => updateDraft(item, { result: value as AuditItemDraft["result"] })} disabled={!isAdmin || recordItemMutation.isPending} className="min-w-[155px]" searchPlaceholder="Tìm kết quả..." options={[{ value: "pending", label: "Chưa kiểm" }, { value: "matched", label: "Khớp" }, { value: "mismatch", label: "Chênh lệch" }, { value: "missing", label: "Không tìm thấy" }]} /><span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[10px] font-extrabold ${badgeTone}`}>{auditResultLabels[draft.result]}</span></td><td className="px-4 py-4">{isDiscrepancy && <div className="mb-2 flex items-center gap-1 text-[10px] font-bold text-[#B44545]"><AlertTriangle size={12} />Bổ sung ghi chú xử lý</div>}<textarea disabled={!isAdmin || recordItemMutation.isPending} value={draft.note} onChange={(event) => updateDraft(item, { note: event.target.value })} placeholder="Mô tả hiện trạng, vị trí hoặc lý do chênh lệch..." className="min-h-[70px] w-full resize-y rounded-md border border-[#DDE7F0] p-2 text-xs leading-5 text-[#193B57] outline-none focus:border-[#0F8C8C] disabled:cursor-not-allowed disabled:opacity-60" />{item.checkedAt && <div className="mt-2 flex items-center gap-1 text-[10px] text-[#8AA0B6]"><CheckCircle2 size={11} />Ghi nhận {new Date(item.checkedAt).toLocaleString("vi-VN")}</div>}</td><td className={`sticky right-0 z-10 w-[180px] min-w-[180px] border-l border-[#E7EEF3] px-5 py-4 text-right ${draft.result === "missing" ? "bg-[#FFF1F3]" : isDiscrepancy ? "bg-[#FFF9EB]" : "bg-white"}`}><div className="flex w-full flex-col items-end gap-2"><button disabled={!isAdmin || recordItemMutation.isPending} onClick={() => recordItem(item)} className="inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-md bg-[#0F8C8C] px-3 py-2 text-[10px] font-bold text-white hover:bg-[#087A6A] disabled:cursor-not-allowed disabled:opacity-60"><Save size={13} />{recordItemMutation.isPending ? "Đang lưu" : "Lưu kết quả"}</button><button type="button" disabled={!isAdmin || removeAuditItemMutation.isPending} onClick={() => setAuditItemToRemove(item.id)} className="inline-flex items-center gap-1 whitespace-nowrap text-[10px] font-bold text-[#B44545] hover:underline disabled:cursor-not-allowed disabled:opacity-60"><Trash2 size={12} />Xóa khỏi đợt</button><button disabled={!isAdmin || recordItemMutation.isPending} onClick={() => updateDraft(item, { actualStatus: "", result: "missing", note: draft.note || "Không tìm thấy tại vị trí kiểm kê." })} className="whitespace-nowrap text-[10px] font-bold text-[#B44545] hover:underline disabled:cursor-not-allowed disabled:opacity-60">Đánh dấu thất lạc</button></div></td></tr>; })}</tbody></table>{!auditItemsQuery.isLoading && filteredAuditItems.length > 0 && <div className="flex flex-col gap-3 border-t border-[#E7EEF3] bg-[#FBFCFD] px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="text-xs font-semibold text-[#60758A]">Hiển thị {(activeAuditItemsPage - 1) * AUDIT_ITEMS_PAGE_SIZE + 1}–{Math.min(activeAuditItemsPage * AUDIT_ITEMS_PAGE_SIZE, filteredAuditItems.length)} / {filteredAuditItems.length} tài sản</div><div className="flex items-center gap-2"><button type="button" aria-label="Trang trước" disabled={activeAuditItemsPage <= 1} onClick={() => setAuditItemsPage((page) => Math.max(1, page - 1))} className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-[#DDE7F0] bg-white text-[#60758A] transition hover:border-[#8BCDC6] hover:text-[#087A6A] disabled:cursor-not-allowed disabled:opacity-40"><ChevronLeft size={16} /></button><span className="min-w-[82px] text-center text-xs font-bold text-[#193B57]">Trang {activeAuditItemsPage}/{auditItemsPageCount}</span><button type="button" aria-label="Trang sau" disabled={activeAuditItemsPage >= auditItemsPageCount} onClick={() => setAuditItemsPage((page) => Math.min(auditItemsPageCount, page + 1))} className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-[#DDE7F0] bg-white text-[#60758A] transition hover:border-[#8BCDC6] hover:text-[#087A6A] disabled:cursor-not-allowed disabled:opacity-40"><ChevronRight size={16} /></button></div></div>}{!auditItemsQuery.isLoading && auditItems.length === 0 && <ModuleEmptyState module="audit" title="Chưa có tài sản trong đợt kiểm kê" description="Hãy chọn tài sản để bắt đầu đối chiếu và ghi nhận kết quả kiểm kê." />}{!auditItemsQuery.isLoading && auditItems.length > 0 && filteredAuditItems.length === 0 && <ModuleEmptyState module="audit" title="Không có tài sản phù hợp" description="Hãy thay đổi bộ lọc kết quả, Phòng ban hoặc Phân loại để xem thêm dữ liệu." />}</div>}
          </div>
          <section className={`border-t border-[#E7EEF3] bg-[#FBFCFD] px-5 py-4 ${isAuditFullscreen ? "hidden" : ""}`}><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2 text-xs font-extrabold text-[#193B57]"><History size={15} className="text-[#2666A8]" />Lịch sử nhập Excel</div><span className="rounded-full bg-[#EAF3FF] px-2.5 py-1 text-[10px] font-extrabold text-[#2666A8]">{auditImportHistoryQuery.data?.length || 0} lần</span></div>{auditImportHistoryQuery.isLoading ? <p className="mt-2 text-xs text-[#71869A]">Đang tải lịch sử nhập Excel...</p> : auditImportHistoryQuery.isError ? <button onClick={() => auditImportHistoryQuery.refetch()} className="mt-2 text-xs font-bold text-[#B44545] underline">Không thể tải lịch sử. Thử lại</button> : auditImportHistoryQuery.data?.length ? <div className="mt-3 space-y-2">{auditImportHistoryQuery.data.map((entry) => <div key={entry.id} className="flex flex-col gap-1 rounded-lg border border-[#E7EEF3] bg-white px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between"><div className="text-xs font-semibold text-[#193B57]">{entry.summary || "Nhập Excel kiểm kê"}</div><div className="text-[10px] text-[#71869A]">{entry.actorName || "Quản trị viên"} · {new Date(entry.createdAt).toLocaleString("vi-VN")}</div></div>)}</div> : <p className="mt-2 text-xs text-[#8AA0B6]">Chưa có lần nhập Excel nào cho đợt kiểm kê này.</p>}</section>
        </section>}
