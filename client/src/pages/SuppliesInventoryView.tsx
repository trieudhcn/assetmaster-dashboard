...
export function SuppliesInventoryView({ canEditSectionLabels = false }: { canEditSectionLabels?: boolean }) {
  ...
  const [groupFilter, setGroupFilter] = usePersistedState("assetmaster.filters.supplies.group", "all");
  ...
  const [holdingSupplyId, setHoldingSupplyId] = useState<number | null>(null);
  ...
  const selectedHoldingSupply =
    supplies.find(item => item.id === holdingSupplyId) || null;
  const holdersQuery = trpc.supplies.supplyHolders.useQuery(
  ...
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    ...
    <div className="mt-6">
      <main className="min-w-0 space-y-5"><div className="rounded-2xl border border-[#DFE9F0] bg-white p-4 shadow-[0_8px_24px_rgba(16,42,67,0.05)]"><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><div className="relative min-w-0 w-full sm:max-w-[420px]"><Search className="pointer-events-none absolute left-4 top-1/2 z-10 -translate-y-1/2 text-[#8AA0B6]" size={17} /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} className="field-input !pl-11" placeholder="Tìm mã, tên hoặc vị trí kho..." /></div><div className="w-full sm:w-[220px]"><SearchableSelect value={groupFilter} onChange={(value) => { setGroupFilter(value); setPage(1); }} options={groupFilterOptions} placeholder="Lọc theo nhóm" searchPlaceholder="Tìm nhóm phụ kiện..." emptyText="Không tìm thấy nhóm phụ kiện" emptyActionLabel="Tạo nhóm phụ kiện" onEmptyAction={(name) => setQuickGroupName(name)} /></div><div className="flex shrink-0 items-center gap-2"><button type="button" onClick={() => { setLowOnly((current) => !current); setPage(1); }} className={`supply-filter-action ${lowOnly ? "border-[#F2B18B] bg-[#FFF2E9] text-[#C75419]" : ""}`}><AlertTriangle size={16} /><span>{lowOnly ? "Đang xem sắp hết" : "Sắp hết hàng"}</span></button><button type="button" onClick={() => { setSearch(""); setGroupFilter("all"); setLowOnly(false); setPage(1); }} className="supply-filter-action"><SlidersHorizontal size={16} /><span>Đặt lại</span></button></div></div></div>
        ...
        <div className="overflow-hidden rounded-2xl border border-[#DFE9F0] bg-white shadow-[0_8px_24px_rgba(16,42,67,0.05)]"><div className="mobile-table-scroll overflow-x-auto"><table className="w-full min-w-[1120px] text-left text-xs"><thead className="bg-[#F8FBFC] text-[10px] uppercase tracking-[.1em] text-[#8AA0B6]"><tr><th className="sticky left-0 z-10 bg-[#F8FBFC] px-4 py-4">Phụ kiện</th><th className="px-3 py-4">Phân loại</th><th className="px-3 py-4 text-right">Tổng nhập</th><th className="px-3 py-4 text-right">Đang giữ</th><th className="px-3 py-4 text-right">Tồn khả dụng</th><th className="px-3 py-4 text-right">Tối thiểu</th><th className="px-3 py-4">Vị trí</th><th className="px-3 py-4">Đơn giá</th><th className="px-4 py-4 text-right">Thao tác</th></tr></thead><tbody>{suppliesQuery.isLoading ? <tr><td colSpan={9} className="p-10 text-center text-sm text-[#71869A]">Đang tải phụ kiện...</td></tr> : rows.map((item) => { const low = Number(item.stockQuantity) <= Number(item.minimumQuantity); return <tr key={item.id} className={`border-t border-[#EDF2F5] ${low ? "bg-[#FFF9F2]" : "hover:bg-[#FBFCFD]"}`}><td className="sticky left-0 z-[1] bg-inherit px-4 py-3"><div className="font-bold text-[#193B57]">{item.name}</div><div className="mt-1 font-mono text-[10px] text-[#8AA0B6]">{item.code} · {item.unit}</div></td><td className="px-3 py-3 text-[#60758A]">{item.categoryId ? categoriesById.get(item.categoryId) || "—" : "—"}</td><td className="px-3 py-3 text-right font-semibold text-[#2666A8]">{quantity(item.totalReceivedQuantity)}</td><td className="px-3 py-3 text-right"><button type="button" onClick={() => setHoldingSupplyId(item.id)} disabled={Number(item.heldQuantity) <= 0} className="rounded-md px-2 py-1 font-semibold text-[#A86B00] transition hover:bg-[#FFF7E2] hover:underline disabled:cursor-default disabled:no-underline disabled:hover:bg-transparent" title={Number(item.heldQuantity) > 0 ? "Xem nhân viên đang giữ" : "Chưa có người đang giữ"} aria-label={`Xem người đang giữ ${item.name}`}>{quantity(item.heldQuantity)}</button></td><td className="px-3 py-3 text-right"><span className={`font-display text-sm font-extrabold ${low ? "text-[#C75419]" : "text-[#087A6A]"}`}>{quantity(item.stockQuantity)}</span>{Number(item.damagedQuantity || 0) > 0 || Number(item.repairQuantity || 0) > 0 ? <div className="mt-1 text-[9px] font-bold text-[#8A6070]">Hỏng {quantity(item.damagedQuantity)} · Cần sửa {quantity(item.repairQuantity)}</div> : null}</td><td className="px-3 py-3 text-right font-semibold text-[#60758A]">{quantity(item.minimumQuantity)}</td><td className="px-3 py-3 text-[#60758A]">{item.location || "—"}</td><td className="px-3 py-3 text-[#60758A]">{money(item.unitCost)}</td><td className="px-4 py-3 text-right"><div className="inline-flex items-center justify-end gap-1"><button type="button" onClick={() => setSelectedId(item.id)} className="supply-row-action text-[#087A6A]" title="Ghi nhận nhập, xuất hoặc cấp phát" aria-label="Ghi nhận nhập, xuất hoặc cấp phát"><ArchiveRestore size={16} /></button><button type="button" onClick={() => beginEdit(item.id)} className="supply-row-action" title="Sửa mã và tên phụ kiện" aria-label="Sửa mã và tên phụ kiện"><Pencil size={16} /></button><button type="button" onClick={() => setSelectedId(item.id)} className="supply-row-action" title="Xem lịch sử biến động" aria-label="Xem lịch sử biến động"><History size={16} /></button></div></td></tr>; })}{!suppliesQuery.isLoading && !rows.length && <tr><td colSpan={9} className="p-10 text-center text-sm text-[#8AA0B6]">Chưa có phụ kiện phù hợp. Hãy dùng nút Thêm phụ kiện để tạo mặt hàng đầu tiên.</td></tr>}</tbody></table></div><Pagination rows={rows.length} total={filtered.length} page={activePage} totalPages={totalPages} onPrevious={() => setPage((current) => Math.max(1, current - 1))} onNext={() => setPage((current) => Math.min(totalPages, current + 1))} /></div><SupplyIssueSlipManager /></main></div></div>
    <Dialog
      open={holdingSupplyId !== null}
      onOpenChange={open => {
        if (!open) setHoldingSupplyId(null);
      }}
    >
      <DialogContent className="flex max-h-[85vh] max-w-2xl flex-col overflow-hidden rounded-2xl border-[#CDE5E5] bg-white p-0 shadow-[0_24px_70px_rgba(16,42,67,.24)]">
        <DialogHeader className="assetmaster-modal-header shrink-0 border-b border-[#E7EEF3] px-5 pt-5 pb-6 pr-12 text-left">
          <div className="text-[10px] font-extrabold uppercase tracking-[.14em] text-[#0F8C8C]">
            Phụ kiện đang được sử dụng
          </div>
          <DialogTitle className="font-display text-lg font-extrabold text-[#193B57]">
            {selectedHoldingSupply?.name || "Chi tiết đang giữ"}
          </DialogTitle>
          <p className="text-xs text-[#71869A]">
            {selectedHoldingSupply?.code || "—"} · Tổng đang giữ{" "}
            <b className="text-[#A86B00]">
              {quantity(selectedHoldingSupply?.heldQuantity)}{" "}
              {selectedHoldingSupply?.unit || ""}
            </b>
          </p>
        </DialogHeader>
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {holdersQuery.isLoading ? (
            <p className="py-8 text-center text-xs font-semibold text-[#71869A]">
              Đang tải danh sách người giữ...
            </p>
          ) : (holdersQuery.data || []).length ? (
            <div className="space-y-2">
              {(holdersQuery.data || []).map((holder, index) => (
                <article
                  key={holder.recipientUserId || `${holder.recipientName}-${index}`}
                  className="rounded-xl border border-[#DFE9F0] bg-[#FBFCFD] p-3"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-extrabold text-[#193B57]">
                        {holder.recipientName}
                      </div>
                      <div className="mt-1 truncate text-[10px] text-[#71869A]" title={holder.sourceReferences.join(", ")}>
                        Phiếu: {holder.sourceReferences.join(", ")}
                      </div>
                    </div>
                    <div className="shrink-0 text-right">
                      <div className="font-display text-lg font-extrabold text-[#A86B00]">
                        {quantity(holder.heldQuantity)}
                      </div>
                      <div className="text-[10px] font-bold text-[#8AA0B6]">
                        {selectedHoldingSupply?.unit || ""}
                      </div>
                    </div>
                  </div>
            ...
            <div className="rounded-xl border border-dashed border-[#CDE5E5] bg-[#F8FBFC] px-4 py-8 text-center text-xs font-semibold text-[#71869A]">
              Không còn nhân viên nào đang giữ phụ kiện này.
            </div>
          )}
        </div>
