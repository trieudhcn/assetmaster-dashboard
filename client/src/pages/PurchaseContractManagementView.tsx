...
export function PurchaseContractManagementView() {
  ...
  const save = () => {
    ...
    if (editingId) updateContract.mutate({ id: editingId, ...payload }); else createContract.mutate(payload);
  };
  const handleDocument = async (file: File) => {
    if (!selectedId) return;
    const allowed = ["application/pdf", "image/png", "image/jpeg", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"] as const;
    if (!allowed.includes(file.type as typeof allowed[number])) { toast.error("Chỉ hỗ trợ PDF, PNG, JPG, DOCX và XLSX."); return; }
    if (file.size > 5 * 1024 * 1024) { toast.error("Chứng từ hợp đồng tối đa 5 MB."); return; }
    const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => typeof reader.result === "string" ? resolve(reader.result) : reject(new Error("Không thể đọc tệp.")); reader.onerror = () => reject(new Error("Không thể đọc tệp.")); reader.readAsDataURL(file); });
    uploadDocument.mutate({ purchaseContractId: selectedId, documentType, fileName: file.name, contentType: file.type as typeof allowed[number], dataUrl });
  };
  ...
  return <div className="min-h-screen bg-[#F4F7FB] px-4 py-7 sm:px-6 lg:px-9 lg:py-8"><div className="mx-auto max-w-[1500px]">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[.16em] text-[#0F8C8C]"><FileCheck2 size={14} />Mua sắm & chứng từ</div><h1 className="mt-1 font-display text-3xl font-extrabold text-[#102A43]">Hợp đồng mua bán</h1><p className="mt-1 max-w-2xl text-sm text-[#71869A]">Lưu một hồ sơ hợp đồng, chứng từ ký và liên kết nhiều tài sản hoặc phụ kiện thuộc cùng giao dịch mua sắm.</p></div><button type="button" onClick={openCreate} className="primary-action"><Plus size={16} />Tạo hợp đồng</button></div>
...
function LinkedList({ title, icon, items, empty }: { title: string; icon: React.ReactNode; items: Array<{ code: string; name: string; detail: string }>; empty: string }) { return <div className="rounded-lg border border-[#E7EEF3] bg-[#FBFCFD] p-3"><div className="flex items-center gap-2 text-xs font-extrabold text-[#193B57]">{icon}{title}</div><div className="mt-2 space-y-2">{items.slice(0, 8).map((item) => <div key={item.code} className="rounded-md border border-[#EDF2F5] bg-white px-2.5 py-2"><div className="font-mono text-[10px] font-bold text-[#087A6A]">{item.code}</div><div className="mt-0.5 truncate text-[11px] font-bold text-[#193B57]">{item.name}</div><div className="mt-0.5 text-[10px] text-[#71869A]">{item.detail}</div></div>)}{items.length > 8 && <p className="text-center text-[10px] font-semibold text-[#71869A]">Còn {items.length - 8} mục khác</p>}{!items.length && <p className="rounded-md border border-dashed border-[#D8E3EA] px-2 py-3 text-center text-[10px] text-[#8AA0B6]">{empty}</p>}</div></div>; }
