      ...
      <div className="grid items-stretch gap-5 xl:grid-cols-[minmax(0,.8fr)_minmax(0,1.2fr)]">
        ...
        <section className="flex min-h-[35rem] flex-col overflow-hidden rounded-xl border border-[#DFE9F0] bg-white shadow-[0_8px_24px_rgba(16,42,67,0.045)]">
          <div className="border-b border-[#E7EEF3] px-5 py-4">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2"><SearchableSelect value={activityFilter} onChange={(value) => setActivityFilter(value as typeof activityFilter)} options={[{ value: "all", label: "Tất cả trạng thái" }, { value: "active", label: "Đang hoạt động" }, { value: "inactive", label: "Ngừng hoạt động" }]} placeholder="Tất cả trạng thái" variant="compact" className="min-w-0" /><SearchableSelect value={assetFilter} onChange={(value) => setAssetFilter(value as typeof assetFilter)} options={[{ value: "all", label: "Tất cả số lượng" }, { value: "with-assets", label: "Có tài sản" }, { value: "empty", label: "Chưa có tài sản" }]} placeholder="Tất cả số lượng" variant="compact" className="min-w-0" /></div>
          </div>
