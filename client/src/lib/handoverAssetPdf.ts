...
export async function openHandoverAssetPdf(input: HandoverPdfInput, company: HandoverPdfCompany, options?: { autoPrint?: boolean; fileName?: string }) {
  ...
  doc.setFontSize(8); doc.setTextColor(112, 134, 154); doc.text(`Địa chỉ: ${company.address || ""}`, left + 24, y + 2); doc.text(`MST: ${company.taxCode || ""} · Điện thoại: ${company.phone || ""}`, left + 24, y + 8);
