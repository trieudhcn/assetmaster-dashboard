...
export type SupplyImportCatalogOption = { value: string; label: string; isActive: boolean };
export const standardSupplyUnits = ["Cái", "Bộ", "Chiếc", "Hộp", "Gói", "Cuộn", "Chai", "Can", "Thùng", "Ram", "Kg", "Mét", "Lít"];
...
export function resolveActiveSupplyImportCatalog(value: string, options: SupplyImportCatalogOption[], label: string) {
  if (!value.trim()) return { id: null, error: null };
  const option = options.find((item) => item.label.localeCompare(value.trim(), "vi", { sensitivity: "accent" }) === 0);
  if (!option) return { id: null, error: `không tìm thấy ${label} “${value.trim()}”` };
  if (!option.isActive) return { id: null, error: `${label} “${option.label}” đã bị vô hiệu hóa` };
  return { id: Number(option.value), error: null };
}
...
export async function buildSupplyImportTemplate(dropdowns: SupplyImportDropdowns) {
  ...
  const units = cleanOptions(dropdowns.units || standardSupplyUnits);
  ...
  lists.addRow(["Đơn vị tính", "Phân loại", "Nhà cung cấp", "Hãng"]);
  const maxRows = Math.max(units.length, categories.length, vendors.length, brands.length, 1);
  for (let index = 0; index < maxRows; index += 1) lists.addRow([units[index] || "", categories[index] || "", vendors[index] || "", brands[index] || ""]);
  lists.columns = [{ width: 20 }, { width: 32 }, { width: 36 }, { width: 32 }];
  ...
  workbook.definedNames.add(`'Danh sách chọn'!$A$2:$A$${maxRows + 1}`, "SupplyUnits");
  workbook.definedNames.add(`'Danh sách chọn'!$B$2:$B$${maxRows + 1}`, "SupplyCategories");
  workbook.definedNames.add(`'Danh sách chọn'!$C$2:$C$${maxRows + 1}`, "SupplyVendors");
  workbook.definedNames.add(`'Danh sách chọn'!$D$2:$D$${maxRows + 1}`, "SupplyBrands");

  for (let row = 2; row <= 201; row += 1) {
    template.getCell(`C${row}`).dataValidation = { type: "list", allowBlank: false, formulae: ["SupplyUnits"], showErrorMessage: true, errorTitle: "Đơn vị tính không hợp lệ", error: "Chọn Đơn vị tính từ danh sách chuẩn." };
