...
export function validateAssetImportHeaders(headers: unknown[]) {
  ...
  const incorrect = assetImportHeaders.filter((header, index) => {
    // Các template trước đây dùng nhãn “Số Hóa đơn”. Chấp nhận chúng để người
    // dùng không phải tạo lại file đã chuẩn bị, nhưng template mới dùng cùng
    // nhãn “Mã Hóa đơn” với danh mục và các báo cáo.
    if (header === "Mã Hóa đơn")
      return normalized[index] !== header && normalized[index] !== "Số Hóa đơn";
...
export function parseAssetImportRows(rows: Array<Record<string, unknown>>) {
  ...
  rows.forEach((row, index) => {
    ...
    const status =
      statusText.toLocaleLowerCase("vi-VN") === "bảo trì"
        ? "maintenance"
        : statusText.toLocaleLowerCase("vi-VN") === "sẵn có"
          ? "available"
          : null;
    const conditionMap: Record<string, AssetImportCandidate["condition"]> = {
      tốt: "good",
      khá: "fair",
      "cần kiểm tra": "needs_inspection",
      "hư hỏng": "damaged",
    };
    ...
    if (!name || !category)
      issues.push({ rowNumber, message: "Cần nhập Tên tài sản và Phân loại." });
    else if (!status)
      issues.push({
        rowNumber,
        message: "Trạng thái chỉ nhận Sẵn có hoặc Bảo trì.",
      });
    else if (!condition)
      issues.push({
        rowNumber,
        message: "Tình trạng chỉ nhận Tốt, Khá, Cần kiểm tra hoặc Hư hỏng.",
      });
    else if (status === "maintenance" && !maintenanceReason)
      issues.push({
        rowNumber,
        message: "Tài sản Bảo trì cần có Lý do bảo trì.",
      });
    else if (purchaseDateSource && !purchaseDate)
      issues.push({
        rowNumber,
        message: "Ngày mua phải theo định dạng dd/mm/yyyy.",
      });
    else if (warrantySource && !warrantyUntil)
      issues.push({
        rowNumber,
        message: "Hạn bảo hành phải theo định dạng dd/mm/yyyy.",
      });
    else if (purchaseValueRaw && !purchaseValueSource)
      issues.push({
        rowNumber,
        message:
          "Giá trị phải là số VNĐ nguyên, ví dụ 25000000 hoặc 25.000.000.",
      });
    else {
      candidates.push({
        rowNumber,
        ...
        condition,
        purchaseDate,
        purchaseValue: purchaseValueSource || null,
        invoiceNumber:
          text(row["Mã Hóa đơn"]) || text(row["Số Hóa đơn"]) || null,
        vendor: text(row["Nhà cung cấp"]) || null,
        brandName: text(row["Hãng"]) || null,
        serialNumber: text(row["Serial/IMEI"]) || null,
        location: text(row["Vị trí"]) || null,
        warrantyUntil,
        note: text(row["Ghi chú"]) || null,
      });
    }
  });
