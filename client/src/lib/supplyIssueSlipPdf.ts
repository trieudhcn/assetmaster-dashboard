...
export async function openSupplyIssueSlipPdf(slip: SupplyIssueSlipPdf, items: SupplyIssueItemPdf[]) {
  ...
  doc.setFontSize(10);
  doc.text(`Mã phiếu: ${slip.referenceCode}`, pageWidth / 2, titleY + 7, { align: "center" });
  doc.text(`Người nhận: ${slip.recipientName}`, margin, informationY);
  doc.text(`Người cấp: ${slip.issuedByName || "Quản trị viên"}`, margin, informationY + 6);
  doc.text(`Ngày cấp: ${new Date(slip.issuedAt).toLocaleString("vi-VN")}`, margin, informationY + 12);
  if (slip.note) doc.text(`Ghi chú: ${slip.note}`, margin, informationY + 18, { maxWidth: pageWidth - margin * 2 });
  const startY = slip.note ? informationY + 27 : informationY + 21;
