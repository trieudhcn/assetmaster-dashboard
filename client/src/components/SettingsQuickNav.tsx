...
function SettingLinks({ compact = false, rail = false }: { compact?: boolean; rail?: boolean }) {
  const className = rail ? "grid h-9 w-9 place-items-center rounded-lg text-[#527089] transition hover:bg-[#F4FBFA] hover:text-[#087A6A] focus:bg-[#F4FBFA] focus:text-[#087A6A]" : compact ? "inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[#DDE7F0] bg-white px-3 py-2 text-[11px] font-bold text-[#527089] transition hover:border-[#8BCDC6] hover:bg-[#F4FBFA] hover:text-[#087A6A]" : "flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-[#527089] transition hover:bg-[#F4FBFA] hover:text-[#087A6A]";
  return <>{settingLinks.map(({ href, label, icon: Icon }) => href === "#settings-directory" ? <button key={href} type="button" title="Mở cấu hình Directory" aria-label="Mở cấu hình Directory LDAP/AD" onClick={() => window.dispatchEvent(new Event("assetmaster:open-directory-settings"))} className={className}><Icon size={14} />{rail ? <span className="sr-only">{label}</span> : label}</button> : <a key={href} href={href} title={label} aria-label={`Đi đến ${label}`} className={className}><Icon size={14} />{rail ? <span className="sr-only">{label}</span> : label}</a>)}</>;
}
