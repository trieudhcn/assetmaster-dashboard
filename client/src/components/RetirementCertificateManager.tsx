...
import { Archive, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, CircleDollarSign, Download, FileCheck2, FileUp, LoaderCircle, PencilLine, Printer, Save, Search, Trash2, TriangleAlert, X } from "lucide-react";
...
type ServiceCostTicket = { id: number; ticketCode: string };
type ServiceCostInfo = { warrantyTickets: ServiceCostTicket[]; repairTickets: ServiceCostTicket[]; totalWarrantyCost: number; totalRepairCost: number; totalActualCost: number; hasWarrantyActualCost: boolean; hasRepairActualCost: boolean; hasActualCost: boolean };
...
function ServiceCostIndicator({ info }: { info?: ServiceCostInfo }) {
  ...
  const channels = [info.hasWarrantyActualCost && "BH", info.hasRepairActualCost && "SC"].filter(Boolean).join(" + ");
  return <span className="mt-1.5 inline-flex w-fit items-center gap-1 rounded-full bg-[#FFF5DC] px-1.5 py-0.5 text-[9px] font-extrabold text-[#A86B00]" title={`Đã ghi nhận phí ${channels}: ${formatVnd(info.totalActualCost)} VNĐ`}><CircleDollarSign size={11} aria-hidden="true" />Đã có phí {channels}</span>;
}

function ServiceCostReference({ info, onPreview }: { info?: ServiceCostInfo; onPreview: (ticketId: number) => void }) {
  if (!info) return null;
  const channels = [
    { key: "warranty", label: "Bảo hành", tickets: info.warrantyTickets, cost: info.totalWarrantyCost, hasCost: info.hasWarrantyActualCost, tone: "text-[#087A6A]" },
    { key: "repair", label: "Sửa chữa", tickets: info.repairTickets, cost: info.totalRepairCost, hasCost: info.hasRepairActualCost, tone: "text-[#A86B00]" },
  ];
  return <div className="mt-3 rounded-lg border border-[#F2D596] bg-[#FFF9EB] px-3 py-2.5 text-[10px] leading-5 text-[#735314]"><div className="flex flex-wrap items-center gap-x-1.5"><span className="font-extrabold text-[#8F5A00]">Tham khảo phí Bảo hành/Sửa chữa</span><span className="text-[#A48548]">(không đưa vào biên bản)</span></div><div className="mt-1.5 space-y-1">{channels.filter((channel) => channel.tickets.length > 0).map((channel) => <div key={channel.key} className="flex flex-wrap items-center gap-x-1.5"><span className={`font-extrabold ${channel.tone}`}>{channel.label}:</span><span>{channel.tickets.map((ticket, index) => <span key={ticket.id}>{index > 0 && ", "}<button type="button" onClick={() => onPreview(ticket.id)} className="font-mono font-extrabold underline decoration-dotted underline-offset-2 hover:text-[#5E3E00]" title={`Mở nhanh phiếu ${channel.label.toLowerCase()}`}>{ticket.ticketCode}</button></span>)}</span><span className="text-[#8F5A00]">· {channel.hasCost ? `${formatVnd(channel.cost)} VNĐ` : "Chưa cập nhật phí thực tế"}</span></div>)}</div><div className="mt-1.5 border-t border-[#F1DFB5] pt-1.5 font-extrabold text-[#8F5A00]">{info.hasActualCost ? `Tổng phí đã ghi nhận: ${formatVnd(info.totalActualCost)} VNĐ` : "Chưa cập nhật chi phí thực tế"}</div></div>;
}
...
function SalvageValueEditor({ certificate, saving, onSave, onClose }: { certificate: Certificate; saving: boolean; onSave: (values: Record<number, string>) => Promise<void>; onClose: () => void }) {
  const [values, setValues] = useState<Record<number, string>>(() => Object.fromEntries(certificate.items.map((item) => [item.id, item.salvageValue || ""])));
  useEffect(() => setValues(Object.fromEntries(certificate.items.map((item) => [item.id, item.salvageValue || ""]))), [certificate.items]);
  const purchaseTotal = certificate.items.reduce((total, item) => total + numeric(item.purchaseValue), 0);
  const salvageTotal = certificate.items.reduce((total, item) => total + numeric(values[item.id]), 0);
  const exceedsPurchaseValue = salvageTotal > purchaseTotal;
  const handleSave = async () => { try { await onSave(values); onClose(); } catch { /* Mutation đã hiển thị lỗi. */ } };
  return <div className="mt-4 rounded-xl border border-[#C9DAE8] bg-[#F8FCFF] p-3"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-xs font-extrabold text-[#193B57]">Cập nhật giá trị thu hồi sau ký tay</div><p className="mt-1 text-[10px] leading-4 text-[#60758A]">Nhập giá trị thu hồi chính thức cho từng tài sản trước khi xác nhận đóng biên bản.</p></div><button type="button" onClick={onClose} disabled={saving} className="modal-close-action">Đóng</button></div><div className="mt-3 space-y-2">{certificate.items.map((item) => <div key={item.id} className="grid gap-2 rounded-lg border border-[#E0E8F0] bg-white p-2.5 sm:grid-cols-[minmax(0,1fr)_180px] sm:items-center"><div className="min-w-0"><div className="truncate text-xs font-bold text-[#193B57]">{item.assetName}</div><div className="mt-0.5 font-mono text-[10px] text-[#8AA0B6]">{item.assetCode} · Nguyên giá {formatVnd(item.purchaseValue)} VNĐ</div></div><CurrencyInput value={values[item.id] || ""} onChange={(value) => setValues((current) => ({ ...current, [item.id]: value }))} aria-label={`Giá trị thu hồi ${item.assetCode}`} placeholder="Chưa cập nhật" className="h-9 text-xs" /></div>)}</div><div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-lg border border-[#D9E8F3] bg-white px-3 py-2"><div className="text-[9px] font-extrabold uppercase tracking-[.08em] text-[#71869A]">Tổng giá trị thu hồi</div><div className="mt-1 text-sm font-extrabold text-[#087A6A]">{formatVnd(salvageTotal)} VNĐ</div></div><div className="rounded-lg border border-[#E0E8F0] bg-white px-3 py-2"><div className="text-[9px] font-extrabold uppercase tracking-[.08em] text-[#71869A]">Tổng nguyên giá</div><div className="mt-1 text-sm font-extrabold text-[#193B57]">{formatVnd(purchaseTotal)} VNĐ</div></div></div>{exceedsPurchaseValue && <div className="mt-3 flex gap-2 rounded-lg border border-[#F2C77C] bg-[#FFF7E3] px-3 py-2 text-[10px] leading-4 text-[#8F5A00]"><TriangleAlert size={15} className="mt-0.5 shrink-0" /><p><b>Cảnh báo:</b> Tổng giá trị thu hồi cao hơn tổng nguyên giá {formatVnd(purchaseTotal)} VNĐ. Bạn vẫn có thể lưu, nhưng nên rà soát lại số liệu trước khi đóng biên bản.</p></div>}<div className="mt-3 flex justify-end gap-2"><button type="button" onClick={onClose} disabled={saving} className="h-9 rounded-lg border border-[#DDE7F0] bg-white px-3 text-xs font-bold text-[#60758A] disabled:opacity-50">Hủy</button><button type="button" onClick={() => void handleSave()} disabled={saving} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-[#2666A8] px-3 text-xs font-bold text-white disabled:opacity-50"><Save size={14} />{saving ? "Đang lưu..." : "Lưu giá trị thu hồi"}</button></div></div>;
}

function CertificateCard({ certificate, company, uploading, closing, cancelling, onUpload, onPrint, onClose, onCancel }: { certificate: Certificate; company: Company; uploading: boolean; closing: boolean; cancelling: boolean; onUpload: (file?: File) => void; onPrint: () => void; onClose: () => Promise<unknown>; onCancel: () => void }) {
  const certificateUtils = trpc.useUtils();
  const inputRef = useRef<HTMLInputElement>(null);
  const [showDetails, setShowDetails] = useState(false);
  const [showCloseConfirm, setShowCloseConfirm] = useState(false);
  const [showCancelConfirm, setShowCancelConfirm] = useState(false);
  const [showSalvageEditor, setShowSalvageEditor] = useState(false);
  const updateSalvageValues = trpc.retirementCertificates.updateSalvageValues.useMutation({ onSuccess: async ({ salvageTotal, exceedsPurchaseValue }) => { toast.success("Đã cập nhật giá trị thu hồi.", { description: exceedsPurchaseValue ? `Tổng thu hồi ${formatVnd(salvageTotal)} VNĐ cao hơn tổng nguyên giá. Vui lòng rà soát trước khi đóng.` : `Tổng giá trị thu hồi: ${formatVnd(salvageTotal)} VNĐ.` }); await certificateUtils.retirementCertificates.list.invalidate(); }, onError: (error) => toast.error(error.message || "Không thể cập nhật giá trị thu hồi.") });
  ...
  const purchaseTotal = certificate.items.reduce((total, item) => total + numeric(item.purchaseValue), 0);
  const salvageTotal = certificate.items.reduce((total, item) => total + numeric(item.salvageValue), 0);
  const hasSalvageValue = certificate.items.some((item) => item.salvageValue !== null && String(item.salvageValue).trim() !== "");
  const detailId = `retirement-certificate-details-${certificate.id}`;
  const confirmClose = async () => {
    try {
      ...
      // Mutation đã hiển thị lỗi; giữ nguyên khung xác nhận để người dùng có thể thử lại.
    }
  };

  return <article className="rounded-xl border border-[#DFE9F0] bg-white px-3 py-3 sm:px-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-sm font-extrabold text-[#0F8C8C]">{certificate.referenceCode}</span><span className={`rounded-full px-2 py-1 text-[9px] font-extrabold ${statusClass}`}>{statusText}</span></div><p className="mt-1 text-[11px] text-[#60758A]">{certificate.items.length} tài sản · Ngày thanh lý {new Date(certificate.retiredAt).toLocaleDateString("vi-VN")}</p></div><div className="grid grid-cols-2 gap-2 sm:flex sm:items-center"><div className="rounded-md bg-[#F7FAFC] px-2.5 py-1.5"><div className="text-[8px] font-extrabold uppercase tracking-[.08em] text-[#8AA0B6]">Nguyên giá</div><div className="mt-0.5 text-[11px] font-extrabold text-[#193B57]">{formatVnd(purchaseTotal)} VNĐ</div></div><div className="rounded-md bg-[#F7FAFC] px-2.5 py-1.5"><div className="text-[8px] font-extrabold uppercase tracking-[.08em] text-[#8AA0B6]">Thu hồi</div><div className={`mt-0.5 text-[11px] font-extrabold ${hasSalvageValue ? "text-[#087A6A]" : "text-[#8AA0B6]"}`}>{hasSalvageValue ? `${formatVnd(salvageTotal)} VNĐ` : "Chưa cập nhật"}</div></div><button type="button" onClick={() => setShowDetails((current) => !current)} aria-expanded={showDetails} aria-controls={detailId} className="col-span-2 inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border border-[#C9DAE8] bg-white px-3 text-xs font-bold text-[#2666A8] transition hover:bg-[#EAF3FB] sm:col-span-1"><span>{showDetails ? "Thu gọn" : "Chi tiết"}</span><ChevronDown size={15} className={`transition-transform ${showDetails ? "rotate-180" : ""}`} /></button></div></div>{showDetails && <div id={detailId} className="mt-3 border-t border-[#E7EEF3] pt-3"><div className="grid gap-3 lg:grid-cols-[minmax(280px,420px)_minmax(0,1fr)]"><div><CertificateTotals items={certificate.items} /><div className="mt-3 flex flex-wrap gap-1.5">{certificate.items.map((item) => <span key={item.id} title={item.assetName} className="rounded bg-[#F0F5F8] px-2 py-1 font-mono text-[9px] font-bold text-[#60758A]">{item.assetCode}</span>)}</div></div><div className="flex flex-wrap content-start gap-2 lg:justify-end">{!isClosed && <button type="button" onClick={onPrint} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#B7D8D4] bg-white px-3 text-xs font-bold text-[#087A6A]"><Printer size={14} />In nháp</button>}{certificate.signedDocumentUrl && <a href={certificate.signedDocumentUrl} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#B7D8D4] bg-white px-3 text-xs font-bold text-[#087A6A]"><FileCheck2 size={14} />Tệp đã ký</a>}{!isClosed && <><button type="button" disabled={uploading} onClick={() => inputRef.current?.click()} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#DDE7F0] bg-white px-3 text-xs font-bold text-[#2666A8] disabled:opacity-50"><FileUp size={14} />{uploading ? "Đang tải..." : "Tải bản ký tay"}</button><input ref={inputRef} type="file" accept="application/pdf,image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => { onUpload(event.target.files?.[0]); event.currentTarget.value = ""; }} />{certificate.status === "draft" && <button type="button" onClick={() => setShowCancelConfirm(true)} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#F2CACA] bg-white px-3 text-xs font-bold text-[#B44545]"><Trash2 size={14} />Hủy nháp</button>}{canClose && <><button type="button" onClick={() => setShowSalvageEditor(true)} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#C9DAE8] bg-white px-3 text-xs font-bold text-[#2666A8]"><PencilLine size={14} />{hasSalvageValue ? "Cập nhật thu hồi" : "Nhập giá trị thu hồi"}</button><button type="button" onClick={() => setShowCloseConfirm(true)} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-[#0F8C8C] px-3 text-xs font-bold text-white"><CheckCircle2 size={14} />Xác nhận đóng</button></>}</>}</div></div>{showSalvageEditor && <SalvageValueEditor certificate={certificate} saving={updateSalvageValues.isPending} onSave={async (values) => { await updateSalvageValues.mutateAsync({ id: certificate.id, items: certificate.items.map((item) => ({ id: item.id, salvageValue: values[item.id] || null })) }); }} onClose={() => setShowSalvageEditor(false)} />}{showCancelConfirm && <div className="mt-3 flex flex-col gap-3 rounded-lg border border-[#F2CACA] bg-[#FFF7F7] p-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-[#B44545]">Hủy nháp sẽ giải phóng toàn bộ tài sản đã chọn. Trạng thái tài sản không thay đổi.</p><div className="flex shrink-0 gap-2"><button type="button" onClick={() => setShowCancelConfirm(false)} className="h-8 rounded-md border border-[#F2CACA] bg-white px-3 text-xs font-bold text-[#B44545]">Không hủy</button><button type="button" disabled={cancelling} onClick={onCancel} className="inline-flex h-8 items-center gap-1.5 rounded-md bg-[#B44545] px-3 text-xs font-bold text-white disabled:opacity-50">{cancelling && <LoaderCircle size={14} className="animate-spin" />}{cancelling ? "Đang hủy..." : "Xác nhận hủy"}</button></div></div>}{showCloseConfirm && <div className="mt-3 flex flex-col gap-3 rounded-lg border border-[#F2D596] bg-[#FFF9EB] p-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-[#8F5A00]">Đóng biên bản sẽ chuyển toàn bộ tài sản trong biên bản sang Khấu hao/Thanh lý và khóa biên bản.</p><div className="flex shrink-0 gap-2"><button type="button" onClick={() => setShowCloseConfirm(false)} className="h-8 rounded-md border border-[#E7D9B9] bg-white px-3 text-xs font-bold text-[#8F5A00]">Hủy</button><button type="button" disabled={closing} onClick={onClose} className="inline-flex h-8 items-center gap-1.5 rounded-md bg-[#A86B00] px-3 text-xs font-bold text-white disabled:opacity-50">{closing && <LoaderCircle size={14} className="animate-spin" />}{closing ? "Đang đóng..." : "Đóng biên bản"}</button></div></div>}</div>}</article>;
}
...
export function RetirementCertificateManager() {
  ...
  const serviceCostByAsset = useMemo(() => {
    const result = new Map<number, ServiceCostInfo>();
    (maintenanceQuery.data || []).forEach((ticket) => {
      const current = result.get(ticket.assetId) || { warrantyTickets: [], repairTickets: [], totalWarrantyCost: 0, totalRepairCost: 0, totalActualCost: 0, hasWarrantyActualCost: false, hasRepairActualCost: false, hasActualCost: false };
      const ticketReference = { id: ticket.id, ticketCode: ticket.ticketCode };
      if (ticket.serviceChannel === "warranty") current.warrantyTickets.push(ticketReference);
      else current.repairTickets.push(ticketReference);
      if (ticket.actualCost !== null && ticket.actualCost !== undefined) {
        const actualCost = numeric(ticket.actualCost);
        current.totalActualCost += actualCost;
        current.hasActualCost = true;
        if (ticket.serviceChannel === "warranty") { current.totalWarrantyCost += actualCost; current.hasWarrantyActualCost = true; }
        else { current.totalRepairCost += actualCost; current.hasRepairActualCost = true; }
      }
  ...
  return (
    <section className="mt-5 overflow-hidden rounded-xl border border-[#D7E8E5] bg-white shadow-[0_8px_24px_rgba(16,42,67,0.045)]">
        ...
        <div className="min-w-0 rounded-xl border border-[#D7E8E5] bg-[#F9FDFC] p-4">
          ...
          <div className="mt-3 max-h-[286px] space-y-3 overflow-auto">
            {selectedIds.length ? selectedIds.map((assetId) => {
              ...
              return <div key={assetId} className="rounded-lg border border-[#D7E8E5] bg-white p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-xs font-extrabold text-[#193B57]">{asset.name}</div><div className="mt-1 font-mono text-[10px] text-[#8AA0B6]">{asset.assetCode}</div><ServiceCostIndicator info={serviceCostInfo} /></div><button type="button" onClick={() => toggleAsset(assetId)} className="grid h-7 w-7 shrink-0 place-items-center rounded-md text-[#71869A] hover:bg-[#FDEDEE] hover:text-[#B44545]" aria-label={`Bỏ chọn ${asset.name}`}><X size={14} /></button></div><ServiceCostReference info={serviceCostInfo} onPreview={(ticketId) => setPreviewRepairTicketId(ticketId)} /><label className="mt-3 block text-[10px] font-extrabold uppercase tracking-[.08em] text-[#60758A]">Lý do riêng</label><input value={detail.reason} onChange={(event) => updateItem(assetId, "reason", event.target.value)} className="field-input mt-1 h-9 w-full text-xs" /><div className="mt-2 grid gap-2 sm:grid-cols-2"><input value={detail.salvageValue} inputMode="decimal" onChange={(event) => /^\d*(\.\d{0,2})?$/.test(event.target.value) && updateItem(assetId, "salvageValue", event.target.value)} placeholder="Giá trị thu hồi chính thức (cập nhật sau)" className="field-input h-9 w-full text-xs" /><input value={detail.note} onChange={(event) => updateItem(assetId, "note", event.target.value)} placeholder="Ghi chú dòng (nếu có)" className="field-input h-9 w-full text-xs" /></div></div>;
      ...
      <div className="mt-3 space-y-3">{certificatesQuery.isLoading ? <div className="py-5 text-center text-xs text-[#71869A]">Đang tải biên bản...</div> : paginatedCertificates.length ? paginatedCertificates.map((certificate) => <CertificateCard key={certificate.id} certificate={certificate as Certificate} company={(companyQuery.data || {}) as Company} uploading={uploadSigned.isPending} closing={closeCertificate.isPending} cancelling={cancelDraft.isPending} onUpload={(file) => { void uploadSignedFile(certificate.id, file); }} onPrint={() => void printDraft(certificate as Certificate)} onClose={() => closeCertificate.mutateAsync({ id: certificate.id })} onCancel={() => cancelDraft.mutate({ id: certificate.id })} />) : <div className="rounded-lg border border-dashed border-[#D7E8E5] bg-white px-4 py-6 text-center text-xs text-[#71869A]">Không có biên bản phù hợp với bộ lọc hoặc từ khóa đã chọn.</div>}</div>
        <div className="mt-4 flex flex-col gap-3 border-t border-[#E7EEF3] bg-[#FBFCFD] pt-3 sm:flex-row sm:items-center sm:justify-between"><span className="text-[11px] text-[#71869A]">Hiển thị <b className="text-[#60758A]">{certificateStartRecord}–{certificateEndRecord}</b> trên <b className="text-[#60758A]">{filteredCertificates.length}</b> biên bản</span><div className="flex items-center gap-1"><button type="button" disabled={currentCertificatePage === 1} onClick={() => setCertificatePage(1)} title="Trang đầu" className="grid h-8 w-8 place-items-center rounded-md border border-[#DDE7F0] bg-white text-[#60758A] transition hover:bg-[#F0F5F8] disabled:cursor-not-allowed disabled:opacity-40" aria-label="Trang đầu"><ChevronsLeft size={16} /></button><button type="button" disabled={currentCertificatePage === 1} onClick={() => setCertificatePage((page) => Math.max(1, page - 1))} title="Trang trước" className="grid h-8 w-8 place-items-center rounded-md border border-[#DDE7F0] bg-white text-[#60758A] transition hover:bg-[#F0F5F8] disabled:cursor-not-allowed disabled:opacity-40" aria-label="Trang trước"><ChevronLeft size={16} /></button><span className="min-w-16 text-center text-[11px] font-bold text-[#60758A]">Trang {currentCertificatePage}/{certificatePageCount}</span><button type="button" disabled={currentCertificatePage === certificatePageCount} onClick={() => setCertificatePage((page) => Math.min(certificatePageCount, page + 1))} title="Trang sau" className="grid h-8 w-8 place-items-center rounded-md border border-[#DDE7F0] bg-white text-[#60758A] transition hover:bg-[#F0F5F8] disabled:cursor-not-allowed disabled:opacity-40" aria-label="Trang sau"><ChevronRight size={16} /></button><button type="button" disabled={currentCertificatePage === certificatePageCount} onClick={() => setCertificatePage(certificatePageCount)} title="Trang cuối" className="grid h-8 w-8 place-items-center rounded-md border border-[#DDE7F0] bg-white text-[#60758A] transition hover:bg-[#F0F5F8] disabled:cursor-not-allowed disabled:opacity-40" aria-label="Trang cuối"><ChevronsRight size={16} /></button></div></div>
      </div>
      {previewRepairTicket && <div className="fixed inset-0 z-[330] flex items-end justify-center bg-[#102A43]/45 p-0 backdrop-blur-[2px] sm:items-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setPreviewRepairTicketId(null); }}><section className="w-full max-w-xl rounded-t-2xl bg-white shadow-[0_24px_60px_rgba(16,42,67,0.28)] sm:rounded-2xl" role="dialog" aria-modal="true" aria-label={`Xem nhanh phiếu Bảo hành/Sửa chữa ${previewRepairTicket.ticketCode}`}><div className="assetmaster-modal-header flex shrink-0 items-start justify-between gap-4 border-b border-[#E7EEF3] px-4 pt-4 pb-5"><div><div className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#A86B00]">Phiếu Bảo hành/Sửa chữa</div><div className="mt-1 font-mono text-sm font-extrabold text-[#0F8C8C]">{previewRepairTicket.ticketCode}</div><h3 className="mt-1 text-base font-extrabold text-[#193B57]">{previewRepairAsset?.name || `Tài sản #${previewRepairTicket.assetId}`}</h3></div><button type="button" onClick={() => setPreviewRepairTicketId(null)} className="grid h-9 w-9 place-items-center rounded-lg text-xl text-[#60758A] hover:bg-[#F0F6FA]" aria-label="Đóng xem nhanh">×</button></div><div className="space-y-3 p-4"><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-lg border border-[#E0E8F0] bg-[#FBFCFE] p-3"><div className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#71869A]">Trạng thái</div><div className="mt-1 text-sm font-bold text-[#193B57]">{repairStatusLabel(previewRepairTicket.status)}</div></div><div className="rounded-lg border border-[#E0E8F0] bg-[#FBFCFE] p-3"><div className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#71869A]">Chi phí thực tế</div><div className="mt-1 text-sm font-extrabold text-[#193B57]">{previewRepairTicket.actualCost === null || previewRepairTicket.actualCost === undefined ? "Chưa cập nhật" : `${formatVnd(previewRepairTicket.actualCost)} VNĐ`}</div></div></div><div className="rounded-lg border border-[#E7EEF3] bg-[#FBFCFE] p-3"><div className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#71869A]">Nội dung yêu cầu</div><p className="mt-1 whitespace-pre-wrap text-xs leading-5 text-[#193B57]">{previewRepairTicket.description || "Chưa có mô tả."}</p></div>{previewRepairTicket.resolution && <div className="rounded-lg border border-[#CDE5E5] bg-[#F4FBFA] p-3"><div className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#087A6A]">Kết quả xử lý</div><p className="mt-1 whitespace-pre-wrap text-xs leading-5 text-[#193B57]">{previewRepairTicket.resolution}</p></div>}</div><div className="border-t border-[#E7EEF3] p-3 text-right"><button type="button" onClick={() => setPreviewRepairTicketId(null)} className="modal-close-action">Đóng</button></div></section></div>}
    </section>
  );
}
