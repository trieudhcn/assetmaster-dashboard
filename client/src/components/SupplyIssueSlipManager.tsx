...
function LegacyInlineHandoverPreviewDialog({ handoverId, onClose }: { handoverId: number | null; onClose: () => void }) {
  ...
  const recipientEmployee = handover ? (handover.recipientUserId ? employeesQuery.data?.find((employee) => employee.id === handover.recipientUserId) : employeesQuery.data?.find((employee) => employee.name?.trim() === handover.recipientName?.trim())) : undefined;
  const branchName = recipientEmployee?.branchId ? branchesQuery.data?.find((branch) => branch.id === recipientEmployee.branchId)?.name : handoverAsset?.branchId ? branchesQuery.data?.find((branch) => branch.id === handoverAsset.branchId)?.name : undefined;
...
export function InlineHandoverPreviewDialog({ handoverId, onClose }: { handoverId: number | null; onClose: () => void }) {
  ...
  const handoverAsset = handover ? assetsQuery.data?.find((asset) => asset.assetCode === handover.assetCode) : undefined;
  const recipientEmployee = handover ? (handover.recipientUserId ? employeesQuery.data?.find((employee) => employee.id === handover.recipientUserId) : employeesQuery.data?.find((employee) => employee.name?.trim() === handover.recipientName?.trim())) : undefined;
  const branch = recipientEmployee?.branchId ? branchesQuery.data?.find((item) => item.id === recipientEmployee.branchId) : handoverAsset?.branchId ? branchesQuery.data?.find((item) => item.id === handoverAsset.branchId) : undefined;
  const branchName = branch?.name;
  const branchEmail = branch?.email || undefined;
  const supplyItems = (handover?.supplyItems || []) as Array<{ id: number; supplyCode: string; supplyName: string; unit: string; issuedQuantity: string; returnedQuantity: string | null }>;
  const defaultFileBaseName = handover ? `${handover.referenceCode}-phieu-cap-phat-tai-san` : "bien-ban-ban-giao";
  const fileBaseName = handover ? fileNames[handover.id] ?? defaultFileBaseName : defaultFileBaseName;
