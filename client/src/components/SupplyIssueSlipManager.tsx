...
export function SupplyIssueSlipManager() {
  ...
  const returnItem = trpc.supplies.returnIssueItem.useMutation({
    onSuccess: (result) => {
      toast.success(result.fullyReturned ? "Đã hoàn trả toàn bộ phiếu về kho." : "Đã ghi nhận hoàn trả vật tư về kho.");
  ...
  const exportReport = async () => {
    ...
    const sheet = XLSX.utils.json_to_sheet(rows.map((row) => ({
      ...
      "Mã vật tư": row.supplyCode,
      "Tên vật tư": row.supplyName,
    ...
    XLSX.utils.book_append_sheet(workbook, sheet, "Lịch sử nhập xuất vật tư");
    await writeBrandedWorkbook(workbook, {
      documentTitle: "BÁO CÁO LỊCH SỬ NHẬP XUẤT VẬT TƯ",
      fileName: `lich-su-vat-tu-${new Date().toISOString().slice(0, 10)}.xlsx`,
      description: "Toàn bộ giao dịch nhập kho, cấp phát, hoàn trả và điều chỉnh tồn kho.",
    });
    toast.success("Đã tạo báo cáo Excel lịch sử vật tư.");
  };
    ...
    <>
      ...
      <aside className="fixed inset-y-0 right-0 z-[87] w-full max-w-xl overflow-y-auto border-l border-[#DFE9F0] bg-white p-5 shadow-2xl sm:p-6">
        ...
        <div className="mt-5 space-y-3">
          ...
          {!itemsQuery.isLoading && !(itemsQuery.data || []).length && <p className="rounded-lg border border-dashed border-[#DDE7F0] p-4 text-center text-sm text-[#8AA0B6]">Phiếu chưa có dòng vật tư.</p>}
        </div>
      </aside>
    </>
  ) : null;
  ...
  const returnDialog = returningItem ? (
    <>
      <button type="button" aria-label="Đóng hoàn trả vật tư" onClick={() => setReturnItemId(null)} className="fixed inset-0 z-[90] bg-[#102A43]/30 backdrop-blur-[1px]" />
      <section role="dialog" aria-modal="true" className="fixed left-1/2 top-1/2 z-[91] w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-5 shadow-2xl">
        <div className="flex justify-between gap-4"><div><div className="text-[10px] font-extrabold uppercase tracking-[.14em] text-[#0F8C8C]">Hoàn trả vật tư</div><h3 className="mt-1 font-display text-lg font-extrabold text-[#102A43]">{returningItem.supplyName}</h3></div><button type="button" onClick={() => setReturnItemId(null)} className="drawer-close-action"><X size={18} /></button></div>
