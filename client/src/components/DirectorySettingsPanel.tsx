...
import { CheckCircle2, CircleAlert, DatabaseZap, FileKey2, Loader2, Network, RefreshCw, ShieldCheck, UsersRound, X } from "lucide-react";
...
export function DirectorySettingsPanel({ onOpenUsers }: { onOpenUsers?: () => void }) {
  ...
  const [groupResults, setGroupResults] = useState<Array<{ dn: string; name: string; description: string | null }>>([]);
  const [syncResult, setSyncResult] = useState<{ scanned: number; synced: number; skipped: number; reachedLimit: boolean; users: Array<{ email: string; name: string | null; role: "admin" | "user"; status: "synced" | "skipped"; reason?: string }> } | null>(null);
  const [visibleSyncUsers, setVisibleSyncUsers] = useState(20);
  const [operationSteps, setOperationSteps] = useState<Array<{ label: string; state: "waiting" | "active" | "done" | "error" }>>([]);

  useEffect(() => {
    const openDirectory = () => {
      setIsVisible(true);
      window.requestAnimationFrame(() => window.requestAnimationFrame(() => document.getElementById("settings-directory")?.scrollIntoView({ behavior: "smooth", block: "start" })));
    };
    window.addEventListener("assetmaster:open-directory-settings", openDirectory);
    return () => window.removeEventListener("assetmaster:open-directory-settings", openDirectory);
  }, []);
  ...
  const testDraftMutation = trpc.directory.testDraft.useMutation({
    onMutate: () => setOperationSteps([{ label: "Chuẩn hóa thông số bản nháp", state: "done" }, { label: "Bắt tay TLS và kiểm tra CA", state: "active" }, { label: "Xác thực tài khoản bind", state: "waiting" }, { label: "Kiểm tra Users Base DN", state: "waiting" }]),
    onSuccess: (result) => { setOperationSteps(result.success ? [{ label: "Chuẩn hóa thông số bản nháp", state: "done" }, { label: "Bắt tay TLS và kiểm tra CA", state: "done" }, { label: "Xác thực tài khoản bind", state: "done" }, { label: "Kiểm tra Users Base DN", state: "done" }] : [{ label: "Không hoàn tất kiểm tra LDAPS", state: "error" }]); result.success ? toast.success(result.message) : toast.error(result.message); },
    onError: (error) => { setOperationSteps([{ label: "Không hoàn tất kiểm tra LDAPS", state: "error" }]); toast.error(error.message || "Không thể kiểm tra bản nháp LDAPS."); },
  });
  const groupSearchMutation = trpc.directory.searchGroups.useMutation({
    onSuccess: (groups) => { setGroupResults(groups); if (!groups.length) toast.message("Không tìm thấy nhóm phù hợp."); },
    onError: (error) => { setGroupResults([]); toast.error(error.message || "Không thể tìm kiếm nhóm LDAPS."); },
  });
  const syncUsersMutation = trpc.directory.syncUsers.useMutation({
    onMutate: () => { setSyncResult(null); setVisibleSyncUsers(20); setOperationSteps([{ label: "Xác thực kết nối và tài khoản bind", state: "done" }, { label: "Đọc các trang Directory, 100 tài khoản/lượt", state: "active" }, { label: "Ánh xạ nhóm sang quyền website", state: "waiting" }, { label: "Lưu metadata tài khoản, không lưu mật khẩu", state: "waiting" }]); },
    onSuccess: (result) => { setSyncResult(result); setOperationSteps([{ label: "Xác thực kết nối và tài khoản bind", state: "done" }, { label: "Đọc các trang Directory, 100 tài khoản/lượt", state: "done" }, { label: "Ánh xạ nhóm sang quyền website", state: "done" }, { label: "Lưu metadata tài khoản, không lưu mật khẩu", state: "done" }]); toast.success(`Đã đồng bộ ${result.synced} tài khoản Directory.`); },
    onError: (error) => { setOperationSteps([{ label: "Không hoàn tất đồng bộ Directory", state: "error" }]); toast.error(error.message || "Không thể đồng bộ tài khoản Directory."); },
  });
  ...
  if (!isVisible) return <div id="settings-directory" data-directory-settings-anchor aria-hidden="true" />;
  ...
  return <section id="settings-directory" data-directory-settings className="mx-auto mt-5 w-[calc(100%-2rem)] max-w-[1100px] rounded-xl border border-[#CDE5E5] bg-white shadow-[0_8px_24px_rgba(16,42,67,0.045)]">
    <header className="flex flex-col gap-4 border-b border-[#DDECEB] bg-[linear-gradient(120deg,#F6FCFB_0%,#FFFFFF_72%)] px-4 py-5 sm:px-6 lg:flex-row lg:items-start lg:justify-between">
      ...
      <div className="flex items-start gap-2"><div className="rounded-xl border border-[#DCEDEA] bg-white px-3 py-2.5 text-right shadow-sm"><div className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#8AA0B6]">Trạng thái môi trường</div><div className={`mt-1 text-xs font-extrabold ${isSelfHosted ? "text-[#087A6A]" : "text-[#A86B00]"}`}>{isSelfHosted ? "Self-hosted đã bật" : "Bản Manus — chỉ xem trước"}</div><div className="mt-1"><TestPill status={settings?.lastTestStatus} /></div></div><button type="button" onClick={() => setIsVisible(false)} className="grid h-9 w-9 place-items-center rounded-lg border border-[#DDE7F0] bg-white text-[#71869A] transition hover:bg-[#F4F7F9] hover:text-[#193B57]" aria-label="Ẩn cấu hình Directory"><X size={16} /></button></div>
    </header>
    ...
    <div className="p-4 sm:p-6">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.18fr)_minmax(320px,.82fr)]">
        <div className="space-y-4">
          ...
          <fieldset className="rounded-xl border border-[#E0E9EF] p-4"><legend className="px-1 text-xs font-extrabold text-[#193B57]">Ánh xạ nhóm quyền AssetMaster</legend><p className="mt-1 text-[11px] leading-5 text-[#71869A]">Mỗi email LDAPS chỉ truy cập khi thuộc một nhóm được ánh xạ. Nếu trùng hai nhóm, quyền Quản trị viên được ưu tiên.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-[#C9DDF5] bg-[#F4F9FF] p-3"><div className="flex items-center justify-between gap-2"><span className="text-[11px] font-extrabold text-[#2666A8]">Quản trị viên</span><span className="rounded-full bg-[#EAF3FF] px-2 py-0.5 text-[9px] font-extrabold text-[#2666A8]">Toàn quyền</span></div><InputField label="DN nhóm Admin" value={draft.adminGroupDn} onChange={(value) => update("adminGroupDn", value)} placeholder="CN=AssetMaster-Admins,OU=Groups,..." /><p className="mt-2 text-[10px] leading-4 text-[#60758A]">Cài đặt Directory, phân quyền, dữ liệu và các thao tác quản trị.</p></div><div className="rounded-xl border border-[#DDE7F0] bg-[#FBFCFD] p-3"><div className="flex items-center justify-between gap-2"><span className="text-[11px] font-extrabold text-[#193B57]">Nhân viên</span><span className="rounded-full bg-[#F0F5F8] px-2 py-0.5 text-[9px] font-extrabold text-[#60758A]">Quyền người dùng</span></div><InputField label="DN nhóm User" value={draft.userGroupDn} onChange={(value) => update("userGroupDn", value)} placeholder="CN=AssetMaster-Users,OU=Groups,..." /><p className="mt-2 text-[10px] leading-4 text-[#60758A]">Xem dashboard và dữ liệu theo phạm vi được cấp trong website.</p></div></div><div className="mt-3 rounded-lg border border-dashed border-[#DDE7F0] bg-[#FBFCFD] px-3 py-2.5 text-[10px] leading-4 text-[#71869A]"><b className="text-[#193B57]">Ngoài nhóm được ánh xạ:</b> từ chối đăng nhập. Quyền cấp trực tiếp cho Admin hiện hữu không tự bị hạ bởi lần đồng bộ LDAP.</div><label className="mt-3 flex cursor-pointer items-start gap-2 rounded-lg bg-[#F8FBFC] p-3 text-[11px] leading-5 text-[#60758A]"><input checked={draft.allowNestedGroups} onChange={(event) => update("allowNestedGroups", event.target.checked)} className="mt-0.5 h-4 w-4 accent-[#0F8C8C]" type="checkbox" /><span><b className="text-[#193B57]">Có nhóm lồng nhau</b><br />Chỉ bật nếu AD doanh nghiệp dùng nested groups và mô-đun LDAPS đã được cấu hình matching rule tương ứng.</span></label></fieldset>
        </div>
        ...
        <aside className="space-y-4"><section className="rounded-xl border border-[#DCEDEA] bg-[#F8FCFB] p-4"><div className="flex items-start gap-2"><ShieldCheck className="mt-0.5 text-[#087A6A]" size={18} /><div><h3 className="text-xs font-extrabold text-[#193B57]">Trình tự an toàn</h3><p className="mt-1 text-[11px] leading-5 text-[#60758A]">Lưu nháp, mount secret, kiểm tra kết nối thành công, rồi mới kích hoạt. Admin bootstrap vẫn là lối vào break-glass khi Directory gặp sự cố.</p></div></div><div className="mt-4 space-y-2 border-t border-[#DCEDEA] pt-3 text-[11px]"><div className="flex items-center justify-between gap-3"><span className="text-[#71869A]">Lần kiểm tra gần nhất</span><b className="text-right text-[#193B57]">{lastTest}</b></div>{settings?.lastTestMessage && <p className={`rounded-lg p-2.5 leading-5 ${settings.lastTestStatus === "success" ? "bg-[#E6F6F2] text-[#087A6A]" : "bg-[#FDEDEE] text-[#B44545]"}`}>{settings.lastTestMessage}</p>}</div></section>
          <section className="rounded-xl border border-[#E0E9EF] p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-xs font-extrabold text-[#193B57]">Kiểm tra và tìm nhóm</h3><p className="mt-1 text-[11px] leading-5 text-[#71869A]">Dùng chính thông số đang nhập, không lưu bản nháp khi kiểm tra.</p></div><button type="button" onClick={() => testDraftMutation.mutate(draft)} disabled={!isSelfHosted || testDraftMutation.isPending} title={!isSelfHosted ? "Chỉ khả dụng trên server self-hosted" : undefined} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[#CDE5E5] bg-white px-3 py-2 text-[10px] font-extrabold text-[#087A6A] disabled:opacity-50">{testDraftMutation.isPending && <Loader2 className="animate-spin" size={13} />}Kiểm tra bản nháp</button></div><div className="mt-3 flex gap-2"><input value={groupSearch} onChange={(event) => setGroupSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); groupSearchMutation.mutate({ settings: draft, query: groupSearch }); } }} placeholder="Tìm tên nhóm LDAPS" className="h-9 min-w-0 flex-1 rounded-lg border border-[#D7E3EB] bg-white px-3 text-[11px] font-medium text-[#193B57] outline-none placeholder:text-[#AAB9C6] focus:border-[#0F8C8C]" /><button type="button" onClick={() => groupSearchMutation.mutate({ settings: draft, query: groupSearch })} disabled={!isSelfHosted || groupSearchMutation.isPending} className="rounded-lg border border-[#DDE7F0] bg-white px-3 text-[10px] font-extrabold text-[#526779] disabled:opacity-50">Tìm</button></div>{groupResults.length > 0 && <div className="mt-3 max-h-48 space-y-2 overflow-y-auto pr-1">{groupResults.map((group) => <div key={group.dn} className="rounded-lg border border-[#E7EEF3] bg-white p-2.5"><p className="truncate text-[11px] font-extrabold text-[#193B57]" title={group.dn}>{group.name}</p>{group.description && <p className="mt-0.5 truncate text-[10px] text-[#71869A]">{group.description}</p>}<div className="mt-2 flex gap-2"><button type="button" onClick={() => update("adminGroupDn", group.dn)} className="rounded-md bg-[#EAF3FF] px-2 py-1 text-[9px] font-extrabold text-[#2666A8]">Gán Admin</button><button type="button" onClick={() => update("userGroupDn", group.dn)} className="rounded-md bg-[#EEF4F7] px-2 py-1 text-[9px] font-extrabold text-[#526779]">Gán User</button></div></div>)}</div>}</section>
          <section className="rounded-xl border border-[#E0E9EF] p-4"><div className="flex items-center gap-2"><UsersRound size={17} className="text-[#2666A8]" /><div><h3 className="text-xs font-extrabold text-[#193B57]">Đồng bộ và quản lý tài khoản</h3><p className="mt-1 text-[11px] leading-5 text-[#71869A]">Nạp tối đa 500 tài khoản, truy vấn theo lô 100 và tải thêm 20 kết quả/lần để rà soát mượt mà. Không đồng bộ mật khẩu nhân viên.</p></div></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => syncUsersMutation.mutate({ limit: 500 })} disabled={!isSelfHosted || syncUsersMutation.isPending || settings?.lastTestStatus !== "success"} title={settings?.lastTestStatus !== "success" ? "Cần lưu và kiểm tra LDAPS thành công trước" : undefined} className="inline-flex items-center gap-1.5 rounded-lg border border-[#C9DDF5] bg-[#F4F9FF] px-3 py-2 text-[10px] font-extrabold text-[#2666A8] disabled:opacity-50">{syncUsersMutation.isPending && <Loader2 className="animate-spin" size={13} />}Đồng bộ 500 tài khoản</button><button type="button" onClick={onOpenUsers} className="inline-flex items-center gap-1 text-[11px] font-extrabold text-[#2666A8] hover:text-[#1E5084]">Mở danh sách tài khoản <span aria-hidden="true">→</span></button></div>{syncResult && <p className="mt-3 rounded-lg bg-[#F6FCFB] p-2.5 text-[10px] leading-4 text-[#087A6A]">Đã quét {syncResult.scanned}; đồng bộ {syncResult.synced}; bỏ qua {syncResult.skipped}. Xem danh sách để kiểm tra quyền.</p>}</section>
      ...
      <DirectoryOperationProgress steps={operationSteps} result={syncResult} visibleCount={visibleSyncUsers} onLoadMore={() => setVisibleSyncUsers((current) => Math.min(current + 20, syncResult?.users.length || current))} />
...
function DirectoryOperationProgress({ steps, result, visibleCount, onLoadMore }: { steps: Array<{ label: string; state: "waiting" | "active" | "done" | "error" }>; result: { scanned: number; synced: number; skipped: number; reachedLimit: boolean; users: Array<{ email: string; name: string | null; role: "admin" | "user"; status: "synced" | "skipped"; reason?: string }> } | null; visibleCount: number; onLoadMore: () => void }) {
  if (!steps.length && !result) return null;
  const entries = result?.users.slice(0, visibleCount) || [];
  return <section className="mt-5 rounded-xl border border-[#DCEDEA] bg-[#FBFEFD] p-4" data-directory-operation-progress><div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-xs font-extrabold text-[#193B57]">Tiến trình Directory</h3><p className="mt-0.5 text-[11px] text-[#71869A]">Chỉ hiển thị trạng thái kỹ thuật an toàn; không hiển thị secret hoặc mật khẩu.</p></div>{steps.some((step) => step.state === "active") && <span className="inline-flex items-center gap-1.5 rounded-full bg-[#EAF3FF] px-2.5 py-1 text-[10px] font-extrabold text-[#2666A8]"><Loader2 className="animate-spin" size={12} />Đang xử lý</span>}</div><ol className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{steps.map((step) => <li key={step.label} className={`rounded-lg border px-2.5 py-2 text-[10px] font-bold ${step.state === "done" ? "border-[#CDE5E5] bg-[#F2FBF8] text-[#087A6A]" : step.state === "active" ? "border-[#C9DDF5] bg-[#F4F9FF] text-[#2666A8]" : step.state === "error" ? "border-[#F2CDCD] bg-[#FFF7F7] text-[#B44545]" : "border-[#E7EEF3] bg-white text-[#71869A]"}`}>{step.state === "active" ? "Đang thực hiện · " : step.state === "done" ? "Hoàn tất · " : step.state === "error" ? "Cần xử lý · " : "Chờ · "}{step.label}</li>)}</ol>{result && <><div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 rounded-lg bg-white px-3 py-2 text-[11px] text-[#526779]"><span>Đã quét: <b className="text-[#193B57]">{result.scanned}</b></span><span>Đồng bộ: <b className="text-[#087A6A]">{result.synced}</b></span><span>Bỏ qua: <b className="text-[#A86B00]">{result.skipped}</b></span>{result.reachedLimit && <span className="text-[#A86B00]">Đã chạm giới hạn lượt đồng bộ; có thể chạy lại.</span>}</div><div className="mt-3 overflow-hidden rounded-lg border border-[#E7EEF3] bg-white"><div className="grid grid-cols-[minmax(0,1fr)_72px_86px] gap-2 border-b border-[#E7EEF3] bg-[#F8FBFC] px-3 py-2 text-[9px] font-extrabold uppercase tracking-[.06em] text-[#71869A]"><span>Tài khoản</span><span>Quyền</span><span>Trạng thái</span></div>{entries.map((entry) => <div key={`${entry.email}-${entry.status}`} className="grid grid-cols-[minmax(0,1fr)_72px_86px] gap-2 border-b border-[#F0F4F6] px-3 py-2 text-[11px] last:border-b-0"><div className="min-w-0"><p className="truncate font-bold text-[#193B57]">{entry.name || entry.email}</p><p className="truncate text-[10px] text-[#71869A]">{entry.email}{entry.reason ? ` · ${entry.reason}` : ""}</p></div><span className="self-center text-[10px] font-bold text-[#526779]">{entry.role === "admin" ? "Admin" : "User"}</span><span className={`self-center text-[10px] font-extrabold ${entry.status === "synced" ? "text-[#087A6A]" : "text-[#A86B00]"}`}>{entry.status === "synced" ? "Đồng bộ" : "Bỏ qua"}</span></div>)}</div>{visibleCount < (result?.users.length || 0) && <button type="button" onClick={onLoadMore} className="mt-3 w-full rounded-lg border border-[#DDE7F0] bg-white px-3 py-2 text-[11px] font-extrabold text-[#2666A8] hover:bg-[#F4F9FF]">Tải thêm 20 tài khoản ({Math.min(visibleCount, result.users.length)}/{result.users.length})</button>}</>}</section>;
}
