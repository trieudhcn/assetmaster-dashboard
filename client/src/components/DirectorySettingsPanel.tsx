...
export function DirectorySettingsPanel({ onOpenUsers }: { onOpenUsers?: () => void }) {
  ...
  const [syncResult, setSyncResult] = useState<{ scanned: number; synced: number; skipped: number } | null>(null);
  ...
  const testDraftMutation = trpc.directory.testDraft.useMutation({
    ...
    onError: (error) => toast.error(error.message || "Không thể kiểm tra bản nháp LDAPS."),
  });
  ...
  const syncUsersMutation = trpc.directory.syncUsers.useMutation({
    onSuccess: (result) => { setSyncResult(result); toast.success(`Đã đồng bộ ${result.synced} tài khoản Directory.`); },
    onError: (error) => toast.error(error.message || "Không thể đồng bộ tài khoản Directory."),
  });
