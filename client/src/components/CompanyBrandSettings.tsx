...
export function CompanyBrandSettings({ companyInfo, onSave }: { companyInfo: CompanyBrandInfo; onSave: (next: CompanyBrandInfo) => void }) {
  ...
  const loginBackgroundStyle = draft.loginBackgroundUrl
    ? { backgroundImage: `url(${JSON.stringify(draft.loginBackgroundUrl)})` }
    : undefined;
    ...
    <div className="mx-auto max-w-[1100px]">
      ...
      <section className="rounded-xl border border-[#DFE9F0] bg-white p-6 shadow-[0_8px_24px_rgba(16,42,67,.045)]">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <div><label className="field-label">Tên công ty <span className="text-[#0F8C8C]">*</span></label><div className="relative"><input value={draft.name} onChange={(event) => update("name", event.target.value)} className="field-input pr-10" />{copyButton(draft.name, "Tên công ty")}</div></div>
          <div><label className="field-label inline-flex items-center gap-1">Tiêu đề website <span className="text-[#0F8C8C]">*</span><span className="group relative inline-flex"><CircleHelp size={14} className="text-[#8AA0B6]" aria-label="Thông tin Tiêu đề website" /><span role="tooltip" className="pointer-events-none absolute left-0 top-full z-30 mt-2 w-56 rounded-lg bg-[#193B57] px-2.5 py-2 text-[10px] font-medium leading-4 text-white opacity-0 shadow-lg transition group-hover:opacity-100">Hiển thị trên tab trình duyệt và khu vực nhận diện đăng nhập.</span></span></label><input value={draft.websiteTitle} onChange={(event) => update("websiteTitle", event.target.value)} placeholder="Hệ thống Quản lý Tài sản" className="field-input" /></div>
          ...
          <div><label className="field-label inline-flex items-center gap-1">Website công ty<span className="group relative inline-flex"><CircleHelp size={14} className="text-[#8AA0B6]" aria-label="Thông tin Website công ty" /><span role="tooltip" className="pointer-events-none absolute left-0 top-full z-30 mt-2 w-56 rounded-lg bg-[#193B57] px-2.5 py-2 text-[10px] font-medium leading-4 text-white opacity-0 shadow-lg transition group-hover:opacity-100">Hiển thị trong các tài liệu xuất, trừ khi bạn bật tùy chọn mẫu nội bộ.</span></span></label><div className="relative"><input type="url" value={draft.websiteUrl} onChange={(event) => update("websiteUrl", event.target.value)} placeholder="https://congty.vn" className="field-input pr-11" /><label className="group absolute right-3 top-1/2 flex -translate-y-1/2 cursor-pointer items-center"><input type="checkbox" checked={draft.hideWebsiteOnInternalPdf} onChange={(event) => update("hideWebsiteOnInternalPdf", event.target.checked)} className="h-4 w-4 rounded border-[#8BCDC6] text-[#0F8C8C] focus:ring-[#0F8C8C]" aria-label="Ẩn Website trên mẫu nội bộ" /><span role="tooltip" className="pointer-events-none absolute bottom-full right-0 z-30 mb-2 w-56 rounded-lg bg-[#193B57] px-2.5 py-2 text-[10px] font-medium leading-4 text-white opacity-0 shadow-lg transition group-hover:opacity-100">Ẩn Website trên các tài liệu xuất dùng trong nội bộ.</span></label></div></div>
      ...
      <section id="settings-login-identity" className="mt-5 scroll-mt-24 rounded-xl border border-[#DFE9F0] bg-white p-6 shadow-[0_8px_24px_rgba(16,42,67,.045)]">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1.18fr_.82fr]">
          <div className="overflow-hidden rounded-xl border border-dashed border-[#9ADBD3] bg-[#F8FCFB]"><div data-login-background-preview data-overlay={draft.loginBackgroundOverlay} className="relative h-44 bg-[#EAF3FF] bg-cover bg-center" style={loginBackgroundStyle}>{draft.loginBackgroundUrl ? <div aria-hidden="true" className={`absolute inset-0 transition-colors ${draft.loginBackgroundOverlay === "dark" ? "bg-[#102A43]/75" : "bg-white/55"}`} /> : null}<div className={`absolute inset-x-4 bottom-4 z-10 text-xs font-bold ${draft.loginBackgroundOverlay === "dark" ? "text-white" : "text-[#102A43]"}`}>{draft.loginBackgroundUrl ? `Bản xem trước · Lớp phủ ${draft.loginBackgroundOverlay === "dark" ? "tối" : "sáng"}` : "Chưa có ảnh nền tùy chỉnh"}</div></div><div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-xs font-extrabold text-[#193B57]">Ảnh nền đăng nhập</div><p className="mt-1 text-[11px] leading-5 text-[#71869A]">PNG, JPG hoặc WebP · tối đa 5 MB.</p></div><div className="flex shrink-0 gap-2"><button disabled={uploadLoginBackground.isPending} onClick={() => backgroundInputRef.current?.click()} className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#8BCDC6] bg-white px-3 py-2 text-xs font-bold text-[#087A6A] disabled:opacity-50">{uploadLoginBackground.isPending ? <LoaderCircle size={14} className="animate-spin" /> : <Upload size={14} />}{uploadLoginBackground.isPending ? "Đang tải..." : "Tải ảnh nền"}</button>{draft.loginBackgroundUrl ? <button type="button" onClick={() => { update("loginBackgroundUrl", ""); toast.success("Đã gỡ ảnh nền. Hãy lưu cài đặt để áp dụng nền mặc định."); }} className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-[#F2C7C7] bg-white px-3 py-2 text-xs font-bold text-[#B44545]"><X size={14} />Gỡ ảnh nền</button> : null}</div><input ref={backgroundInputRef} type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => { chooseBackground(event.target.files?.[0]); event.currentTarget.value = ""; }} /></div></div>
          <div className="space-y-4"><div><label className="field-label">Lớp phủ ảnh nền</label><div className="grid grid-cols-2 gap-2"><button type="button" aria-pressed={draft.loginBackgroundOverlay === "light"} onClick={() => update("loginBackgroundOverlay", "light")} className={`flex items-center justify-center gap-2 rounded-lg border px-3 py-2.5 text-xs font-bold transition ${draft.loginBackgroundOverlay === "light" ? "border-[#8BCDC6] bg-[#ECF8F7] text-[#087A6A]" : "border-[#DDE7F0] bg-white text-[#60758A]"}`}><Sun size={15} />Sáng</button><button type="button" aria-pressed={draft.loginBackgroundOverlay === "dark"} onClick={() => update("loginBackgroundOverlay", "dark")} className={`flex items-center justify-center gap-2 rounded-lg border px-3 py-2.5 text-xs font-bold transition ${draft.loginBackgroundOverlay === "dark" ? "border-[#294F6A] bg-[#173A56] text-white" : "border-[#DDE7F0] bg-white text-[#60758A]"}`}><Moon size={15} />Tối</button></div><p className="mt-2 text-[11px] leading-5 text-[#8AA0B6]">Bản xem trước thay đổi ngay; nhấn “Áp dụng nhận diện đăng nhập” để lưu.</p></div><div><label className="field-label">Câu chào màn hình đăng nhập</label><textarea value={draft.loginGreeting} maxLength={300} onChange={(event) => update("loginGreeting", event.target.value)} placeholder="Quản lý tài sản, theo đúng vai trò của bạn." className="field-input min-h-[132px] resize-y" /><div className="mt-1 flex justify-between text-[11px] text-[#8AA0B6]"><span>Hiển thị ở phần nổi bật của màn hình đăng nhập.</span><span>{draft.loginGreeting.length}/300</span></div></div></div>
        </div>
