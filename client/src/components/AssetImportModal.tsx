...
import { configureAssetImportTemplate, uniqueTemplateNames } from "@/lib/assetImportTemplate";
...
const statusLabel = (status: AssetImportCandidate["status"]) => status === "maintenance" ? "Bảo trì" : "Sẵn có";
const conditionLabel = (condition: AssetImportCandidate["condition"]) => ({ good: "Tốt", fair: "Khá", needs_inspection: "Cần kiểm tra", damaged: "Hư hỏng" })[condition];
const displayDate = (value: unknown) => value ? new Date(value as string | number).toLocaleDateString("vi-VN") : "—";
const displayMoney = (value: unknown) => value ? `${Number(value).toLocaleString("vi-VN")} VNĐ` : "—";
...
export function AssetImportModal({ onClose, onImported }: { onClose: () => void; onImported: () => void }) {
  ...
  const [phase, setPhase] = useState<"idle" | "reading" | "validating" | "ready" | "importing" | "complete">("idle");
  ...
  const closeModal = () => {
    if (phase === "reading" || phase === "validating" || phase === "importing") return;
    if (parsed || draftRows.length) {
      toast.warning("Đóng phiên import?", { description: "Dữ liệu xem trước chưa import sẽ bị hủy.", action: { label: "Bỏ dữ liệu", onClick: onClose } });
      return;
    }
    onClose();
  };
  ...
  const importMutation = trpc.assets.import.useMutation({
    onSuccess: (result) => {
      ...
      if (result.created || result.updated) {
        toast.success(`Đã tạo ${result.created} và cập nhật ${result.updated} tài sản.`, { description: "Bạn có thể hoàn tác trong Lịch sử import ở hàng bộ lọc Danh mục tài sản.", action: { label: "Mở Lịch sử", onClick: () => window.dispatchEvent(new Event("assetmaster:open-import-history")) } });
        onImported();
    ...
    onError: (error) => {
      setPhase("ready");
      setProgress(100);
      setStatusDetail("Import chưa hoàn tất; dữ liệu được giữ nguyên do transaction đã rollback.");
  ...
  const processFile = (file?: File) => {
    ...
    setParsed(null); setDraftRows([]); setServerIssues([]); setPreviewPage(1); setPhase("reading"); setProgress(8); setStatusDetail("Đang đọc tệp Excel...");
    const reader = new FileReader();
    reader.onprogress = (event) => { if (event.lengthComputable) setProgress(Math.max(8, Math.min(40, Math.round((event.loaded / event.total) * 40)))); };
    reader.onload = () => {
      try {
        setPhase("validating"); setProgress(50); setStatusDetail("Đang kiểm tra sheet, header, ngày tháng và số tiền...");
        ...
        if (!validateAssetImportHeaders(headerRow).valid) throw new Error("header");
        ...
        setParsed({ ...result, fileName: file.name, sourceRows: rows.length, rawRows: rows });
        setDraftRows(result.candidates);
        setProgress(75); setPhase("ready");
        setStatusDetail(result.issues.length ? `Đã đọc ${rows.length} dòng; ${result.issues.length} dòng lỗi cần xử lý trước khi import.` : `Đã kiểm tra ${rows.length} dòng. Sẵn sàng xem trước và import.`);
      } catch (error) {
        ...
        setStatusDetail(message); toast.error(message);
      }
    };
    reader.onerror = () => { setPhase("idle"); setProgress(0); setStatusDetail("Không thể đọc tệp đã chọn."); toast.error("Không thể đọc tệp đã chọn."); };
  ...
  const allPreviewRows = draftRows.map((row) => {
    const matches = row.serialNumber ? existingBySerial.get(row.serialNumber.trim()) || [] : [];
    const action: ImportAction = matches.length > 1 ? "conflict" : updateExisting && matches.length === 1 ? "update" : "create";
  ...
  const previewRows = allPreviewRows.slice((activePreviewPage - 1) * 10, activePreviewPage * 10);
  ...
  const conflicts = allPreviewRows.filter((row) => row.action === "conflict").length;
  ...
  const rowsForImport = allPreviewRows.map(({ action, existingAsset, ...row }) => row);
  const isWorking = phase === "reading" || phase === "validating" || phase === "importing";
  ...
  const doImport = () => {
    ...
    setPhase("importing"); setProgress(84); setStatusDetail(`Đang ghi ${rowsForImport.length} dòng trong một transaction an toàn...`);
  ...
  const exportErrors = () => {
    ...
    window.setTimeout(() => { void (async () => {
      try {
        ...
        const book = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(book, XLSX.utils.json_to_sheet(rows), "Dòng lỗi");
        await writeBrandedWorkbook(book, { documentTitle: "DANH SÁCH DÒNG LỖI IMPORT TÀI SẢN", fileName: `AssetMaster-Loi-Import-${new Date().toISOString().slice(0, 10)}.xlsx`, description: `${rows.length} dòng cần chỉnh sửa trước khi nhập lại vào hệ thống.` });
        toast.success(`Đã xuất ${rows.length} dòng lỗi ra Excel.`, { id: loadingToast });
      } catch (error) {
        console.error(error); toast.error("Không thể xuất tệp Excel lỗi. Vui lòng thử lại.", { id: loadingToast });
      } finally { setIsExportingErrors(false); }
    })(); }, 180);
  };
      ...
      <div className="max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-2xl border border-[#DDE7F0] bg-white shadow-[0_24px_70px_rgba(16,42,67,0.24)]">
        ...
        <div className="space-y-5 p-6">
          ...
          <section className="rounded-xl border border-dashed border-[#9ADBD3] bg-[#FBFEFD] p-4 text-center">
            ...
            <button disabled={isWorking} onClick={() => inputRef.current?.click()} className="mt-3 rounded-lg bg-[#0F8C8C] px-4 py-2 text-xs font-bold text-white disabled:opacity-60">Chọn tệp Excel</button>
            <input ref={inputRef} className="sr-only" type="file" accept=".xlsx" onChange={(event) => { processFile(event.target.files?.[0]); event.currentTarget.value = ""; }} />
          </section>
          <section className="rounded-xl border border-[#DCEDEA] bg-[#F9FEFD] p-4" aria-live="polite">
            <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2 text-xs font-extrabold text-[#087A6A]">{isWorking ? <LoaderCircle size={15} className="animate-spin" /> : phase === "complete" ? <CheckCircle2 size={15} /> : <FileSpreadsheet size={15} />}{statusDetail}</div><span className="shrink-0 text-xs font-extrabold text-[#4B8884]">{progress}%</span></div>
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#DCEDEA]"><div className="h-full rounded-full bg-[#0F8C8C] transition-[width] duration-300" style={{ width: `${progress}%` }} /></div>
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{progressSteps.map((step) => <div key={step.label} className={`rounded-md px-2 py-1.5 text-[10px] font-bold ${step.complete ? "bg-[#E6F6F2] text-[#087A6A]" : "bg-white text-[#8AA0B6]"}`}>{step.complete ? "✓ " : "○ "}{step.label}</div>)}</div>
          </section>
          {phase === "reading" || phase === "validating" ? <section className="overflow-hidden rounded-xl border border-[#DFE9F0] bg-white"><ModalTableSkeleton rows={6} columns={7} /></section> : null}
          {parsed ? <>
            <section className="rounded-xl border border-[#CDE5E5] bg-[#F9FEFD] p-4">
              <label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={updateExisting} onChange={(event) => setUpdateExisting(event.target.checked)} disabled={isWorking} className="mt-0.5 h-4 w-4 rounded border-[#8BCDC6] accent-[#0F8C8C]" /><span><span className="block text-sm font-extrabold text-[#193B57]">Tự động cập nhật tài sản trùng Serial/IMEI</span><span className="mt-1 block text-xs leading-5 text-[#4B8884]">Khi bật, một Serial/IMEI khớp duy nhất sẽ được cập nhật. Khi tắt, dòng đó được tạo như tài sản mới. Luôn xem bảng so sánh bên dưới trước khi xác nhận.</span></span></label>
              <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-lg bg-white p-2"><b className="block text-[#087A6A]">{creates}</b><span className="text-[#60758A]">Tạo mới</span></div><div className="rounded-lg bg-white p-2"><b className="block text-[#2666A8]">{updates}</b><span className="text-[#60758A]">Cập nhật</span></div><div className="rounded-lg bg-white p-2"><b className="block text-[#A86B00]">{conflicts}</b><span className="text-[#60758A]">Xung đột Serial</span></div></div>
            </section>
            ...
            {updateRows.length ? <section className="overflow-hidden rounded-xl border border-[#B9D7F5] bg-[#F7FBFF]"><div className="border-b border-[#DDEBFA] px-4 py-3"><div className="text-sm font-extrabold text-[#193B57]">Trường sẽ thay đổi khi cập nhật</div><p className="mt-1 text-xs text-[#4E7CAA]">Chỉ các dòng Serial/IMEI khớp duy nhất mới xuất hiện tại đây.</p></div><div className="max-h-72 overflow-auto"><table className="w-full min-w-[780px] text-left text-xs"><thead className="bg-[#EAF3FF] text-[10px] uppercase tracking-[.08em] text-[#376EAA]"><tr><th className="px-3 py-3">Tài sản</th><th className="px-3 py-3">Serial/IMEI</th><th className="px-3 py-3">Trường</th><th className="px-3 py-3">Hiện tại</th><th className="px-3 py-3">Sau import</th></tr></thead><tbody>{updateRows.flatMap((row) => {
              ...
              const currentCategory = (categoriesQuery.data || []).find((item) => item.id === current?.categoryId)?.name || "—";
              const currentBrand = (brandsQuery.data || []).find((item) => item.id === current?.brandId)?.name || "—";
              const changes = [
                ["Tên tài sản", current?.name || "—", row.name], ["Phân loại", currentCategory, row.category], ["Trạng thái", current?.status === "maintenance" ? "Bảo trì" : "Sẵn có", statusLabel(row.status)], ["Tình trạng", current?.condition ? conditionLabel(current.condition) : "—", conditionLabel(row.condition)], ["Ngày mua", displayDate(current?.purchaseDate), displayDate(row.purchaseDate)], ["Giá trị", displayMoney(current?.purchaseValue), displayMoney(row.purchaseValue)], ["Nhà cung cấp", current?.vendor || "—", row.vendor || "—"], ["Hãng", currentBrand, row.brandName || "—"], ["Vị trí", current?.location || "—", row.location || "—"], ["Hạn bảo hành", displayDate(current?.warrantyUntil), displayDate(row.warrantyUntil)],
              ].filter(([, before, after]) => String(before) !== String(after));
              return changes.length ? changes.map(([field, before, after], index) => <tr key={`${row.rowNumber}-${field}`} className="border-t border-[#E4EFFB]"><td className="px-3 py-2 text-[#193B57]">{index === 0 ? current?.name : ""}</td><td className="px-3 py-2 font-mono text-[#60758A]">{index === 0 ? row.serialNumber : ""}</td><td className="px-3 py-2 font-semibold text-[#376EAA]">{field}</td><td className="px-3 py-2 text-[#8A6C55]">{before}</td><td className="px-3 py-2 font-semibold text-[#087A6A]">{after}</td></tr>) : <tr key={`${row.rowNumber}-unchanged`} className="border-t border-[#E4EFFB]"><td className="px-3 py-2 text-[#193B57]">{current?.name}</td><td className="px-3 py-2 font-mono text-[#60758A]">{row.serialNumber}</td><td className="px-3 py-2 text-[#60758A]" colSpan={3}>Không có trường nào thay đổi.</td></tr>;
            })}</tbody></table></div></section> : null}
          </> : null}
          {issues.length ? <section className="rounded-xl border border-[#F2D596] bg-[#FFF9EB] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2 text-xs font-extrabold text-[#8F5A00]"><AlertTriangle size={15} />Các dòng cần xử lý ({issues.length})</div><button onClick={exportErrors} disabled={isExportingErrors} className="inline-flex items-center gap-2 rounded-lg border border-[#E7C981] bg-white px-3 py-2 text-xs font-bold text-[#A86B00] disabled:opacity-60"><Download size={14} />{isExportingErrors ? "Đang xuất..." : "Xuất Excel lỗi"}</button></div><div className="mt-3 max-h-28 space-y-1 overflow-auto text-xs leading-5 text-[#9B7131]">{issues.slice(0, 20).map((issue, index) => <div key={`${issue.rowNumber}-${index}`}><b>Dòng {issue.rowNumber}:</b> {issue.message}</div>)}</div></section> : null}
          <footer className="flex flex-col-reverse justify-end gap-2 border-t border-[#E7EEF3] pt-4 sm:flex-row"><button onClick={closeModal} disabled={isWorking} className="rounded-lg border border-[#DDE7F0] px-4 py-2 text-xs font-bold text-[#60758A]">Đóng</button><button disabled={!rowsForImport.length || isWorking || draftIssues.length > 0 || conflicts > 0} onClick={doImport} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#0F8C8C] px-4 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><Upload size={15} />{phase === "importing" ? "Đang import an toàn..." : `Xác nhận import ${rowsForImport.length} tài sản`}</button></footer>
        </div>
      </div>
    </div>
  );
