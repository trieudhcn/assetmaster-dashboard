    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[#102A43]/45 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Import tài sản từ Excel">
      <div className="max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-2xl border border-[#DDE7F0] bg-white shadow-[0_24px_70px_rgba(16,42,67,0.24)]">
        ...
        <div className="space-y-5 p-6">
          ...
          <footer className="flex flex-col-reverse justify-end gap-2 border-t border-[#E7EEF3] pt-4 sm:flex-row"><button onClick={closeModal} disabled={isWorking} className="modal-close-action">Đóng</button><button disabled={!rowsForImport.length || isWorking || draftIssues.length > 0 || conflicts > 0} onClick={doImport} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#0F8C8C] px-4 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><Upload size={15} />{phase === "importing" ? "Đang import an toàn..." : `Xác nhận import ${rowsForImport.length} tài sản`}</button></footer>
        </div>
      </div>
    </div>
  );
