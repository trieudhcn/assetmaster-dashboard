...
export function QuickServiceTicketPreview({ ticket, asset, assigneeName, currencyMode, pdfPreparing, onPrint, onExportPdf, onClose }: QuickServiceTicketPreviewProps) {
  ...
  return <div className="fixed inset-0 z-[180] flex items-end justify-center bg-[#102A43]/45 p-4 backdrop-blur-[2px] sm:items-center" role="presentation" onMouseDown={closeOnBackdrop}>
    <section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-[0_24px_60px_rgba(16,42,67,0.28)]" role="dialog" aria-modal="true" aria-label={`Xem nhanh phiếu ${ticket.ticketCode}`}>
      <div className="flex items-start justify-between gap-4 border-b border-[#E7EEF3] px-5 py-4">
        <div>
          ...
          <h2 className="mt-1 text-lg font-extrabold text-[#193B57]">Xem nhanh phiếu Bảo hành/Sửa chữa</h2>
        </div>
      ...
      <div className="flex flex-col-reverse gap-2 border-t border-[#E7EEF3] px-5 py-4 sm:flex-row sm:items-center sm:justify-end">
