...
export function LatestImportUndo({ onUndone }: { onUndone: () => void }) {
  ...
  useEffect(() => {
    if (!session || session.isUndone || session.createdCount + session.updatedCount === 0) { setIsCollapsed(false); return; }
    setIsCollapsed(false);
    const timer = window.setTimeout(() => setIsCollapsed(true), AUTO_COLLAPSE_MS);
    return () => window.clearTimeout(timer);
  }, [session?.id, session?.isUndone]);
  useEffect(() => {
    if (!confirm) return;
  ...
  return <>{isCollapsed ? <button type="button" onClick={() => setIsCollapsed(false)} aria-label="Mở thẻ hoàn tác import" className={`fixed bottom-5 left-5 z-[85] inline-flex items-center gap-2 rounded-full border bg-white px-3 py-2 text-xs font-extrabold shadow-[0_12px_28px_rgba(16,42,67,0.18)] transition hover:-translate-y-0.5 ${isExpiringSoon ? "border-[#F2B18B] text-[#C75419]" : "border-[#E7C981] text-[#A86B00]"}`}><Clock3 size={15} /><span>Hoàn tác import</span><ChevronUp size={14} /></button> : <aside className={`fixed bottom-5 left-5 z-[85] w-[min(23rem,calc(100vw-2.5rem))] rounded-xl border bg-white p-4 shadow-[0_16px_42px_rgba(16,42,67,0.20)] ${isExpiringSoon ? "border-[#E87D34]" : canUndo ? "border-[#F2D596]" : "border-[#DDE7F0]"}`}><div className="flex items-start gap-3"><div className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg ${isExpiringSoon ? "bg-[#FDEDE4] text-[#C75419]" : canUndo ? "bg-[#FFF5DC] text-[#A86B00]" : "bg-[#F0F5F8] text-[#71869A]"}`}>{isExpiringSoon ? <AlertTriangle size={18} /> : <Clock3 size={18} />}</div><div className="min-w-0 flex-1"><div className="text-xs font-extrabold text-[#193B57]">{isExpiringSoon ? "Sắp hết hạn hoàn tác import" : canUndo ? "Có thể hoàn tác import gần nhất" : "Đã hết hạn hoàn tác import"}</div><p className="mt-1 text-[11px] leading-5 text-[#71869A]">{session.referenceCode} · tạo {session.createdCount}, cập nhật {session.updatedCount} tài sản.</p><p className={`mt-1 text-[10px] font-bold ${isExpiringSoon ? "text-[#C75419]" : canUndo ? "text-[#A86B00]" : "text-[#8AA0B6]"}`}>{isExpiringSoon ? `Còn khoảng ${remainingHours} giờ. Hãy kiểm tra và hoàn tác nếu cần.` : canUndo ? `Cho phép hoàn tác đến ${deadline}` : `Hết hiệu lực từ ${deadline}`}</p></div><button type="button" onClick={() => setIsCollapsed(true)} aria-label="Thu nhỏ thẻ hoàn tác import" className="grid h-7 w-7 shrink-0 place-items-center rounded-md text-[#8AA0B6] transition hover:bg-[#F0F5F8] hover:text-[#526779]"><ChevronDown size={17} /></button></div><button disabled={!canUndo} onClick={() => setConfirm(true)} className={`mt-3 inline-flex w-full items-center justify-center gap-2 rounded-lg border px-3 py-2 text-xs font-bold disabled:cursor-not-allowed disabled:border-[#DDE7F0] disabled:bg-[#F7FAFC] disabled:text-[#8AA0B6] ${isExpiringSoon ? "border-[#F2B18B] bg-[#FFF2E9] text-[#C75419] hover:bg-[#FDEDE4]" : "border-[#E7C981] bg-[#FFF9EB] text-[#A86B00] hover:bg-[#FFF5DC]"}`}><RotateCcw size={14} />{canUndo ? "Hoàn tác phiên import này" : "Thời hạn hoàn tác đã kết thúc"}</button></aside>}{confirm && <div className="fixed inset-0 z-[95] grid place-items-center bg-[#102A43]/50 px-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#FFF5DC] text-[#A86B00]"><AlertTriangle size={20} /></div><div><h3 className="font-display text-lg font-extrabold text-[#102A43]">Hoàn tác import gần nhất?</h3><p className="mt-2 text-sm leading-6 text-[#60758A]">Các tài sản được tạo sẽ được lưu trữ; các tài sản được cập nhật sẽ khôi phục lại giá trị trước import. Thời hạn thao tác đến {deadline}.</p></div></div><div className="mt-6 flex justify-end gap-2"><button onClick={() => setConfirm(false)} className="rounded-lg border border-[#DDE7F0] px-4 py-2 text-xs font-bold text-[#60758A]">Hủy</button><button disabled={undo.isPending} onClick={() => undo.mutate({ sessionId: session.id })} className="inline-flex items-center gap-2 rounded-lg bg-[#B76C00] px-4 py-2 text-xs font-bold text-white disabled:opacity-50"><RotateCcw size={14} />{undo.isPending ? "Đang hoàn tác..." : "Xác nhận hoàn tác"}</button></div></div></div>}</>;
}
