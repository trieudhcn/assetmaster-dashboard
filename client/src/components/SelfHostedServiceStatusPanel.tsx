...
export function SelfHostedServiceStatusPanel() {
  ...
  const visibility = useHiddenSelfHostedSettingsPanel(
    ...
    "assetmaster:open-self-hosted-service-status"
  );
  ...
  const directoryQuery = trpc.directory.get.useQuery(undefined, {
    enabled:
      isAdmin && visibility.isVisible && healthQuery.data?.selfHosted === true,
  });
  ...
  if (!visibility.isVisible)
    return <HiddenSelfHostedSettingsAnchor id="self-hosted-service-status" />;
  if (healthQuery.isSuccess && !healthQuery.data.selfHosted) return null;

  return (
    <SelfHostedSettingsPanelFrame
      id="self-hosted-service-status"
      eyebrow="Giám sát self-hosted"
      title="Trạng thái hạ tầng"
      description="Kiểm tra an toàn MySQL, Redis và LDAPS; không hiển thị host, password hoặc Docker secret."
      icon={ServerCog}
      iconClassName="bg-[#EAF3FF] text-[#2666A8]"
    ...
    >
      {healthQuery.isLoading ? (
        ...
        >
          ...
          Không thể đọc trạng thái dịch vụ. Hãy làm mới hoặc xem log container.
        </div>
