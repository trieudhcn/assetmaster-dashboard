...
export function DatePickerField({ value, onChange, className = "", disabled, placeholder = "Chọn ngày", "aria-label": ariaLabel, "data-purchase-date-picker": purchaseDatePicker }: DatePickerFieldProps) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" sideOffset={8} className="z-[180] w-[296px] overflow-hidden rounded-xl border border-[#CDE5E5] bg-white p-0 shadow-[0_18px_44px_rgba(16,42,67,0.16)]">
        <div className="flex items-center justify-between border-b border-[#E7EEF3] bg-[#F7FCFC] px-4 py-3">
          <div>
            <div className="text-[10px] font-extrabold uppercase tracking-[0.13em] text-[#0F8C8C]">Lịch AssetMaster</div>
            <div className="mt-0.5 text-xs font-bold text-[#193B57]">{selectedDate ? selectedDate.toLocaleDateString("vi-VN", { weekday: "long", day: "2-digit", month: "long", year: "numeric" }) : "Chọn ngày cần áp dụng"}</div>
          </div>
        ...
        <div className="p-3">
          <Calendar
            ...
            className="w-full p-0 [--cell-size:2.25rem] [&_[data-selected-single=true]]:bg-[#0F8C8C] [&_[data-selected-single=true]]:text-white [&_[data-selected-single=true]]:shadow-[0_4px_10px_rgba(15,140,140,0.25)] [&_.rdp-button:hover]:bg-[#E6F6F2] [&_.rdp-caption_label]:font-extrabold [&_.rdp-caption_label]:font-extrabold [&_.rdp-caption_label]:text-[#193B57] [&_.rdp-dropdown]:rounded-md [&_.rdp-dropdown]:border-[#CDE5E5] [&_.rdp-dropdown]:bg-white [&_.rdp-dropdown]:px-1.5 [&_.rdp-dropdown]:py-1 [&_.rdp-dropdown]:text-xs [&_.rdp-dropdown]:font-bold [&_.rdp-dropdown]:text-[#193B57] [&_.rdp-dropdown]:outline-none [&_.rdp-dropdown]:focus:ring-2 [&_.rdp-dropdown]:focus:ring-[#A9DDD6] [&_.rdp-caption_label]:font-extrabold [&_.rdp-caption_label]:text-[#193B57] [&_.rdp-weekday]:text-[10px] [&_.rdp-weekday]:font-extrabold [&_.rdp-weekday]:uppercase [&_.rdp-weekday]:tracking-[0.08em] [&_.rdp-weekday]:text-[#8AA0B6]"
          />
        </div>
        <div className="flex items-center justify-between border-t border-[#E7EEF3] px-4 py-2.5">
          <button type="button" onClick={() => { onChange(""); setOpen(false); }} className="text-[11px] font-bold text-[#60758A] transition hover:text-[#087A6A]">Xóa ngày</button>
          <button type="button" onClick={() => { const today = new Date(); onChange(toIsoDate(today)); setOpen(false); }} className="text-[11px] font-extrabold text-[#087A6A] transition hover:text-[#0A6666]">Hôm nay</button>
        </div>
      </PopoverContent>
    </Popover>
  );
