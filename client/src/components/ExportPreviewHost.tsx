...
export function ExportPreviewHost() {
  ...
  useEffect(() => {
    ...
    payload.blob.arrayBuffer().then((buffer) => {
      ...
      const rows = previewSheet ? XLSX.utils.sheet_to_json<unknown[]>(previewSheet, { header: 1, defval: "" }).slice(0, 24).map((row) => row.slice(0, 12)) : [];
      if (active) setExcelPreview({ sheets: workbook.SheetNames, rows, activeSheet: previewSheetName });
    }).catch(() => { if (active) setExcelPreview({ sheets: [], rows: [], activeSheet: "" }); });
    return () => { active = false; };
  }, [payload]);
  ...
  return <div onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }} className="fixed inset-0 z-[100] flex items-center justify-center bg-[#102A43]/55 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Xem trước ${payload.title}`}>
    <div className="flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-[#DDE7F0] bg-white shadow-[0_26px_70px_rgba(16,42,67,0.28)]">
      ...
      <div className="min-h-0 flex-1 overflow-auto bg-[#F4F7FB] p-4 sm:p-5">{payload.kind === "pdf" ? <iframe title={`Xem trước ${payload.title}`} src={fileUrl || undefined} className="h-[68vh] w-full rounded-xl border border-[#DDE7F0] bg-white" /> : <div className="overflow-hidden rounded-xl border border-[#DDE7F0] bg-white"><div className="flex flex-wrap gap-2 border-b border-[#E7EEF3] bg-[#FBFCFD] px-4 py-3">{excelPreview?.sheets.map((sheet, index) => <span key={sheet} className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${sheet === excelPreview?.activeSheet ? "bg-[#E6F6F2] text-[#087A6A]" : "bg-[#F0F5F8] text-[#60758A]"}`}>{sheet}</span>) || <span className="text-xs text-[#71869A]">Đang đọc workbook...</span>}</div><div className="overflow-auto"><table className="w-full min-w-[720px] text-left text-xs"><tbody>{excelPreview?.rows.map((row, rowIndex) => <tr key={rowIndex} className={`border-b border-[#EDF2F5] ${rowIndex === 0 ? "bg-[#F4FBFA] font-extrabold text-[#193B57]" : "text-[#527089]"}`}>{Array.from({ length: Math.max(1, excelPreview.rows[0]?.length || 1) }, (_, cellIndex) => <td key={cellIndex} className="max-w-[260px] truncate px-3 py-2.5">{String(row[cellIndex] ?? "")}</td>)}</tr>)}</tbody></table>{excelPreview && excelPreview.rows.length === 0 && <div className="px-5 py-12 text-center text-sm text-[#71869A]">Không thể hiển thị nội dung xem trước, nhưng bạn vẫn có thể tải file.</div>}</div></div>}</div>
