...
export function ExportPreviewHost() {
  ...
  return <div onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }} className="fixed inset-0 z-[100] flex items-center justify-center bg-[#102A43]/55 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Xem trước ${payload.title}`}>
    <div className="flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-[#DDE7F0] bg-white shadow-[0_26px_70px_rgba(16,42,67,0.28)]">
      ...
      <div className="flex flex-col-reverse gap-2 border-t border-[#E7EEF3] px-5 py-4 sm:flex-row sm:justify-end"><button onClick={close} className="rounded-lg border border-[#DDE7F0] px-4 py-2 text-xs font-bold text-[#60758A] hover:bg-[#F7FAFC]">Đóng xem trước</button><button onClick={() => { downloadPreviewFile(payload); close(); }} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#0F8C8C] px-4 py-2 text-xs font-bold text-white hover:bg-[#087A6A]"><Download size={15} />{payload.kind === "pdf" ? "Tải PDF" : "Tải Excel"}</button></div>
    </div>
  </div>;
}
